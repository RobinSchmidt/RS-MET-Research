...
\begin{tikzpicture}
    ...
    \draw[->] (\ra-0.5,0) -- (4+0.5,0);
    \draw[->] (0,\ra-0.5) -- (0,4+0.5);
    \draw[dashed] (\ra,0) -- (\ra,1) node[above] {$a$};
    \draw[dashed] (4,0) -- (4,-1) node[below] {$b$};
    
    %Right-Hand
%    \foreach \x in {\ratwo,0,...,4} % <--- my issues
%    \draw[thick, fill=red!25] (\x-.5,0) -- (\x-.5,{sin(deg(\x))}) -- (\x,{sin(deg(\x))}) -- (\x,0) -- cycle;
    
    %Left-Hand
    \foreach \x in {\ra,-0.5,...,\rbtwo} % <--- my issues
    \draw[thick, fill=blue!25] (\x,0) -- (\x,{sin(deg(\x+0.25))}) -- (\x+.5,{sin(deg(\x+0.25))}) -- (\x+.5,0) -- cycle;
    
    \draw[ultra thick, domain=\ra:4, smooth, samples=100, variable=\x] plot ({\x},{sin(deg(\x))});
