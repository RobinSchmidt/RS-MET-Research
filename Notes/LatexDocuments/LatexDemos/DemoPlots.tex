...
Figure \ref{Fig:Monomials} shows monomials of degrees up to 5 with \verb|xscale=6, yscale=3|.
...
Figure \ref{Fig:SineAndCosine} shows the sine and cosine function in default size, i.e. without using \verb|scale|, \verb|xscale|, \verb|yscale|. 
...
\draw[color=rsBlue]   plot (\x,{sin(\x r)}) node[right] {$\sin x$};
\draw[color=rsGreen]  plot (\x,{cos(\x r)}) node[right] {$\cos x$};
...
\draw (2,2) node[dot, rsBlue, label = {above:$\mathbf{p}_1$}]{};
\draw (4,3) node[dot, rsBlue, label = {above:$\mathbf{p}_2$}]{};

% Vector v and its components:
\draw[rsRed, <->] (2,2) -- (4,2);
\draw[rsRed] (3,2) node[below] {$v_x$};
\draw[rsRed, <->] (4,2) -- (4,3);
\draw[rsRed] (4,2.5) node[right] {$v_y$};
\draw[rsRed, ->] (2,2) -- (4,3);
\draw[rsRed] (3,3) node[below] {$\mathbf{v}$};
