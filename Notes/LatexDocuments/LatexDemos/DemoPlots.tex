...
\begin{tikzpicture}
\begin{axis}[xmin=-2,xmax=2,ymin=-0.2,ymax=5.2,samples=100,thick,legend pos=north west]


  \addplot[domain=-2:2, color=black] {exp(\x )};
  \addlegendentry{$\exp(x)$}
  
  \addplot[domain=-2:2, color=rsRed] {1};
  \addlegendentry{$T_0(x)$}
  
  \addplot[domain=-2:2, color=rsYellow] {1 + x};
  \addlegendentry{$T_1(x)$}  
  
  \addplot[domain=-2:2, color=rsGreen] {1 + x + x^2/2};
  \addlegendentry{$T_2(x)$}    
