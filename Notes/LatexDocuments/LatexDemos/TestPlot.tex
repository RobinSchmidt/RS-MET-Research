...
\begin{figure}[h]
\centering
\caption{Complete and partial torus}
\label{Fig:Tori}

\pgfplotsset{width=8cm,compat=1.18}
\begin{tikzpicture}
\begin{groupplot}[group style={group size=2 by 1}, height=6cm, width=9cm]

% Complete torus:
\nextgroupplot[view={60}{60}]
  \addplot3[surf, colormap/viridis, samples=30, domain=0:2*pi, y domain=0:2*pi, z buffer=sort]
...
% Partial torus:
\nextgroupplot[view={60}{60}]
  \addplot3[surf, colormap/viridis, samples=30, domain=0:2*pi, y domain=0:2*3*pi/4, z buffer=sort]
  ({(3+cos(deg(x)))*cos(deg(y+pi/2))}, 
   {(3+cos(deg(x)))*sin(deg(y+pi/2))}, 
   {sin(deg(x))});
...
% But modified - different view angle and colormap
