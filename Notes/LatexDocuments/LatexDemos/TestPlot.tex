...
\begin{axis}[xlabel={$x$}, ylabel={$y$}, xmin=-1.5,xmax=1.5,ymin=-1.5,ymax=+1.5, samples=15]
...
\pgfplotsset{width=16cm,height=10cm,compat=1.12}
\begin{tikzpicture}[thick, >=stealth']
\begin{axis}[xlabel={$x$}, ylabel={$y$}, xmin=-6,xmax=6,ymin=-5,ymax=+5]
...
% ToDo: the arrow-head still looks kinda ugly - try to improve that
