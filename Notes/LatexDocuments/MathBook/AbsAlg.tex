...
In the course of this book, we have encountered a great number of so called algebraic structures such as different number systems, vector spaces, etc.. So far, each of these structures more or less stood by itself. It's time to take a step back and recognize commonalities and differences among them and bring some order into this disorganized chaos. Doing so is the realm of abstract algebra. It abstracts out the commonalities between these disparate algebraic structures and establishes a common terminology and notation to talk about such things. Furthermore, it establishes some theorems that will hold for all structures that satisfy a certain set of axioms. This may potentially economize a lot of math: if you come up with a new algebraic structure by yourself and are able to verify that the axioms of a given known structure hold for your structure, you will immediately have a whole bunch of proven theorems in your hand without having to wade through all the tedious proofs for your concrete case again. Checking, if a smallish set of axioms holds true for your new structure will typically be much easier than trying to re-prove each and every theorem from some other known structure for the new structure. It also proves that certain structures that you may hope to discover (like, say, a 3D division algebra) cannot possibly exist, potentially saving you from some headaches in a futile search. The main concepts of abstract algebra are: groups, rings, fields, vector spaces and algebras. These 5 are specific classes of algebraic structures that are important enough to have their own names and they build on top of each other in the given order. There are a couple of more named algebraic structures but this would be my personal list of the most important ones. The other ones are more like side quests from this mainline. A simple algebraic structure consists of a set and one or more operations. These operations take as input a certain number of elements of the set and produce as output another element of the set. Especially important are the binary operations which take two elements as input. The different algebraic structures differ in their requirements for what sort of operations must be present and which rules the operations have to satisfy - things like associativity, commutativity, presence of neutral elements, etc.. In some more complex algebraic structures (e.g. vector spaces), there may be more than one set (vector spaces have two: scalars and vectors). Abstract algebra doesn't really fit into the usual dichotomy between discrete and continuous mathematics. It's more about overarching principles that apply to both of these big branches. What we will learn here is so universal that it will apply virtually everywhere. So let's get started.

% ToDo: Point out relation to elementary algebra which is about solving equations for unknowns and/or 
% simplifying and rearrange equations. We
% try to find explicit formulas for the unknowns. Abstract algebra is more concerned with questions
% about characterizing which equations are and aren't solvable in the first place, I think - and in
% which structures they are or aren't solvable. So the focus of attentention shifts from deriving
% explicit formulas for the solutions (which is not generally possible anymore for equations of
% higher degree) to questions like: How many solutions are there (zero, one, finitely many, 
% infinitely many, etc.). The answer to thsi question may depend on *where* we look for these 
% solutions, i.e. in which field. The field in which the solutions lie may have to be an extension
% to the field from which the coefficients in the equation(s) come from.  For example, x^2 = 2 has
% solutions in R but not in Q and x^2 = -1 has solutions in C but not in R. To solve x^2 = 2, we can
% also use Q[sqrt(2)] (Q-adjoin sqrt(2)) which is an extension of Q just barely large enough to
% contain the solutions. Constructing such extension fields is a big theme in field theory and
% Galois theory.

% TODO: Make the beginnings of the chapters conistent: Start with a couple of intrdouctory sentences for the motivation, Make a "Notation and Terminology" paragraph, Make an "Axioms" paragraph. Currently, the "Axioms" are listed within the introductory text. But maybe we can also do it consistently like that. But it seems better to introduce the notation before stating the axioms becuse the axiom already use the notation. ...although it may actually be better to list the axioms first because in the "notation and terminology" paragraph, we also list notation and terms that we will not use. They are an overview over what is used in the literature
...
% Maybe it can be said that abstract algebra is about finding the rules by which operations interact? And when we have these rules figured out, we can use them to simplify certain sequences (or rather trees) of operations into different ones that are easier to carry out (i.e. evaluate). For example, when we have an expression a*b + a*c which requires two muls and one add to evaluate directly, we can simplify this to a*(b+c) which requires only one mul and one add. The reason we are allowed to do this is the distributive law. the associative law may help us to optimize evaluation of matrix products with many factors.

% But I think, the biggest goal is not simplicfication of calculations but solving equations.
...
From Magmas to Fields: a trippy excursion through algebra - SoME2 3b1b
https://www.youtube.com/watch?v=y1b1GxZ-52s
-Set:             no further structure required
-Magma:           has a binary operation
-Semigroup:       the operation is associative
-Monoid:          there is a neutral element
-Group:           there are inverse elements
-Abelian group:   the op is commutative
-Rngs:            there's a 2nd associative op (multiplication) that distributes over the 1st
-Rings:           mul has also a neutral element
-Comm. Rings      mul is commutative
-Fields           mul has inverse elements

Concrete examples after which the structures in abstract algebra are modeled:
Groups: Permutations, Symmetries, Integers with only addition, Rationals without zero Q* with only
        multiplication
Rings:  Integers Z, Rings of polynomials (very important!), matrices
Fields: Rationals Q, Reals R, Complexes C, Finite fields (modular arithmetic and Galois fields)
Vector Spaces: Euclidean spaces R^n, Space of matrices, functions, complex vector spaces,
               Eigenspaces of matrices or linear operators, projective space, spaces of 
               geometric algebra, spaces resulting from adjoining elements to a field like
               Q[x], Q[i], Q[sqrt(2)], ... -> algebraic number theory

How do Lie groups fit in - in particular when commutators are involved? 

Algebra, Group, Ring, Rng, Field, Monoid, Vector space | Abstract algebra systematized
https://www.youtube.com/watch?v=PaNAUBilkLQ
https://github.com/kuyzirf/algebraic_structures/blob/main/Algebraic_structures.pdf


The Unreasonable Effectiveness of Abstraction in Math
https://www.youtube.com/watch?v=rwOu56bR1OA
-Levels: 1: Visual, 2: Numbers, 3: Variables, 4: Structures, 5: Categories
