...
\end{itemize}
% ToDo: maybe instead of "itemize", use the same formatting as below (using tabular)
...
% Galois Fields: Arithmetic on a Finite Set of Numbers #SoME5
% https://www.youtube.com/watch?v=HmJsvtWZHAM
...
% A common ,method to extend fields is to consider $\mathbb{F}[x] / (p(x))$ with some irreducible polynomial p(x). The parentheses around this polynomial are not just cosmetic - they are meant is the sense of the notation for "ideal generated by". This extension includes a root of that polynomials
% see:
% https://www.youtube.com/watch?v=oRH-HTrAvxw
...
% I think, NaN is not exactly the same as the bottom element of wheel theory because the NaNs are signed whereas the bottom is unsigned
...
https://www.youtube.com/watch?v=Gl3TeygqBbE
...
You can divide by Zero, but ... | How Math works
https://www.youtube.com/watch?v=JMw0yGgs-gw	

ToDo: Explain field isomorphisms:
https://proofwiki.org/wiki/Definition:Field_Homomorphism
https://proofwiki.org/wiki/Definition:Isomorphism_(Abstract_Algebra)/Field_Isomorphism
-The definition of a field homomorphism looks actually exactly like the one for a ring 
 homomorphism. Shouldn't there be additional requirements such as that the morphism must also
 preserve inversion operation? Like phi(1/a) = 1/phi(a). But maybe that follows already from the 
 two requirements? For group and ring homomorphism we actually also do not require that
 phi(-a) = -phi(a), so yeah - this probably follows automatically. Figure out why and add that
 finding to the explanation of group isomorphisms. Explain why from the definition of a group
 homomorphism phi follows phi(-a) = phi(a). Maybe formualte it as a theorem.


https://de.wikipedia.org/wiki/Zerf%C3%A4llungsk%C3%B6rper
