...
The first 3 items describe infinite families of groups whereas the last item, the sporadic groups, are outliers that do not fit into any of the systematic ways to construct groups. Let's now have a look at what kind of groups these classed s entail

\paragraph{Cyclic Groups}
The cyclic groups are named like that because they arise from cycling through their elements from a single generator [VERIFY]. We can think of them as encoding the rotational symmetries of regular polygons. We can also think of them in terms of modular addition [VERIFY]. TBC...
% why do we need primes? With non-primes, modular addition gives also a group - but maybe then it's not a simple group but a composed group.
