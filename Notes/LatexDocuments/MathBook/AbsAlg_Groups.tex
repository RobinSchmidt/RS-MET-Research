...
A permutation of a finite set $A = \{ 1,2,\ldots,n \}$ with $n$ elements is an arrangement of its elements. Sets themselves do not have any notion of order, so we denote such an arrangement of the $n$ elements in the form of an $n$-tuple. Let's take the set $A = \{ 1,2,3 \}$ and assume its natural order is $(1,2,3)$. ...TBC...

% but sets don'T care about order? Shouldn't w use tuples?
% https://en.wikipedia.org/wiki/Permutation

\subsubsection{Transpositions and Parity}
Given a tuple $x = (x_1, x_2, \ldots, x_n)$ we call the act of swapping two elements in this tuple a \emph{transposition}.
% Tranposition = Swap
% https://en.wikipedia.org/wiki/Cyclic_permutation#Transpositions

\subsubsection{Decomposition into Cycles}
