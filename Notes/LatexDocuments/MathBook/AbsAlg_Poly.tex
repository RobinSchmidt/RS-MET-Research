...
%\section{Theory of Polynomials} % Maybe rename to just Polynomials
...
% In a way, the theory of polynomials is the point where all the other classical topics of abstract alegebra (groups, rings, fields, vector spaces) culminate in order to provide a theory about the solution structure of polynomial equations. We will encounter polynomial rings over a given field or ring (and certain quotient rings of those polynomial rings), we will meet the "Galois group" and "splitting field" of a polynomial where the latter will usually be represented in terms of a vector space.

% So, there are two distinct ways in which we may view the algebra of polynomials: (1) as an algebraic structure arising from the polynomials themselves by adding, multiplying, subtracting and dividing polynomials which doesn't require evaluation and (2) as an attempt to answer questions about the solution set of a given specific polynomial equation which requires evaluation.

% Polynomial algebra is the field of math where all the ideas discussed so far in abstract algebra come together. A set of polynomials over a given _field_ of coefficients is itself a _commutative_ring_. This is true for univariate and multivariate polynomials. When we focus on the _vector_ of coefficients rather than the polynomial as complete expressions, our set of polynomials can also be seen as a _vector_space_. The vector space of all polynomials over a given field is infinite dimensional but we often look at finite dimensional subspaces of that by limiting the degree of the polynomials. When we view the polynomials not just as formal expressions but rather as functions, we can ask questions about their zero sets. For univariate polynomials, these zero sets happen to be discrete sets of points called the "roots" of the polynomial. When we consider certain ways to permute these roots, we are lead to the idea of the Galois _group_ of the given polynomial (verify!). Now I have mentioned all 4 of the fundamental algebraic structures we have seen so far - groups, rings, fields and vector spaces. In polynomial algebra, all of this groundwork culimnates in powerful theorems about the roots of polynomials (Abel Ruffini Theorem)

% There's also Bezout's Theorem - not sure if that's considered part of polynopmial ablgebra, though. More generally, there are theorems from algebraic geometry...
% ...Hilbert's Basis Theorem...

% Explain that in the light of the previous chapter, we could veiw the algebra of polynomials as an infinite dimensional algebra over R (verify!) although we more typically view the set of polynomials as a ring rather than an algebra. I think, it's both (verify!). Maybe the ring point of view is more common because the idea of ideals turns out to be an important concept in the context of polynomial algebra because ideals define algebraic varieties.

% TODO: Figure out if polynomial algebra and commutative algebra are often used interchangably.
% Nah - maybe not: 
% https://en.wikipedia.org/wiki/Commutative_algebra
% But we usually use polynomials to present the material, I think. Wikiepdia also says that it
% was formerly often called "Ideal Theory"
...
https://www.youtube.com/watch?v=eVzIrn7hE4w

You Mean We CAN Solve A Quintic? | Practical Galois Theory #2 | #SoME4
https://www.youtube.com/watch?v=0E-XqpdHacQ


How to Get Galois Groups Using Field Extensions
https://www.youtube.com/watch?v=Tg4g0iyDQ2g

Ramanujan Did WHAT?? | Practical Galois Theory #3 | #SoME4
https://www.youtube.com/watch?v=m_9CsmcV1to

I can solve any quintic equation!!
https://www.youtube.com/watch?v=t4u_lwONAdc

An Overview of the Exact Solution to the General Quintic...and Why You Should Never Use It
https://www.youtube.com/watch?v=SFXQ_920d4s

Ramanujan Did WHAT?? | Practical Galois Theory #SoME4
https://www.youtube.com/watch?v=zk6tZZXQf6I

Mystery of the Quintic Formula
https://www.youtube.com/watch?v=9HIy5dJE-zQ

The Proof That Ended a 300-Year Search
https://www.youtube.com/watch?v=WJcDKuZZH_0

This One Idea Explains i, j and epsilon
https://www.youtube.com/watch?v=9KIPA-Yv9TU

Why the Quintic Has No Formula
https://www.youtube.com/watch?v=Ga3LouuNdTI

ToDo:

-Explain why and how the field of complex numbers can be expressed as
 R[x]/(x^2 + 1). See "Zahlen" (Ebbinghaus), p. 51. What do residual class rings (Restklassenringe) 
 have to do with it? Try multiplying two polynomials from R[x] and compute the remainder of the 
 division by p(x) = 1 + x^2 and compare that to what happens when just using the formula x^2 = -1 
 which is true in the special case when we set x = i. Will they always be equal? I guess so. But 
 why should that be the case?
 See also: Satz von Kronecker / Kronecker's Theorem:
 https://de.wikipedia.org/wiki/Satz_von_Kronecker_(K%C3%B6rpertheorie)

-Figure out what happens if we consider only polynomials up to a given degree n and replace
 the convolution in the multiplication by a circular convolution. Also consider polynomials
 modulo some given polynomial. Maybe try f(x) = x^n
