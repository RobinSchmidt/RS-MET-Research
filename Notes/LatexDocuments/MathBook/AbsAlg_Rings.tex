...
In some contexts, it makes sense to start with a given \emph{field}\footnote{A field is a ring with certain additional properties and will be discussed in detail in the section about field theory at page \pageref{Sec:FieldTheory}} such as $\mathbb{Q}$, $\mathbb{R}$ or $\mathbb{C}$ rather than just a ring like $\mathbb{Z}$ and adjoining elements to that field. For example, rather than forming the ring $\mathbb{Z}[x]$ one could form the ring $\mathbb{R}[x]$ which is the ring of polynomials over the real numbers, i.e. polynomials with real coefficients. One could also adjoin more than one variable symbol like $x$, for example two like $x$ and $y$ and thereby form the ring of bivariate polynomials over $\mathbb{R}$.  ...TBC...


%\paragraph{The Polynomial Ring $\mathbb{Z}[\mathbf{x}]$}
