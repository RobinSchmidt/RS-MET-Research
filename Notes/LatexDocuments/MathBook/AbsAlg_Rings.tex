...
In the integers, we have the two units $1$ and $-1$. Any two numbers that differ only by a factor given by one the units are considered to be \emph{associated} with each other. For example $-5$ is associated with $5$ because they differ only by a factor of $-1$ which is a unit. In any ring, pairs of additive inverses are always associated with each other. In the Gaussian integers, any $a + b \i$ will be associated with $-a - b \i$. But there are more associations. In the Gaussian integers, we have 4 units, namely $1,-1,\i,-\i$ where 1 and $-1$ are still their own inverses and $\i$ and $-\i$ are inverses of each other. Multiplication by $\i$ rotates by 90 degrees counterclockwise and multiplication by $-\i$ by the same angle clockwise. In the Gaussien integers, any number $a + b \i$ will also be associated with its complex conjugate $a - b \i$ and the negative of the complex conjugate $-a + b \i$, so any Gaussian integer is a member of a quartet of 4 associated numbers. In a sense, these 4 numbers are considered to be equivalent. Associatedness is an equivalence relation on a ring $R$. If $x$ is associated with $y$, we will write $x \sim y$.
