...
\section{Ring Theory}
After having seen groups, the next idea to explore is that of a ring. We assume to have a commutative group $\mathcal{G} = (G, +)$ as a starting point. Note that here we use $+$ to denote our commutative group operation instead of the previous usage of $\cdot$ or juxtaposition within group theory. That is because in ring theory, the dot or juxtaposition will be needed for a second operation that we now want to introduce. We have seen that the integers form a group under addition. But we can also multiply integers. We now want to capture the behavior of integers under addition and multiplication in an abstract way. To this end, we assume that we start with a commutative group and we will call the group operation "addition" from now on. We assume furthermore that we have defined a second operation between elements of the group which we intend to be our abstract "multiplication". 

\paragraph{Axioms}
Abstractly, a ring $\mathcal{R}$ can be defined as a triple $\mathcal{R} = (R,+,\cdot)$ consisting of a set $R$ with two binary operations between elements of the set such that the following rules hold true, which we shall henceforth call ring axioms:
\begin{itemize}
...
% ToDo: Maybe instead of "itemize" use the same formatting as for the group axioms and field axioms
...
% Mention that closure under addition and multiplication taken together imply closure under linear combinations (maybe together with associativity of addition).
...
% My favorite isomorphism
% https://www.youtube.com/watch?v=1GC8CYOXai0
% -About the ring isomorphism between the complex plane and 2x2 matrices of the form [a,-b; b,a]
...
Instead of adjoining an unspecified symbolic variable $x$, we may also adjoin the symbol $\i$ with its concrete interpretation as the imaginary unit with the property $\i^2 = -1$. At first glance, we seem to be creating again the ring of all polynomials over $\mathbb{Z}$. However, the special property $\i^2 = -1$ will have the effect that all polynomials of degree higher than 1 will boil down to a linear polynomial of the general form $a_0 + a_1 \i$ where $a_0,a_1 \in \mathbb{Z}$. This happens because $\i^2$ and higher powers of $\i$ can be simplified into $\pm 1$ and $\pm \i$. Consider the polynomial $2 - 3 \i + 4 \i^2 - 5 \i^3 + 6 \i^4 - 7 \i^5$. Wouldn't we have any rule, we would just keep it as a polynomial in $\i$ and we would be working with a polynomial ring just that we would now call our variable $\i$ instead of $x$. But we do have the rule $\i^2 = -1$ and when we apply it, the polynomial simplifies to $4 - 5 \i$. This will always happen. Every element of our new ring can be expressed in the form $a + b \i$ where $a,b \in \mathbb{Z}$. We denote that ring by $\mathbb{Z}[\i]$ and call it "$\mathbb{Z}$-adjoin-i" or the "Gaussian integers" after Carl Friedrich Gauss who worked a lot with numbers of that kind in his number-theoretic investigations. The picture we should have in mind for these Gaussian integers is the lattice of points of complex numbers whose real and imaginary part are integers, i.e. a lattice of points in the complex plane.

\paragraph{The Polynomial Rings $\mathbb{R}[x]$, $\mathbb{R}[x,y]$, $\mathbb{R}[x_1,\ldots,x_n]$}
In some contexts, it makes sense to start with a given \emph{field}\footnote{A field is a ring with certain additional properties which will be discussed in detail in the section about field theory at page \pageref{Sec:FieldTheory}} such as $\mathbb{Q}$, $\mathbb{R}$ or $\mathbb{C}$ rather than just a ring like $\mathbb{Z}$ and adjoining elements to that field. For example, rather than forming the ring $\mathbb{Z}[x]$ one could form the ring $\mathbb{R}[x]$ which is the ring of polynomials over the real numbers, i.e. polynomials with real coefficients. One could also adjoin more than one variable symbol like $x$, for example two like $x$ and $y$ and thereby form the ring $\mathbb{R}[x,y]$ of bivariate polynomials over $\mathbb{R}$, i.e. the ring of polynomials in two variables with real coefficients. Of course there is nothing special about two variables. We could adjoin an arbitrary number of variables, say $x_1,x_2, \ldots, x_n$, to form the ring of polynomials in $n$ variables $\mathbb{R}[x_1,\ldots,x_n]$. By putting the individual variables $x_1,\ldots,x_n$ into a vector $\mathbf{x}$, we could also denote this ring as $\mathbb{R}[\mathbf{x}]$ although I don't know if that notation is common (figure out!).  ...TBC...


% ToDo: Say a bit more about polynomial rings. They seem to be very a important application/motivation/concrete example in ring theory. Rings of polynomials happen to be commutative rings and they seem to be the primary subject of interest in commutative algebra (verify). Of particular interest in algebraic geometry are rings of multivariate polynomials. In these rings, ideals are defined by sets of polynomial equations. A bunch of polynomials p_1(\mathbf{x}),  p_2(\mathbf{x}), ... can be seen as a basis for an ideal in the polynomial ring $\mathbb{F}(\mathbf{x})$ where $\mathbb{F}$ can be an arbitrary field. Such an ideal, defined by a given basis for it, defines an algebraic variety. ...TBC... It's important to notr that algebraic varieties are defined by an ideal, not by a particular set of defining equations. The set of the defining equations is just one of infinitely many ways to specify the same variety. It's a bit like with a vector space that can be defined in terms of specifying a basis for it - but a given vector space can have infinitely many different bases. When we are given a set of polynomial equations that to specify an algebraic variety, we can take this set as basis for the ideal that corresponds to that avreity. But there are different bases and we can transform our given basis into a different one by certain algorithms. One important such algorihtm is the Buchberger algorithm which transforms a given ideal (specified via a set of polynomial equations) into a special kind of basis, known as a Groebner basis, which has some convenient properties. ...TBC...

% I think, the ring R[x] is one of the main objects of study in Galois theory (verify!)

% But wait: Above we have defined Z[x]. Here, when we form F[x] we seem to require the underlying set of coefficients to be a field rather than just a ring. I think, we really need a field in order to always be able to to polynomial division. We really do not want the (non)divisibility of coefficients to limit our divisibility of polynomials, I guess. Figure that out! Explain in the text why in this context, we really like to use a field for the coefficients rather than just a ring. We really want the coeffs to always be divisible, I think. Otherwise, many things wouldn't work out as desired. In particular, the polynomial division algorithm which is the basis for many other things. We don't thins algo to get stuck just because the coeffs of the leading terms are not divisible. Could we perhaps modify the algorithm in such a way as to multiply both polynomials in each step by different scalers such that both leading coeffs (of dividend and divisor) are scaled to the lcm of the original coeffs? I think, as long as we are interested only in the zeros, i.e. in manipiulating polynomial equations with a rhs of zero, we should be allowed to scale both, dividend and divisor by different factors in each step.
...
After having seen some methods to create new, larger rings from existing rings, we'll now look into dissecting rings into their important constituents. Some of these constituents that we will identify, will themselves also be rings which we will therefore call \emph{subrings}. That gives us another way to "construct" new rings from existing rings although it might be more appropriate to call it an "extraction" rather than a "construction".
...
A two-sided zero divisor is an element that is both a left and right zero divisor. Note that we do \emph{not} require $x z = 0 \wedge z x = 0$ for some $x$ from a two-sided zero divisor, i.e. we do not require that $z$ annihilates the same element $x$ from the left and from the right. Rather, we require that for each type of product with $z$ being the left or right factor, there exists some element that gets annihilated by $z$ in that product but which element(s) that is(are) may differ for left and right multiplication. Zero itself counts also as zero divisor and is called the trivial zero divisor. Some authors exclude zero itself from the definition of zero divisors. Doing it one or the other way is sometimes more and sometimes less convenient. I opted for this convention because it seems more consistent to me. We will later also define units which are a generalization of the idea of 1 and I view the zero divisors as a similar generalization of 0. Elements that are not zero divisors are also called \emph{regular} or \emph{cancellable}. In non-commutative rings, these may also be qualified as left regular etc. An example for nontrivial zero divisors are the non-invertible matrices in matrix rings. A ring that has no nontrivial zero divisors is called a \emph{domain}. If that ring is commutative, it's sometimes also called an \emph{integral domain}. A product ring has always nontrivial zero divisors. \emph{Nilpotent} and \emph{idempotent} elements are always two-sided zero-divisors. These are defined as elements $x$ for which there exists some natural number $n$ such that $x^n = 0$ (nilpotent) or $x^n = x$ (idempotent). That means, some power of a nilpotent element gives zero and some power of an idempotent element gives back the element itself. 

\medskip
Just for the sake of completeness, I want to mention that there is also a notion of \emph{unipotent} elements. One might guess, that these are elements $x$ for which some power equals unity, i.e. $x^n = 1$ for some $n$. However - that's actually not how this term is defined. Instead, an element $x$ of a ring is said to be unipotent, if $x-1$ is nilpotent, i.e. if $(x-1)^n = 0$ for some $n$. Of course, the $1$ here is meant to be the multiplicative identity in the given ring which we must suppose to exist when we use this definition (remember that not all rings have such a $1$ according to the convention we have adopted). Now one might guess that these two possible definitions may be equivalent - but I don't think so. Consider what happens when expanding $(x-1)^n$ according to the binomial theorem. We'll certainly get a different expression than $x^n$. [TODO: figure out, why this definition was made so counterintuitively.]

% I guess, $(x-1)^n = 0$ is a mathematically more useful property? It reminds me of generalized eigenvalues. But x^n = 1 is something that does happen in practice. For example (-1)^2 = 1, i^4 = 1. But maybe such a definition of unipotent would then be redundant with the definition of idempotent elements because raising an element with the property $x^n = 1$ to the $(n+1)$th power, we would also get $x$ - so idempotent and unipotent would imply each other (right?) and it is therefore not very meaningful to have two different terms for the same property. ...although something like that is also not totally unheard of - think of "analytic" and "holomorphic" functions in complex analysis. But this feature of $x^n = 1$ is also brought up later with the cyclotomic equation. Maybe such elements could be called cyclotomic? Actually, that feature makes $x$ an $n$th root of unity
...
% It says that a unitpotent element x is defined as: x-1 is nilpotent. Is that definition equivalent
% to x^n = 1? I don't think so. Exponentiating (x-1) introduces cross-terms between x and -1 so
% we get a different expression.
% https://de.wikipedia.org/wiki/Unipotente_Matrix
% https://de.wikipedia.org/wiki/Unipotentes_Element
...
\medskip
In group theory, we are very interested in studying the structure of a given group by analyzing the way in which it can be decomposed into subgroups. One might expect that subrings could play a similar role in ring theory. But here, the concept of a subring in and of itself is actually not that important. Instead, what we actually need are subrings with a certain additional property. These very special subrings are called...
...
The notion of an \emph{ideal} is more restrictive than that of a subring. Among other requirements, a subring $S$ must be closed under multiplication. For an ideal $I$ of $R$, the closure under multiplication requirement is tightened: It must be the case that the products of elements $i \in I$ from the ideal $I$ with elements $r \in R$ from the original ring $R$, \emph{not} necessarily from $I$, must again produce elements inside the ideal. The subset is not only closed in the sense that you \emph{cannot leave it} by combining elements from it via the operations but it also acts as an "attractor" in the sense that you immediately \emph{get sucked into it} as soon as you multiply \emph{anything} by an element of it, so to speak. Another way to characterize an ideal is to say, that for every element that is inside the ideal, all of its multiples (as in: products with other ring elements) are also inside the ideal. When you hear "ideal" you may translate that in your head into "absorbing subring". If the ring is not commutative, we must distinguish between left, right and two-sided ideals. For these, we require:
...
The "left" and "right" in this terminology refers to where the factor $r$ from the ring $R$ occurs in the product. A two sided ideal is a left and right ideal at the same time and sometimes just called an ideal without qualifier. In commutative rings, all three notions coincide. The ideal generated by an element $i$ is denoted by $(i)$. As an example, consider again the set of even numbers $2 \mathbb{Z}$. We already know that it is a subring of $\mathbb{Z}$ because any product of any pair of even numbers is again an even number. But is it also an ideal? Yes, because it also satisfies the stronger requirement that the product of any even number with any integer number whatsoever gives again an even number. By similar arguments, any set of the form $n \mathbb{Z}$ is also an ideal in $\mathbb{Z}$ for any $n \in \mathbb{Z}$ [VERIFY]. As an example of a subring that is not an ideal, consider $\mathbb{Z}$ as a subring of $\mathbb{Z}[\i]$. An ideal in $\mathbb{Z}[\i]$ would be $2\mathbb{Z}[\i]$, the Gaussian integers with even real and imaginary parts. [ToDo: explain why]

% ToDo: Check if the notation (i) for an ideal is common. I think, in group theory, we used 
% \langle g \rangle for a group generated by g. Maybe we should use a notation that is consistent with that. Also, we just casually say "ideal generated by an element $i$" as if it would be self-evident that ideals are "generated" this way. I think, this requires some more explanation. Maybe one sentance mentioning taking a ring element r and forming all of its multiples does indeed generate an indeal, iff the so genrated set together with the operations does indeed form a ring. More generally, we can generate ideals by giving a basis b1,b2,b3,... Then all linear combinations r1*b1 + r2*b2 + r3*b3 + ... of these basis elements (with coefficients r1,r2,... coming from the original ring) form the ideal that is generated by the given basis. Again under the assumption that the set generated by all these possible linear combinations happens to be a subring in the first place. VERIFY!
...
% An ideal can be seen as a set of all linear combinations that can be created from the elements of its generating set. It like a space spanned and the generating set is like a basis for this space..
...
%Now that we have defined what the product of two ideals $I$ and $J$ is supposed to be, it is natural to define a \emph{prime ideal} as an ideal that cannot be expressed as a product of two other ideals [!!!VERIFY!!! I've literally made that up myself bcs it seems so natural].
...
We have already met group homomorphisms. These are defined by a function $f: G \rightarrow H$ such that $f(a b) = f(a) f(b)$. Analogously, we define a \emph{ring homomorphism} to be a function $f: R \rightarrow  S$ from one ring $R$ to some (possibly) other ring $S$ that respects both of our ring operations and the multiplicative identity. That is:
\begin{equation}
  f(a   +   b) = f(a)   +   f(b), \qquad
  f(a \cdot b) = f(a) \cdot f(b), \qquad
  f(1_R) = 1_S
\end{equation}
where again, it is implicitly understood that on the left hand side, we use the $+,\cdot$ operations of $R$ and on the right hand side, we use the $+,\cdot$ operations of $S$. By $1_R$ we denote the multiplicative identity in $R$ and by $1_S$ the multiplicative identity in $S$. From this definition follows (among many other facts) that $f(0_R) = 0_S$ and $\forall a \in R: f(-a) = -f(a)$. As you may have guessed, $0_R$ and $0_S$ denote the additive identities in $R$ and $S$ respectively. When we talk about rings and just say "homomorphism" rather than "ring homomorphism" then it is implicitly understood that we talk about a ring homomorphism. TODO: isomorphism, automorphism, list more facts that follow 

% For more facts that follow from the definition, see:
% https://en.wikipedia.org/wiki/Ring_homomorphism#Properties

% Maybe use (or at least mention) the notation:
% $f(a +_R b) = f(a) +_S f(b)$  and  $f(a \times_R b) = f(a) \times_S f(b)$
% Using indices in the operators is more consistent with our usage of indices for the identity
% elements. I think, we should either use explicit notation everywhere or implicit notation 
% everywhere. Maybe state the definition in both notations.
...
% See "Zahlen" (Ebbinghaus), pg. 259. I think, the construction there can be generalized as follows:
%
% Let R be a ring and I be an ideal in R. We then define an equivalence relation ~ in R as follows:
%
%   a ~ b   iff   a-b \in I
%
% With this equivalence relation in hand, we can form the set of equivalence classes with respect 
% to ~. This is our quotient ring denoted as R / I. The relation a ~ b is often written as:
%
%   a \equiv b  \mod I
%
% Maybe we can also write ~_I for the equivalence relation if I is not clear from the context.
%
% Questions: 
% -Does it have to be a _maximal_ ideal? There, it is. Why does it have to be an ideal in
%  the first place? Wouldn't any subset of R work? Figure out!
% -How does the "mod" notation specialize to modular integers? Here, the ideal is the multiples of
%  the modulus m. Let's denote this ideal by M, i.e. M = { k m | k \in Z }. When we write
%  a \equiv b  \mod m, is this an abbreviation of  a \equiv b  \mod M
...
% https://www.youtube.com/watch?v=NZFm1BFw204&list=PLRcOL8MUr-pdt34AEueXl7aCoE7v-Mya_

% ring theory visualized with set operations
% https://www.youtube.com/watch?v=jw29fnxLqaQ
...
- Maybe define absorbing elements. For multiplication, it would be zero. For addition it may be
  +- infinity. For multiplication +inf is absorbing for all a > 0 but not for a <= 0. For max(a,b)
  it would be +inf, for min(a,b) it would be -inf. I'm not sure, if this is standardized.

- Maybe this can be brought up in the context of discussing ideals - because ideals do also have 
  this "absorbing" feature

- Maybe make a section about commutators. They may be an interesting object of study in 
  non-commutative rings (e.g. matrix rings)

Double Modular Arithmetic
https://www.youtube.com/watch?v=iK1gV0J6EOo

The Beautiful Connection between Unique Factorization and Ramanujan's Constant
https://www.youtube.com/watch?v=OKmv9weSMkg
