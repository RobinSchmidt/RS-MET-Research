...
\medskip
As example, take $I = \mathbb{Z}_6, J = \mathbb{Z}_8$ which are both ideals of $\mathbb{Z}$. Their intersection is given by $I \cap J = \mathbb{Z}_{24}$. That is because $24$ is the lowest common multiple of $6$ and $8$ [VERFIY claim].

% You may ask yourself, if we have sum product and quotient, then what about the difference? I think, the answer is that if we would define a difference as $I - J = \{a - b : a \in I, b \in J\}$, which may seem natural, then we would obtain the exact same set as for the sum $I + J$  [VERIFY]. so it wouldn't give us anything new.
