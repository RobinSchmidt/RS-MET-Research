...
The basic commutative laws for addition and multiplication say $a+b = b+a$ and $a \cdot b = b \cdot a$. We may generalize these laws from two operands to any number of operands as follows: In a sum (or product) of arbitrary many summands (or factors), we can reorder these operands in any way we like without changing the final result.

% ToDo: Maybe explain how we can derive this from the basic law. We may need associativity for this as well. Give examples for 3 and 4 factors. 3: abc = acb = bac = bca = cab = cba, 4: abcd = ... Note that the number of ways grows larger really fast. Mention the factorials (it's a forward reference.)
