...
\label{Sec:IteratedBinaryOperations}
...
% In logic, there are also symbols "big-or" and "big-and"

% If it's associative, it doesn't make a difference if we left-fold or right-fold (aka reduce). If it's also commutative, we can reorder the arguments in any way we like. If the operation has a neutral element, we can define the empty iterated operation to be that element (an iteration over just one element is always that element itself). We may even be able to define countably infinite numbers of iterations if a suitable idea of convergence is available.
...
%   order (like left-to-right or right-to-left).
% - In this discussion, some professor seems to be using a curved arrow to indicate a chaining
%   order:  https://math.stackexchange.com/questions/1391874/matrix-product-notation

% Is it generally true that the (left) distributive law a * (b + c) = a*b + a*b implies
% a * (b + c + d + ...) = a*b + a*c + a*d + ...? If so, how can that be proven or at least be
% understood? I tend to assume that to be true but on closer look, I'm not so sure anymore why that
% would be justified. Similarly, does the associative law a + (b + c) = (a + b) + c imply that for
% a chain of arbitrary many operations, we can apply them in any order, i.e. place the parentheses
% in any way? What about commutativity? Does a + b = b + a mean that in an arbitrary sum, we can 
% permute the operands in any way? Or maybe that holds only when associativity also holds? Are there
% actually any operations that are commutative but non-associative? Is such a thing even possible?
% Maybe exponentiation? We have a^(b^c) != (a^b)^c but, I think, we have 
% a^(b^c) = a^(b*c) = (a^c)^b, so maybe we should call it "right-commutative"? The complication is
% that commutativity only involves 2 operands whereas associativity involves 3. Maybe that's why we
% typically learn commutativity first? It's simpler - even though associativity is even more
% important (i.e. more common and without it, a lot more of our nice algebraic rules break down).

%---------------------------------------------------------------------------------------------------
%Generlaized Laws of Arithmetic

% Maybe that section should come before introducing the sum and product notation. It makes sense because these laws are a preliminary for the sum and product notation to be well defined (I think). But: the generalized distributive law makes use of the sum notation which would favor the current order. Maybe keep the order as is but make a little introductory segment that says something like: "In the definitions of the sum and product notations, we actually already assumed certain laws that may seem obvious but which we have not yet spelled out explicitly. We shall now ...(nachholen)... this. Or maybe state the generalized distributive rule without the sum notation using dots like (a_1 + a_2 + a_3 + ...) (b_1 + b_2 + b_3 + ...) = a_1 b_1 + a_1 b_2 + a1_ b_3 ... + a_2 b_1 ...

\subsubsection{Generalized Arithmetic Laws}

\paragraph{Commutativity}
The basic commutative laws for addition and multiplication say $a+b = b+a$ and $a \cdot b = b \cdot a$. We may generalize these laws from two operands to any number of operands as follows: In a sum (or product) of arbitrary many summands (or factors), we can reorder these operands in any way we like without changing the final result. For example, for a product of 3 factors $a,b,c$, we have the following 6 possible ways to order the factors $abc = acb = bac = bca = cab = cba$. With 4 factors, there would be 24 ways to order them. In general, for a product of $n$ factors, there will be $n!$ different ways to order the factors where $n!$ stands for the factorial of $n$ which is explained on page \pageref{Def:Factorial}.

\paragraph{Associativity}
The basic associative laws for addition and multiplication state that in a sum of 3 summands $a + b + c$ or a product of 3 factors $a b c$, we can evaluate these expressions either from left to right as $(a+b)+c$ and $(ab)c$ or from right to left as $a+(b+c)$ and $a(bc)$. This fact is the reason that the expressions $a+b+c$ and $abc$ are well defined in the first place. Without that rule, they would be ambiguous because one wouldn't know in which way one should evaluate the expressions. The generalized associative law says that in a sum of arbitrary many summands or a product of arbitrary many factors, we can set the parentheses in any way we like without changing the final result. For example, for a product of 4 factors $abcd$, we have the following 5 ways to set the parentheses: 

...TBC...ToDo: Explain that we can set the parentheses arbitrarily in a sum (or product) with arbitrary many summands (or factors)

% Give as example the 5 ways to evaluate a product with 4 factors abcd

% Mention the Catalan numbers (also a forward reference)

\paragraph{Distributivity}
We have already seen the distributive law as $a (b + c) = a b + a c$ for numbers on page \pageref{Sec:ArithmeticLaws}. It could also be rewritten as $(b + c) a = b a + c a$ by right multiplying the $(b+c)$ expression by $a$ rather than left multiplying. In some mathematical areas where the product is not commutative, for example when $a$ and $b$ are not numbers but some other kinds of objects, for example matrices (which will be introduced later), we really need the two different distributive laws which can then be called left-distributive and right-distributive laws (see page \pageref{Sec:RingTheory} for more details). Whenever we deal with mathematical objects for which these two distributive laws hold, we can use them to derive the following:
\begin{eqnarray}
(a+b)(c+d) &= a(c+d) + b(c+d) &= ac + ad + bc + bd \\
(a+b)(c+d) &= (a+b)c + (a+b)d &= ac + bc + ad + bd 
\end{eqnarray}
In the top line, we have used right-distributivity first and then left-distributivity and in the bottom line, we used left-distributivity first and then right-distributivity. Together with the commutativity of addition, we observe that the expressions on the right hand side are equal in both lines - which they must be because the left hand sides are literally equal. To generalize further, we can say that whenever we have left- and right distributive laws at our disposal, the following holds:
\begin{equation}
\left(\sum_{i=1}^{m} a_i \right) \left(\sum_{j=1}^{n} b_j \right) =
\sum_{i=1}^{m} \sum_{j=1}^{n} a_i b_j
\end{equation}
The law says that to compute the product of two sums, we need to multiply each summand $a_i$ in the first sum which each summand $b_j$ and the second sum and then take the sum of all of these pairwise products $a_i b_j$. Due to the generalized commutativity and associativity of addition, it does not matter in which order we compute the sum. This formula can be seen as a direct consequence of the distributive law and could therefore also seen as a generalization of the distributive law. It reduces to the old distributive law if one of the sums has only one summand and the other has two. If you do an internet search for "generalized distributive law", you will find some stuff about message passing algorithms which seem to use a law that is an even further generalization and refer to that as generalized distributive law. ToDo: Maybe state and explain that even more general law, too. Check if for this generalization to apply, we also need other laws (such as associativity of addition).

% Maybe mention that the left hand side will usually be the more efficient evaluation algorithm (I think) such that the formula's purpose is not to give us a better evaluation algorithm. But maybe we can use it in reverse to simplify certain calculations?

% Q: Does a(b+c) = ab + ac imply a(b+c+d) = ab + ac + ad? I think, for the expression (a+b+c) to be well defined, we need + to associative because otherwise it wouldn't be clear if mean ((a+b)+c) or (a+(b+c)). Only when we have associativity of +, that doesn't matter. In this case, we could evalaute it as a(b+c+d) = a((b+c)+d) = a(b+c) + ad = ab + ac + ad  or, equivalently as a(b+(c+d)) = ab + a(c+d) = ab + ac + ad.

% I also think, with associativity of addition, the right hand sides ac + ad + bc + bd  and  ac + bc + ad + bd  would not be well defined without parentheses. I think, they should be set like that:
%  (ac + ad) + (bc + bd)   and   (ac + bc) + (ad + bd) ....verify! So that means, a generalized distributive law that doesn't assume associative additian would look like that? Maybe try it with a non-associative (but neverthless distributive) structure. Is there such a thing? If not, can we invent one?

% There is actually a non-contrived (and actually very much practically relevant) example of a structure where addition is not associative, namely: an add-and-round operation in the real numbers which is what we in fact do in practice all the time - also with floating point numbers on a computer. Example: Let's assume that we use numbers with 3 significant decimal digits and we form the 3-term sums (123 + .456) + .123  vs  123 + (.456 + .123). In the first case, we would get 123 as result and in the second case 124 when we assume that we round to 3 digits after each 2-term addition. Interestingly, the commutative law is actually not affected. It still holds for add-and-round. So, we have an example for a structure where commutativity holds but associativity doesn't! Intersting! Examples for it being the other way around are rather common in math - just take matrix-multiplication, for example.

% https://en.wikipedia.org/wiki/Associative_property#Nonassociativity_of_floating-point_calculation

% Could it make sense to define a structure where we round after addition but do not round after multiplication? In such a structure, we may have to zero-pad operands, when one of them has less precision that the other.

% ToDo: make similar sections about generalized associative and commutative laws. The geenralized associative law would say that in an expression like a+b+c+d, we could set the parentheses in whichever way we want. The generalized commutative law would say that we can reorder the terms in whichever way we want. Maybe give all the possibilities for the case of 3 and 4 terms. The number of ways to do that are given by the Catalan numbers and the factorials respectively. But: The factorial is intoduced only later. But it's directly below so maybe it's a god way for a transition, i.e. say, "the number of ways to do it is given b the factorial, as descirbed below..."

% Maybe then call the subsubsection "Generlaized Laws of Arithmetic" with paragraphs "Associativity", "Commutativity", "Distributivity" ...maybe do commutativity first just like we did in the initial introdcution of the laws. Commutativity is the easiest to understand - although associativity is in some ways more basic and important. There are many relevant structures where associativity holds but commutativity does not.

% Commutativity:
% a+b+c = a+c+b = b+a+d = b+d+a = c+a+b = c+b+a  and abc = acb = bad = bda = cab = cba
%   abcd = abdc = acbd = acdb = adbc = adcb
% = bacd = badc = bcad = bcda = bdac = bdca
% = cabd = ....
% We order the different possibilities lexicographically. It follows directly from the 2-term rule by applying it to one of pairs (like bc) and then to the result of the operation and the other operand. But: does that imply that the "can reorder in any way" requires associativity as well? Maybe try to test it with non-associative (but commutative) structure like floating point numbers

% https://en.wikipedia.org/wiki/Associative_property#Generalized_associative_law
% https://math.stackexchange.com/questions/2459697/prove-generalized-associative-law
% https://ntouikan.ext.unb.ca/MATH6022/IntroCGGT/html_output/sec_gen_assoc.html

% I think, we indeed need associativity of addition because we deal a special case of the situation explained above with the "iterated binary operation" which is addition in this case here.

% See also:
% https://en.wikipedia.org/wiki/Generalized_distributive_law
% -This seems to use the term "generalied distributive law" for a further generaliation

% https://en.wikipedia.org/wiki/Distributive_property#Generalizations

% Explain also lerf-associative and right-associative operations:
% https://en.wikipedia.org/wiki/Associative_property#Notation_for_non-associative_operations
% examples: division, subtraction, exponentiation
...
\end{equation}
The factorial of $0$ is defined to be $1$. This definition is consistent with the definition of the empty product: $\prod_{k=1}^{0} k = 1$. With the factorial, we can define the so called \emph{binomial coefficients} as follows:
\begin{equation}
...
\end{equation}
ToDo: Give more formulas. Especially important are the various recursion formulas, I think. Maybe evaluate the expression $(a+b)^n$ for $n=2,3,4,5$ to show what's happening. Maybe give Pascal's triangle - although, we already do that in the Combinatorics section - maybe refer to there for details.
...
% -About a general formula for (a+b)^n where n can be any real number
% -Has also a nice demonstration of why these binomial coeffs can be computed via Pascal's trinagle
%  (at 2:00 min). You add up two shifted (by 1) copies of the line above. 
...
...TBC...TODO: Give general formula (Faulhaber's Formula), explain how these formulas can be used to compute sums that don't start at 1 and/or have an increment other than 1 (general arithmetic progressions)
...
% ToDo: give other important sum formulas
...
https://de.wikipedia.org/wiki/Zwei-Quadrate-Satz#Eulers_Beweis_mit_der_Methode_des_unendlichen_Abstiegs
-Maybe use a,b,c,d instead of u,v,x,y
...
The Factoring Trick Schools Skip
https://www.youtube.com/watch?v=yEzUTTIb0mU
-Solves 1/x + 1/y = 1/7 for positive integers x,y
...
How to solve ANY equation w/ one variable and one solution.
https://www.youtube.com/watch?v=H-SapkufU-E

Maybe make a little section about the various types of equations that arise in math.
See:  C:\Users\rob\GitRepos\RS-Private\Documents\MathNotes\TypesOfEquations.txt
