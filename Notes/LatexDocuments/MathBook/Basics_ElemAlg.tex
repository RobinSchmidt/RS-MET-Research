...
We have already seen the distributive law as $a (b + c) = a b + a c$ for numbers. It could also be rewritten as $(b + c) a = b a + c a$ by right multiplying the $(b+c)$ expression by $a$ rather than left multiplying. In some mathematical areas where the product is not commutative, for example when $a$ and $b$ are not numbers but some other kinds of objects, for example matrices (which will be introduced later), we really need the two different distributive laws which can then be called left-distributive and right-distributive laws. Whenever we deal with mathematical objects for which these two distributive laws hold, we can use them to derive the following:
\begin{eqnarray}
