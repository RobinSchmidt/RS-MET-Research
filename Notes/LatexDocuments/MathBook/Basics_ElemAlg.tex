...
\end{equation}
The law says that to compute the product of two sums, we need to multiply each summand $a_i$ in the first sum which each summand $b_j$ and the second sum and then take the sum of all of these pairwise products $a_i b_j$. This formula can be seen as a direct consequence of the distributive law and could therefore also seen as a generalization of the distributive law. It reduces to the old distributive law if one of the sums has only one summand and the other has two. If you do an internet search for "generalized distributive law", you will find some stuff about message passing algorithms which seem to use a law that is an even further generalization and refer to that as generalized distributive law. ToDo: Maybe state and explain that even more general law, too. Check if for this generalization to apply, we also need other laws (such as associativity of addition).
