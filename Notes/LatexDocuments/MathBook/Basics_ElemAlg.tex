...
% There is actually a non-contrived (and actually very much practically relevant) example of a structure where addition is not associative, namely: an add-and-round operation in the real numbers which is what we in fact do in practice all the time - also with floating point numbers on a computer. Example: Let's assume that we use numbers with 3 significant decimal digits and we form the 3-term sums (123 + .456) + .123  vs  123 + (.456 + .123). In the first case, we would get 123 as result and in the second case 124 when we assume that we round to 3 digits after each 2-term addition.

% Could it make sense to define a structure where we round after addition but do not round after multiplication? In such a structure, wemay have to zero-pad operands, when one of them has less precision that the other.
