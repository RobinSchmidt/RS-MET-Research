...
\end{eqnarray}
In the top line, we have used right-distributivity first and then left-distributivity and in the bottom line, we used left-distributivity first and then right-distributivity. Together with the commutativity of addition, we observe that the expressions of the right hand side are equal in both lines which the must be because the left hand sides are literally equal. To generalize further, we can say that whenever we have left- and right distributive laws at our disposal, the following holds:
\begin{equation}
\left(\sum_{i=1}^{m} a_i \right) \left(\sum_{j=1}^{n} b_i \right) =
\sum_{i=1}^{m} \sum_{j=1}^{n} a_i b_j
\end{equation}
The law says that to compute the product of two sums, we need to multiply each summand in the first sum which each summand and the second sum and then take the sum of all of these pairwise products.

%ToDo: Check if for this generalization to apply, we also need other laws such as associativity of addition. I think so.
