...
In a set, neither the order of the elements nor their multiplicity matters where by "multiplicity" we mean the number of times, an element occurs. For example: $\{ 1,2,2 \} = \{ 2,1,2 \} = \{ 1,2 \}$. In a tuple, both of these things matter: $(1,2,2) \neq (2,1,2) \neq (1,2,3) \neq (1,2)$ where the last tuple $(1,2)$ is actually even different in terms of its "data type" from the two others which match in data type but are different tuples. There is an intermediate situation where we may want to distinguish collections based on multiplicity but not based on the exact position of occurrence of an element. A structure suited for that purpose is the so called multiset. Like in a set, it is immaterial at which position we list an item - but it does matter whether an item occurs once or twice or any other number of times. ...TBC...
...
%the order of the components matters and of course, it also makes a difference
