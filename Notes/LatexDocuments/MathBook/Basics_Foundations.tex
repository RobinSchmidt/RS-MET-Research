...
%\begin{comment}
\begin{tikzpicture}
[mydot/.style={circle, fill, inner sep=2pt}, >=latex, shorten >= 3pt, shorten <= 3pt]
...
\node[mydot, right=2cm of a1, label={right:0}] (b1) {}; 
...
\path[->] (a2) edge (b4);
\path[->] (a3) edge (b4);
%\path[->] (a1) edge (b1) edge (b2); 
%\path[->] (a2) edge (b3) edge (b4);	
%\path[->] (a3) edge (b1) edge (b3);
%\path[->] (a4) edge (b2) edge (b4);
\end{tikzpicture}
%\end{comment}
