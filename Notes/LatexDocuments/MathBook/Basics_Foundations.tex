...
%  codomain - not just those that are actually reached by f.
...
% ToDo:
% -Introduce the notion of arity for relations. So far, we discussed only binary realtions.
...
% -Generalize to n-ary relations with (n-1)-variate functions as cases of these.
% -How do functions with multiple inputs and outputs fit into this framework? Maybe use examples
%  with 2 ins and 3 outs like sumDiff(x,y) = x+y, x-y  and (less contrived) divMod(x,y) = x/y, x%y.
