...
\paragraph{The Subset Relation} We'll now shift our perspective a little bit. In the previous paragraphs, we defined certain \emph{types} of relations in terms of what properties they need to have to qualify for the given name. We have looked at the relations from the perspective of putting the pairs $(a,b)$ of \emph{elements} of two sets $A$ and $B$ into the relation or not. What we will do now is to consider the pair $(A,B)$ itself. We will define some very specific relations in which any such pair of sets may or may not be.  ...TBC...

% define a specific relation between the two sets $A$ and $B$ themselves. ...TBC...
...
%then it makes sense to ask, if some $a \in A$ is an element of some $b \in B$. 
%We could also ask, if $a \in A$ is a subset of $b \in B$. We'll define later formally what a subset of a given set is, but you'll probably already know it or have the right intuition. The subset relation is a partial order, by the way [VERIFY].

% Maybe drag the section with the operations before the section about relations

%\paragraph{Element Relation}
...
% Disjoint union is sometimes also denoted by a plus - which makes sense in the context of cardinal arithmetic
