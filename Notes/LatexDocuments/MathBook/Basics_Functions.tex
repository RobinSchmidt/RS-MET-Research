...
Now we want to cosider functions defined by so called \emph{implicit equations}. These are equations of the form $g(x,y) = 0$ for some given function $g$. They define a set of points in the $xy$-plane. The goal is to find a function $f$ for $y$ with $y = f(x)$ such that $g(x,f(x)) = 0$ is satisfied for all $x$ in some domain. ...TBC...
