...
% ToDo: In the context of upper and lower bounds, also mention the supremum and infimum (as least 
% upper bound and greates lower bound). Maybe also mention the relation to the maximum and minimum. 
% They are supremums and infimums that are actually reached. This may depend on the specification of
% the domain. For example, the identity function f(x) = x has a maximum of 1 on the domain [0,1] but
% only a supremum of 1 on the domain [0,1).
...
% "monogamous", "reversible". Surjective is also called "onto"
% ...there are also some comments about this in the "Functions" paragraph in the discussion about
% relations in the naive set theory section 
...
A slightly more complicated way to define a function would be to have different calculation rules for different subsets of the domain. If these subsets are successive intervals of the real number line, this way of defining a function is called a \emph{piecewise definition}. They are written down like this:
\begin{equation}
f(x) = 
\begin{cases} 
 0 \quad& x < 0 \\
...
% ToDo:
% Define the Busy Beaver function. Maybe as a very notorious example for how a mathematical function
% definition does not necessarily give you an explicit evaluation algorithm. ...although..maybe here
% we could actually give a brute force algorithm - it's just impractical to run because of its
% insane computational complexity?
% ...yeah - that seems to be the case. See:
% https://www.youtube.com/watch?v=U1TdlgrldNM  This Number Cannot Ever Be Computed
% ...or wait - no - it isn't! The number is not computable because a brute force method to find the
% machine that runs longest (but still only a finite number of steps) requires solving the halting
% problem for each of the machines that ae considered - which is not solvable! So, the busy-beaver
% function is actually fundamentally unevaluatable (the correct term is uncomputable, I think)!
...
% The Fibonacci Sequence Has a Hidden Curve
% https://www.youtube.com/watch?v=hBupt2Db3Xg
% -Constructs the "Fibonacci function" - a function that smoothly interpolates the Fibonacci sequences and
%  obeys the functional equation and the desired limiting behavior
...
% Give examples, e^x is fixed foint of d/dx, I think, sinh and cosh are fixed points of d^2/dx^2 and
% sin, cos (and arbitrary phase shifted sines) of  d^4/dx^4. A Gaussian is fixed point of the 
% Fourier trafo. So is a delta-comb
...
% Mathematicians Dubbed His Work "Evil". It Broke Mathematics
% https://www.youtube.com/watch?v=UNNLM9y2S50
...
% Note: Listing logarithms is redundant when we have already listed exponentials and said that the 
% inverses are also in the set. I'm not sure, if similar reasoning applies to roots. They are 
% inverses of (integer?) powers which are already included in saying "all the arithemetic operators"
% which includes multiplication. But: Does a general power f(x) = x^p where p is a _real_ number 
% still count as elementary? -> Figiure this out!
...
% How sqrt(x) Encodes Decisions
% https://www.youtube.com/watch?v=2wNUZ-2rsZU
% - |x| = sqrt(x^2), max(x,y) = (x + y + |x-y|) / 2, min(x,y) = (x + y - |x-y|) / 2, 
%   step(x) = (|x| + x) / 2, ....

% A function you cannot do without
% https://www.youtube.com/watch?v=6Qb6QtC6QMs
% - Introduces a "smoothmax" or "smax" function whose derivations is based on
%   max(x,y) = (x + y + |x-y|)
...
% - Maybe define an operator \conj that puts an overbar over the content

% The book "Polynomials of One Variable..." by Caldwell says on page 8 something like:
% The number i is a zero of x^2 + 1 = 0 and the other one is -i. So far, so standard. But then it
% says something more interesting that I haven't heard yet, namely that "because of this ambiguity"
% we define the concept of conjugation. I was never quite sure why that operation was considered
% so important and fundamental. Then, on page 10 in an excercise, it states that 
% conj(z) + conj(w) = conj(z+w), conj(z) - conj(w) = conj(z-w),
% conj(z) * conj(w) = conj(z*w), conj(z) / conj(w) = conj(z/w). So, the operation of conjugation
% distributes over all the arithmetic operations. Maybe complex conjugation is the only possible
% field isomorphism on Q[i] and that's why it is so important? It would produce Q[-i].
...
% explain the term "monic"
% -Mention that for some authors, only the x^n without the coeff a_n is called a "monomial". I 
%  think, leaving out the coeff is especially prevalent in the literature about multivariate
%  polynomials (verify!) where the x^n is replaced by a product x_1^n_1 x_2^n_2 ... x_M^n_M.
%  I think, these authors all the power (or product) with the coeff a "term"
...
% ToDo: Say something about the degree of the result. It is bounded by 
% deg(p+q) <= max(deg(p), deg(q)). It can be smaller than that when higher order coefficients sum to zero.
...
% ToDo: Explain that it doesn't matter if we try to recove the a- or the b-coeffs (from c,b or c,a respectively) because due to the commutativity of convolution, we may freely swap their roles.
...
% - Bei 2:12: f(x) = (x-r1)^m1 * (x-r2)^m2 * (x-r2)^m3 * ...
%   -> f'(x)/f(x) = m1/(x-r1) + m2/(x-r2) + m3/(x-r3) + ...



\paragraph{Factorization} ...TBC...

% Monic polynomial in product form:
%
%   p(x) =    (x  - r_1) *    (x -  r_2) *    (x - r_3 ) *  ...
%        = r1*(x/r1 - 1) * r2*(x/r2 - 1) * r3*(x/r3 - 1) *  ...
%        =    (x/r1 - 1) *    (x/r2 - 1) *    (x/r3 - 1) *  ... * r1*r2*r3*...
%
% So: It seems to be more natural to factor out the roots r1,r2,r3,... from the linear factors 
% rather than their negatives. Doingt the latter would result in the form:
%  
%   p(x) = (1 - x/r1) * (1 - x/r2) * (1 - x/r3) *  ... * (-r1)*(-r2)*(-r3)*...
...
%---------------------------------------------------------------------------------------------------
%\subsubsection{Arbitrary Powers of $x$} % Maybe rename to Power functions
...
% Give general base change formula: a^x = c^{x \log_c(a)} ...but that fits better into the section
...
%  -inf is not a number? So, it has no roots (zeros) in the whole complex plane
...
% Every Situation Where 0^0 = 1
% https://www.youtube.com/watch?v=ce7rDrI05AU

% The Fascinating Story of the Exponential Function
% https://www.youtube.com/watch?v=Cx1_XFs9qx8
...
%\paragraph{Sum-To-Product Formulas}
%Let $s = x+y$ and $d = x-y$. Then:
%\begin{eqnarray}
%\sin x + \sin y &=  2  \sin(s/2) \cos(d/2) \qquad
%\sin x - \sin y  =  2  \sin(d/2) \cos(s/2) \\
%\cos x - \cos y &= -2  \sin(s/2) \sin(d/2) \qquad
%\cos x + \cos y  =  2  \sin(s/2) \sin(d/2)
%\end{eqnarray}
%ToDo: Fix alignment

% and maybe reorder such that the sin+sin and sin-sin are both in the left column
% VERIFY the formulas!
...
% The Trigonometry Guide I Wish I Had
% https://www.youtube.com/watch?v=kDnLvRQBTQM
...
% -Rules: log(a*b) = log(a) + log(b), log(a/b) = log(a) - log(b), log(x^n) = n * log(x),
%  log(1/a) = -log(a), log_b(1) = 0 for any base b, 

% Natural Logarithm as you’ve never seen it
% https://www.youtube.com/watch?v=A6PqkykJOIQ
...
%\subsection{Manipulating Function Graphs}

% ToDo: explain how to shift (along x and d), scale (along y), stretch (along x), reflect (in x and y and in the diagonal, i.e. function inversion) ...maybe even rotate and shear (that's less common and more complicated, though). Maybe also include replacing x by 1/x which sort of turns the graph inside out. Maybe include some more exotic transformations like periodization, clipping, folding, etc.

% In general, all these transformations can be subsumed under function composition (pre or post) which some given (often simple) functions


%===================================================================================================
\subsection{Special Functions}
% ToDo: Sort them in such a way that we first list the function that are (mostly) relevant as
% real functions and then those that are mostly relevant as complex functions. I think, the gamma
% function belongs somewhere in between?
...
% https://www.youtube.com/watch?v=qoJacpk_OXo
% -Very good video about the prime conting function and how the Li function approximates it
% -Justifies the lower integration limit of 2 (2 is the first prime and we avoid the singularity
%  in the integrand)
...
% https://www.youtube.com/watch?v=3ge-AywiFxs
% -About an improvement on the Dijkstra algorithm but also explains the growth of the factorial
%  function. I think, it grows like:  n! ~ e^(n * log(n))  if I get that right. Before that, I only
%  knew that it grows superexponentially but didn't know how super.
...
% https://www.youtube.com/watch?v=UuqwDQfg0GE

% Wait... What's Going On Here?!
% https://www.youtube.com/watch?v=EIpaEDoogig


%\subsubsection{Lambert's W-Function $W(x)$}

% The Lambert W Function Introduction
% https://www.youtube.com/watch?v=cMZ_blqKKZU
% -functional equation(?): x = W(x e^x)  ...by definition
% -W(0) = 0, W(1) = \Omega, W(e) = 1, W'(0) = 1
% -W(x) is solution to the ODE y' = y / (x*(1+y))
% -For values of x where infinite tetration converges, it converges to -W(-ln(x))/ln(x)
...
% https://www.youtube.com/watch?v=Owzej81Y5EA
% -Uses W to solve 1 + x = x * ln(1/x)

% https://www.youtube.com/watch?v=mJwfpcXwYRU
% -Uses W to solve  2^x + x = 5

% Maybe compare it to x e^{-x} which, for x >= 0, is a nice attack/decay curve

% How Euler Invented a New Function to Save Mathematics
% https://www.youtube.com/watch?v=IrFocEvJf0A
% -Has a section about Lagrange's inversion formula - gives a series expansion for an iverse function

% We Have To Talk About the Gamma Function
% https://www.youtube.com/watch?v=gx0Kvv0RvPg


%\subsubsection{Bessel's Cylinder Functions $J_n(x)$}



%\subsubsection{Riemann's Zeta Function $\zeta(z)$}

% - Mention also generalizations (L-functions)


%\subsubsection{Weierstrass's $\mathcal{P}$-Function $\mathcal{P(z)$}


%\subsubsection{Jacobi's Elliptic Functions $\sn(z), \cn(z), \dn(z), \ldots$}


% What does it feel like to INVENT MATH ?   by FiguR3 iT ouT
% https://www.youtube.com/watch?v=NmPQ71epaBE

% Deriving the Jacobi Elliptic Sine
% https://www.youtube.com/watch?v=Stl2KZBMOgE


%\subsubsection{Meijer's $G$ Function $G(s)$}

% One Function to Rule Them All: Exploring the Meijer G-Function
% https://www.youtube.com/watch?v=guvMJzBIOXY 
% https://en.wikipedia.org/wiki/Meijer_G-function
% https://mathworld.wolfram.com/MeijerG-Function.html
% -It seems to be a parametric family of functions with quite a lot of parameters which
%  encompasses many important elementary and special functions as particular cases for 
%  particular choices of the parameters.




% Beyond Meijer G: Why the Aleph Function Might Be the 'Theory of Everything' in Math
% https://www.youtube.com/watch?v=8Bbg-zTmLYs
% -Gives the hirarchy of the special functions in the sense that the higher ones generalize lower
%  ones 
% -Says that integration algorithms in mathematica and maple are based on Meijer-G

% 10 Important Mathematical Functions You Have Probably Never Heard Of
% https://www.youtube.com/watch?v=srlimXra3TM
...
% gamma, bessel, elliptic integrals, Gaussian integrals (aka "error function")
...
This Functional Equation Looks Impossible (The Trick is Genius)
https://www.youtube.com/watch?v=FBhmhRzLhak

All Functions Can Be Collapsed to Only One, Mathematician Proves
https://www.youtube.com/watch?v=hwtqJaS42xk
https://arxiv.org/html/2603.21852v2
https://arxiv.org/abs/2603.21852
-Makes the case that all elementary functions an be built from eml(x,y) = exp(x) - ln(y)
-This is kind of analoguous to how all boolean functions can bebuilt from nand(x,y) = not(and(x,y))

https://www.youtube.com/shorts/iAjWqH8vx48
-reports a problem with the eml function

The Search For Math's Simplest Building Block | Symbolic Computation
https://www.youtube.com/watch?v=aEY_hh2BiJ0

Can You Build a Scientific Calculator with ONE Button? | Symbolic Computation
https://www.youtube.com/watch?v=aEY_hh2BiJ0

The Top 10 Most Beautiful Mathematical Functions
https://www.youtube.com/watch?v=WilKgMUy3sY


ToDo: 
-Make a section about some specific composed elementary functions such as the eml function above or
 the Gudermann function gd(x) = 2 atan(e^x) - pi/2
