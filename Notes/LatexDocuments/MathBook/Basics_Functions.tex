...
The trigonometric functions $\sin(x), \cos(x)$ originally arise from certain geometric considerations about triangles (more on that later in the geometry chapter) but can also be defined in terms of power series:
\begin{equation}
\sin(x) = \sum_{n=0}^{\infty} \frac{(-1)^n x^{2n+1}}{(2n+1)!}, \qquad
\cos(x) = \sum_{n=0}^{\infty} \frac{(-1)^n x^{2n}  }{(2n)!}
\end{equation}
These series expansions can actually be obtained from the series expansion of the exponential function by splitting it into the parts with even and odd exponents and introducing a sign alternation by the $(-1)^n$ factor. There are also differential and functional equations for sine and cosine that are reminiscent of the equations for the exponential function. Sine and cosine satisfy the following system of coupled differential equations:
\begin{equation}
...
\e^{\i z} = \cos(z) + \i \sin(z)
\end{equation}
for any complex number $z$. Yes - we can also plug complex numbers into $\sin, \cos$ and $\exp$. How to evaluate them for complex arguments is completely defined via the series expansions because all of these three series expansions do indeed converge in the entire complex plane.  ...TBC...
...
...
\end{equation}
Again, everything still makes sense when the argument is a complex number. ...TBC...
