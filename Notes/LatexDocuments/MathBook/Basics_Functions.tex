...
A simple but very important function is the \emph{absolute value} of some number $x$ which we commonly write as $|x|$ and read as "absolute value of $x$" or in some contexts also "norm of $x$". The latter term applies especially in cases where $x$ is a more complicated object than a real number. Yes - that means the absolute value (or norm) function will later be generalized to more complicated objects (complex numbers, vectors, matrices, etc.) and by doing so, it will retain some of its important properties and that's perhaps the chief reason why it is such an important function. For a real number $x$ it can be defined as:
\begin{equation}
 |x| = \sqrt{\overline{x} \cdot x} = 
\begin{cases} 
...
These are the properties that all functions shall have if they want to deserve to qualify for calling them a "norm" even when $x$ and $y$ can later be more complicated objects than real numbers. Some more properties immediately follow from these: $|x/y| = |x|/|y|$, $|x-y| = |x| + |y|$, $\bigl| (|x|-|y|) \bigr| \leq |x \pm y|$.

%The first two properties may seem obvious but multiplicativity and triangle inequality are not so trivial


\paragraph{The Signum Function}
A related function is the so called signum or sign function. It is the counterpart of the absolute value function in the sense that in contains precisely the information that was thrown away when applying the absolute value function. For real x, it is defined as:
\begin{equation}
\sign(x) = 
\begin{cases} 
 -1 \quad & \text{iff } x < 0 \\
...
% x = sign(x) \cdot |x|
% generalizes to a normalized direction vector when x is a vector and toa unit magnitude complex number when x is complex. It is, in general, a direction.



% I have said that the elementary functions can be constructed from the arithmetic operations, roots, etc. It is in fact possible to construct the absolute value for real numbers as $\sqrt{x^2}$ (and the sign function subsequently as $x / |x|$ if $x \neq 0$ and $0$ if $x = 0$) but that's not how we usually thing about these functions - at least not over the real numbers. There, the absolute function means just: throw away the minus, if present and the sign just means: retain \emph{only} the sign.

% i.e. map positve numbers to +1 and negative numbers 
...
\e^{x+y} = \e^x \e^y
\end{equation}
This functional equation is pretty interesting indeed. It means that the exponential function translates between our two most important arithmetic operations addition and multiplication \footnote{subtraction and division are less important because they can be seen as mere inverses of the more fundamental operations of addition and multiplication}. The differential equation is quite noteworthy as well. It means that the exponential function is its own derivative. In the complex domain, it also subsumes the - undeniably also very important - trigonometric functions sine and cosine that are so intimately related to the circle. All of these features are quite remarkable and secure the exponential function a top spot in the hall of fame of all functions in mathematics - perhaps even \emph{the} top spot.

[TODO: give formulas for change of basis]
