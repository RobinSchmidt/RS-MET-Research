...
% Maybe move out of the "special functions section"
...
In the real numbers $\mathbb{R}$, when taking the $n$-th root for a given real number $x$ and a given positive natural number $n$, we denote the result or root as $r = \sqrt[n]{x}$. It all works out fine when $n$ is odd. In this case, $r = \sqrt[n]{x}$, defines a bona-fide function because the equation $r^n = x$ will always have a unique real-valued solution. For even $n$, however, we run into two problems: for negative $x$, there is no real solution whatsoever and for positive $n$, there are two real solutions. We usually "solve" the problem by saying: OK, for even $n$, the function $\sqrt[n]{x}$ is undefined whenever $x < 0$ and for $x \geq 0$, we pick the nonnegative solution by definition. But this is an ugly hack. We may get kinda away with it on $\mathbb{R}$ but when we enter the complex plane $\mathbb{C}$, it is not good enough anymore.

%we usually define the $n$-th root of a number $x$, denoted as $\sqrt[n]{x}$ as the unique nonnegative real number $r$ that satisfies $r^n = x$. 


%\medskip
%This is indeed how we usually interpret the square-root function on $\mathbb{C}$. [VERIFY] In $\mathbb{R}$, we would say that $\sqrt[4]{16} = 2$. In $\mathbb{C}$, we would say that $\sqrt[4]{16} = \{2,2\i,-2,-2\i\}$ and in general, the $n$th root of any nonzero complex number would produce a set of $n$ solutions. 
