...
% Explain connection to the Euclidean algorithm for the GCD. I think, the correspondence is as follows: We pass a pair of (num,den) to the variant of CFE algo that deals with rational numbers. The floor(x) operation corresponds to an integer division - that is, b = num/den. The modulo operation in the GCD algo maps to the x-b operation. The swapping operation maps to the reciprocation in x = 1 / (x-b). In the Euclidean algo, we are only interested in the final result of the mod operation. Here, we are interested in the whole sequence of numbers that is produced along the way.
