...
\end{equation}
...TBC...ToDo: explain how the size of the coeffs determines the error of the approximation, note that truncating after $1/7$ gives $3 + (1/7) = 22/7$ which is a rather well known rational approximation of $\pi$. Also check if the number $x$ needs to be positive (or at least non-negative) for this algo to work - we assumed that further above but don't mention it here anymore.
...
% Maybe link to the the sequences of numerators and denominators of the convergents:
...
\end{tabular}
\end{center}
It computes the simple CFE of the rational number $\frac{415}{93}$ which is given by $[4;2,6,7]$. It is, of course, finite because $x$ is a rational number. So it really ends in the final $7$. I didn't cut off a tail here. ...TBC...ToDo: Explain connection to the Euclidean algorithm. 
...
While it is totally possible to evaluate a finite continued fraction with the naive backward evaluation algorithm given above, there is actually also a way that avoids starting at the innermost subexpression and working our way back to the outermost. I will refer to this alternative evaluation algorithm as \emph{forward evaluation} and may occasionally refer to the naive algorithm as \emph{backward evaluation}. Such a forward evaluation algorithm will turn out to be particularly convenient when we want to evaluate infinite CFEs because in this case, there isn't any innermost subexpression. ...TBC...
...
- Maybe mention other way to approximate real number by rational numbers and/or to produce all
  real numbers. See:
...
https://www.youtube.com/watch?v=UcsyXiNjaEk
-About Khinchin's constant.
...
https://www.youtube.com/watch?v=-mXaU3N9e8Y
-The geometric mean of a sequence of CFE coeffs of an irrational number converges to this value with
 a probability of 100%. It's Khinchin's contant. By the way: probability of 100% does _not_ mean
 "always".

Khinchin's Constant (extra footage) - Numberphile
https://www.youtube.com/watch?v=jl8Fk7PPeOc
-This actually explains _why_ the geometric mean comes out as Khinchin's constant. It has to do with
 the CFE coeffs eventually approaching a Gauss-Kuzmin distribution and the geometric mean of that 
 distribution happens to be Khinchin's constant - if I get it right.
