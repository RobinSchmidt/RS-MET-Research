...
%  $(a f + b g)^{(n)} = a f^{(n)} + b g{(n)}$. The difference rule also follows automatically
...
%\end{equation}
This formula is structurally similar to the multinomial theorem given in equation (\ref{Eq:MultinomialTheorem}). To see this formula in action, let's consider the example of computing the 2nd derivative of 3 factors ...TBC... ToDo: Maybe write the formula down in terms of the left hand side of the multinomimal theorem (Eq \ref{Eq:MultinomialTheorem}). Such a form could be much more practical (i.e. efficient) to evaluate.
...
% Ah - no - on wikipedia, the do it the otehr way around - they go from the outside in rather than from the inside out. But I think, inside out makes more sense

% Seems like the inside-out notation is more common in category theory. See this discussion with
% claude.ai
