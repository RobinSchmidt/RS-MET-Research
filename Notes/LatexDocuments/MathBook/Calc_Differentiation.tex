...
f_1^{(k_1)}(x) \cdots f_m^{(k_m)}(x) \cdot
\frac{d^{n+1}}{dx^{n+1}} (f_1(x) \cdots f_m(x))
\end{equation}
...TBC... ToDo: explain multnomial coeffs, give example with 2nd derivative of 3 factors.
Maybe write the formula down with product notation rather than using things like $f_1^{(k_1)}(x) \cdots f_m^{(k_m)}(x)$
