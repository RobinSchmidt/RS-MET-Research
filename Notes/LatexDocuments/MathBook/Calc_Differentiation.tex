...
% There is this $(f g^k)^{(n)}$ in the numerator. I think, for this we need to combine the product rule for two factors and some sort of power rule for evaluating $(f^k)^{(n)}$. Such a rule can be obtained froma special case of the product rule for higher derivatives for multiple factors (the factors are all the same in this case - namely f)
