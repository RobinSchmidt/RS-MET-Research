...
\begin{equation}
 ...
 \qquad \forall a > 0
\end{equation}
The theorem applies if the integral converges absolutely. The integrals are to be understood in the sense of a Cauchy principal value. The special case above can be generalized as follows:
\begin{equation}
 \int_{-\infty}^{\infty} f(x) \; dx  = 
 \int_{-\infty}^{\infty} f \left( x - \sum_{n=1}^N \frac{a_n}{x - b_n} - c \right) \; dx 
 \qquad \text{if } \forall n: a_n > 0
\end{equation}
which is the theorem proper. Here $a_n, b_n, c$ are all real numbers. The special case can recovered from the general one by setting $N=1, a_1=a, b_1=0, c=0$. See \cite{GlasserMasterWiki} for more information. This identity can be useful for solving....TBC...
[VERIFY!]
