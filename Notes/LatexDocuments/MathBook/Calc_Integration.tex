...
\paragraph{Glasser's Master Theorem} tells us something about the definite integral of a function $f: \mathbb{R} \rightarrow \mathbb{R}$ over the whole real number line:
\begin{equation}
 \int_{-\infty}^{\infty} f(x) \; dx  = 
 \int_{-\infty}^{\infty} f(x - \frac{a}{x}) \; dx 
 \qquad \forall a > 0
\end{equation}
The theorem applies if the integral converges absolutely. The integrals are to be understood in the sense of a Cauchy principal value. This is actually a special case that can be generalized as follows:
\begin{equation}
 \int_{-\infty}^{\infty} f(x) \; dx  = 
 \int_{-\infty}^{\infty} f \left( x - a - \sum_{n=1}^N \frac{a_n}{x -b_n} \right) \; dx 
 \qquad \text{if } \forall n: a_n > 0
\end{equation}
See \cite{GlasserMasterWiki} for more information. This identity can be useful for solving....TBC...
[VERIFY!]


% https://en.wikipedia.org/wiki/Glasser%27s_master_theorem
% https://mathworld.wolfram.com/GlassersMasterTheorem.html
% The equenations on wikipedia and wolfram are different! On wikipedia, a is subtracted and on wolfram, x is multiplied by a. What is going on? Is one of them wrong?
