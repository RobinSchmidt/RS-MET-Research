...
% https://www.youtube.com/watch?v=WgUZKeQHlO8
...
% In some sense, differentiation throws away a piece of information and integration asks us, to put it back in. The amount of information is precisely one real number. This can also be seen from integrating and differentiating polynomials where differentiation reduces the degree by one (that means: throwing away one of the coefficients) and integration increases the degree by one (we must provide the constant term, i.e. the $a_0$ coefficient). This one real number could also be determined by prescribing a desired function value $y_0$ at some arbitrary input $x_0$. In the context of differential equations (which can be framed in terms of a generalized integration problem), this value prescription would be called a boundary condition. In the context of integration, we usually just call it the integration constant, though.
...
% Why Do We Only Need the End Points for Integrals?
% https://www.youtube.com/watch?v=DDH82HxyGCs

% The hidden connection inside every integral
% https://www.youtube.com/watch?v=GdsTDQPSYJc
% -Explains relation between integration constant C and lower integration limit a 
...
\end{equation}
This notation is useful when instead of the symbolic $F(x)$, we deal with a written out and possibly complicated expression for $F(x)$, like e.g. $x + e^x \cos(2 x^2 + \pi x /2 )$, because then we don't need to write it out twice. Another way to write down the FTC is as follows:
\begin{equation}
 \int_a^b f'(x) \, dx = f(b) - f(a)
\end{equation}
It's pretty neat because it only needs the single function symbol $f$ rather than the two symbols $f$ and $F$ whose relationship needed to be specified in the surrounding text. Here, the information about this relationship is contained within the formula itself provided that reader knows what the dash (aka prime) in the $f'$ notation means. The form using $f$ and $F$ given further above is more commonly seen, though.

\medskip
With the fundamental theorem of calculus in hand, we can easily compute (signed) areas under the graph of a function $f$ over an interval $[a,b]$ provided that we know an antiderivative $F$ of $f$. We just need to evaluate our antiderivative $F$ at two points $b$ and $a$ and subtract the results. To find such an antiderivative for some simple functions, we just need to read our table of derivatives backwards. Note that the integration constants that are part of our antiderivative will cancel in the subtraction such that this constant really doesn't matter when computing areas (or, more generally, \emph{definite} integrals). For finding antiderivatives for functions that are not listed in our table, there are certain laws that we can use. However, the task can become difficult rather quickly because it is often not so easy to see, which sequence of application of the laws (if any at all) will lead to a success.

% For finding antiderivatives of more complicated functions, there are certain laws that we can use. %Before we get to them, let's try to convince ourselves that this theorem is plausible. 
...
% https://www.youtube.com/watch?v=97KMcFjD62w
...
% Integration to Infinity - Why Most People Get It Wrong
% https://www.youtube.com/watch?v=DJ_uxBvGiZo
...
\end{equation}
That is: we pick a \emph{single} parameter $a$, derive an expression for the definite integral in the symmetric interval from $-a$ to $+a$ and then take the limit of that expression as $a$ approaches infinity. ....TBC...verify that! Explain why that solves the problem and the value is well defined. Is it, though?
...
The integral was introduced as a way to compute the (signed) area between a function graph and the $x$-axis but it turns out that we can also use it to compute quite a lot of other quantities that could potentially be of interest. ...TBC...

%---------------------------------------------------------------------------------------------------
\subsubsection{Areas}
ToDo: Explain how integrals can be used to not only compute areas between function graphs and the x-axis but also areas of geometric objects defined as the region enclosed between two function graphs, areas of geometric shapes like circles and ellipses, etc.

% Maybe make a subsection about applications (maybe after "Integration Techniques")
% -One application is the computation of areas between a function and the x-axis
% -It can be generalized to compute the area enclosed between two functions
% -In some cases, we may use it to compute the areas of geometric figures such as circles and
%  ellipses

%---------------------------------------------------------------------------------------------------
\subsubsection{Lengths}
ToDo: Explain how integration can be used to compute (arc) lengths of function graphs and of geometric objects described by parametric equations.
% Can it also be used when no parameztric form is available and only an implicit equation is
% available?

% By applying integration not to $f(x)$ itself but to the function $\sqrt{(f'(x))^2 + 1}$, we can compute the arc length of the graph of $f(x)$ (verify!)



% -One can use integrals to compute the arc-length of a function graph
% -We can use it to compute energies in physics (like the total energy picked up by a particle that 
%  moves in a force field). Maybe we can compute other physical quantities: position when velocity
%  or acceleration is given, charge if current is given, etc.
% -We can use it to compute average values of functions over an interval. We can generalize this
%  to higher order "moments" in statistics
% -We can use it to compute an error measure between a function and an approximant (using e.g.
%  least  squares)
% -We can use it to define similarity measures between functions (using the dot-product and 
%  cross-correlation, which is basically the dot product evaulated at various values for a shift/lag
%  applied to one of the two functions).
% -We can use it to combine two functions - for example using a convolution integral. Convolution
%  can be seen as another generalization of averaging. in averaging the kernel is a rectangular
%  pulse - in convolution, we may use other functions as well. We could frame convolution in terms
%  of weighted averaging (or, more generally, weighted summing - calling it weighted averaging is
%  only appropriate if the kernel has unit area)
% -We can use it to compute global properties of the function that we then may want to optimize
%  (like the total curvature, for example) - this ultimately results in the calculus of
%  variations
% -Integrals can be used to approximate sums. Often, integrals are more complicated to compute 
%  than so the usefulness of this may be questionable - but that's not always the case. Sometimes
%  evaluating an integral can indeed be simpler. That can help to determine the convergence 
%  properties of certain sums.
% -Volumes of rotation bodies
% -..I deliberately try to mention only applications of 1D integrals here. If we go to nD, we get 
%  even more applications - but that should be defered to the section about multiple integrals
...
% Integration to Infinity - Why Most People Get It Wrong
% https://www.youtube.com/watch?v=DJ_uxBvGiZo
% -Considers the intetral of x/(1+x^2) over the whole real line. As an improper Riemann integral,
%  it has no unique value - the value depends on the exact way we exhaust the real number line.
%  for example, by integrating from -R to +R and taking the limit as r gos to inf, we get zero
%  but integrating from -R to +2R and taking the limit, we get ln(2). He then considers a
%  change of variables transformation and applied Lebesgue integration which gives the unique answer
%  that the function is not Lesbesgue integrable. This seems to somehow contradict the former 
%  statement that the Lebesgue integral has a wider application domain than the Riemann integral.
%  For improper integrals, it seems to behave differently. It doesn't make an unintegrable function
%  integrable. ..instead...it shows more clearly and directly that it is unintegrable...or something?
% -Gist: Lebesgue integration is stable under a change of variables
% -The channel has more interesting videos about integration:
%  https://www.youtube.com/@dr2bayes/videos
...
In equation (\ref{Eq:InclusionExclusion}) on page \pageref{Eq:InclusionExclusion}, we already encountered the inclusion-exclusion principle for cardinalities of sets. There is also an analoguous principle for measures. It is given by: 
\begin{equation}
\mu \left( \bigcup_{i=1}^{n} A_i \right) = 
\sum_{k=1}^{n} (-1)^{k-1} 
 \left( \sum_{1 \le i_1 < \dots < i_k \le n} \mu ( A_{i_1} \cap \dots \cap A_{i_k} ) \right)
\end{equation}
It's the exact same formula just with the cardinality function $|\ldots|$ replaced by the measure function $\mu(\ldots)$. ToDo: Maybe make notation consistent, i.e. use $X_k,k$ here or use $A_i,i$ above.

%...TBC... Give formula for inclusion-exclusion principle for measures.



% ToDo: Mention how the inclusion-exclusion principle works for measures. See "Harmonic Analysis..." by Chirikijan/Kyatkin (extended and updated Edition) pg. 770


\subsubsection{The Lesbesgue Measure}
After these general considerations, we now want to construct a concrete measure $\mu$ on the set of real numbers $\mathbb{R}$. That means, we want to define a function into which we plug in a subset of $\mathbb{R}$ and it spits out a nonegative real number or infinity. Intuitively, that measure should suitably generalize the idea of a length of an interval $[a,b]$, so we would certainly expect that $\mu([a,b]) = b-a$. Furthermore, we would expect $\mu$ to be translation invariant, i.e. $\mu(X + t) = \mu(X)$ for any measurable set $X \subseteq \mathbb{R}$ and any real number $t \in \mathbb{R}$ that acts as a translation or offset that shifts the set $X$ by an amount of $t$ along the real number line. The construction of such a function $\mu$ on the whole power set $\mathcal{P}(\mathbb{R})$ turns out to be impossible, though. That's the reason why we need this complication with the $\sigma$-algebras. So, if we can't use $\mathcal{P}(\mathbb{R})$ as our $\sigma$-algebra, i.e. as the domain of $\mu$, what other set of subsets of $\mathbb{R}$ could be suitable to serve as our $\sigma$-algebra? The answer turns out to be the so called \emph{Borel algebra} ...TBC...
...
% How Integration Should Be Taught
% https://www.youtube.com/watch?v=85aH8XgVPB0

% The Simple Truth Behind Lebesgue vs Riemann
% https://www.youtube.com/watch?v=y7rTbl__fp0

% Measure Theory Made Doable: a Complete Self-Study Guide
% https://www.youtube.com/watch?v=UpwXBlTx7KE

% The Integral That Changed Mathematics Forever
% https://www.youtube.com/watch?v=tysFpLAK20s
...
Every Type of INTEGRAL Explained in 6 Minutes
https://www.youtube.com/watch?v=jok9fPHospc

Every type of Integral Explained in under 6 Minutes
https://www.youtube.com/watch?v=TSrOrHFW8kg

Every Integration Technique Explained in 13 minutes (part 2)
https://www.youtube.com/watch?v=RKgIYMwOwgM

When Does Integration Become Impossible? | Nonelementary Integrals
https://www.youtube.com/watch?v=HcwZrOaS7sE

Here’s How to Master Integrals
https://www.youtube.com/watch?v=idh_u7jasPw
