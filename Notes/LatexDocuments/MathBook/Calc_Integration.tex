...
\begin{equation}
\mu \left( \bigcup_{k=1}^{n} X_k \right) = 
\sum_{k=1}^{n} (-1)^{k-1} 
 \left( \sum_{1 \le i_1 < \dots < i_k \le n} \mu ( X_{i_1} \cap \dots \cap X_{i_k} ) \right)
\end{equation}
It's the exact same formula just with the cardinality function $|\ldots|$ replaced by the measure function $\mu(\ldots)$. It's interesting to note that both cardinalities and measures are ways to quantify the sizes of sets and for both of these ways, we have such inclusion-exclusion principles.

% See "Harmonic Analysis..." by Chirikijan/Kyatkin (extended and updated Edition) pg. 770
