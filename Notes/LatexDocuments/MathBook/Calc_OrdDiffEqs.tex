...
% Explain the types of behaviors that this ODE produces. Most importantly: Damped (i.e. decaying) oscillations. It can also produce growing oscillations and undamped oscillations as the special case that separates the decaying from the growing oscillations. It can also produce exponential deacy and growth as special cases. So, this ODE is felxible enough to model a great variety of behaviors that may occur in nature.
...
%\paragraph{Ricatti Equations}
% https://en.wikipedia.org/wiki/Riccati_equation

%\paragraph{Exact Differential Equations}
...
\paragraph{Sturm-Liouville Equations}
A Sturm-Liouville equation is an ODE of the general form:
\begin{equation}
(s(x) \cdot y'(x))' + (\lambda w(x) - q(x)) \cdot y(x) = 0
\quad \text{or shorter:} \quad
(s \cdot y')' + (\lambda w - q) y = 0
\end{equation}
where $\lambda$ is a constant and $s(x),w(x),q(x)$ are given functions defined on a closed interval $[a,b]$ with $s(x) > 0, w(x) > 0$ on $(a,b)$ and $s(x) \geq 0, w(x) \geq 0$ on $[a,b]$, i.e. these function must be strictly greater than zero on the whole open interval $(a,b)$ but are allowed to be zero at the endpoints. The relevance of this particular form of an ODE is that the set of solutions to such an ODE has certain properties that are desirable when we want to expand an arbitrary function $f(x)$ by using the solutions $y_i(x)$ of this ODE as our basis. Namely, the set of solutions is complete and orthogonal. In the present context that means: ...TBC...

% ToDo:
% -Give the definition of orthogonality with respect to the given weight function w(x) and
%  explain how this definition relates to our normal understanding of orthogonality
% -Explain what completeness means. It means that any (nice enough) function f can be approximated 
%  to arbitrary precision using a (finite) linear combination of the functions. The infinite 
%  (weighted) sum converges to the function f (in some suitable sense - maybe not pointwise, but at
%  least in squared-integrated-error sense). Here "nice enough" means: square integrable, absolutely
%  integrable and continuous (I think). Or maybe it needs to be smooth? Figure out! Write these 
%  conditons down in math notation. Like $f \in C^0([a,b]), \int_a^b |f(x)| dx < \infty, ...$
% -The orthogonality makes it easy to compute the weighting coefficients - namely via an inner 
%  product between the fucntion f and the basis function y_i (i.e. a projection of f onto y_i). The 
%  inner product is just the weighted integral over the product f(x) * y_i(x) * w(x) over the 
%  interval [a,b]. The contributions of the different basis function do not interfere in the 
%  approximation of f. That is: The optimum weight for y_i (viewed as 1D optimization problem) does
%  not depend on the present weight of any y_j (that we may take as a fixed and given parameter of 
%  the optimization problem in this context).
%   ...verify all of these! A lot of it is just from the top of my head.
% -Figure out and explain the meaning and purpose of the other functions s(x) and q(x). Are there
%  any requirements of q(x), too? Or are we totally free to choose any q we like. Maybe it needs to
%  be at least piecewiese continuous? And maybe bounded? 
% -Explain how the \lambda_i can be interpreted as eigenvalues and the solutions y_i(x) as
%  eigenfunctions.
% -Explain how to incorporate boundary conditions
% -Explain how the Fourier basis arises as a special case for a specific choice of s,w,q,lambda
% -See "Harmonic Analysis for ..." (Chirikjian, Kyatkin), pg 45 ff
% -Is it right to say that Sturm-Liouville theory can be seen as a generalization of Fourier
%  theory? ...and maybe also of the theory of polynomial expansions (Chebychev, Legendre, etc.)?

% 

%\paragraph{Example: Fourier Series}

% What other examples are relevant? Do the Chebychev polynomials also arise as the solution to
...
% Green's functions are just inverse matrices
% https://www.youtube.com/watch?v=SD_2UGcL8rM
% -I think, a Green's function can be seen as a sort of time-varying impulse response, i.e. intead of
%  h(t-tau), we would have to use a more general bivariate function g(t,tau) inside an integral that 
%  otherwise looks like a convolution integral?
...
-Explain how to re-interpret an ODE as integral equation (see Vadim's book)

-Explain differences between initial value problems (IVP) and boundary value problems (BVP)
 -IVPs usually occur in situations where we want to model a situation starting from a known initial 
  state such as position and velocity of a point mass (e.g. in a mass-spring system or a thrown ball
  in a gravitational field). We may also want to model a situation where we know the initial and
  final position but want to find the velocities. In such a case, the initial velocity is part of 
  the solution rather than given.
 -BVPs usually occur when we want to model some static equilibrium situation such as a hanging 
  string/rope/chain. We want to fix bot of the endpoints. 
 -Combinations may occur in PDEs: The motion of a string in acoustics has to satisfy spatial 
  boundary conditions (e.g. fixed at the ends) but we also want to give it an initial displacement 
  shape so the initial "value" is actually a whole function or shape.


The functions that are equal to their own third derivative
https://www.youtube.com/watch?v=OSL5z-fzBsg
-Shows the general solution of the ODE f''' = f or f''' - f = 0.
-Uses the ansatz: f = k1*f1 + k2*f2 + k3*f3 for 3 constants k1,k2,k3 and 3 linearly independent
 solution functions f1,f2,f3
-Uses an exponential ansatz f(x) = k e^{a x} with f'''(x) = k a^3 e^{a x} which leads to a^3 = 1.
 a^3 = 1 has the 3 solutions a1 = 1, a2 = (-1+sqrt(-3))/2, a3 = (-1-sqrt(-3))/2 = a2^2
-The general solution is f(x) = k1 e^x + k2 e^{a1 x} + k3 e^{a2 x}. 
-For a real valued solution, we must have k2 = k3 and we get:
 f(x) = k1 e^x + k2 (e^(-x/2) sin(b x)) + k3 (e^(-x/2) cos(b x))  with b = sqrt(3)/2
-So: The interesting solution (i.e. the one which is not just e^x) is an exponentially decaying
 sinusoid (with a specific frequency, phase and decay rate)
-A commnet says: Ex. 3.11 p. 137 form Carl M. Bender, Steven A. Orszag, Advanced Mathematical
 Methods for Scientists and Engineers Asymptotic Methods and Perturbation Theory, 1978.

The functions that are equal to their own n-th derivative
https://www.youtube.com/watch?v=fnFabzutXNg
-f^{(n)} = f -> f(x) = sum_{k=0}^n c_n e^{x e^{i w_k}}   with w_k = (2 pi k) / n
-Gives also formulas for the real valued solutions. They are based on observing that the real and
 imaginary part of a complex valued solution to the ODE are also solutions to the ODE, so we
 just need to extract real and imag parts of the complex solutions. 


6 Steps to Master Differential Equations
https://www.youtube.com/watch?v=5hPo6Pp7yNM


The Equation That Broke Everything - How Bernoulli Fixed It
https://www.youtube.com/watch?v=T6pi1qOX1KY
-About Bernoulli ODEs and variation of parameters


Differential Equations Have Their Own Language
https://www.youtube.com/watch?v=BSOOBxPyMcE

All Solutions to 1st-Order Ordinary Differential Equations
https://www.youtube.com/watch?v=qXMjeJHC5wQ
-About different solution techniques for different types of ODEs
