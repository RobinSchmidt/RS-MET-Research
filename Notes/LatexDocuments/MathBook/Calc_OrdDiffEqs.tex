...
\medskip
[Q: Can it happen that the so found equation for $y$ cannot be solved explicitly for $y$, i.e. can we arrive at an implicit equation for $y$? I think so. Try to find an example, where this happens. Maybe something where we get an expression $H(y) = y + \sin(y)$. Basically, the function $y(x)$ that we are interested in will be the inverse of $H$. I think, the solution is $y(x) = H^{-1}(F(x))$]
%then we can 
