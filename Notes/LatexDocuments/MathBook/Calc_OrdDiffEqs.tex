...
If the ODE is given (or can be transformed into) in the special form $y' = f(x) g(y)$, i.e. the right hand side is a product of functions $f,g$ where $f$ only depends on $x$ and $g$ only depends on $y$. In this case, we can rewrite $y'$ in the Leibniz notation as $dy / dx$ and treat these $dx, dy$ differentials formally like variables. We bring everything with $y$ an one side and everything with $x$ to the other side:
\begin{equation}
\frac{dy}{dx} = f(x) g(y) 
...
\end{equation}
Now we integrate both sides separately with respect to their respective integration variable. Let's see this method in action on an example. Let the ODE be given as:



\medskip
[Q: can it happen that the so found equation for $y$ cannot be solved explicitly for $y$, i.e. can we arrive at an implicit equation for $y$? Try to find an example, where this happens.]
%then we can 
