% TODO: Maybe use n isntead of k as index for consistency with the section on series. But be careful: In the early section, we already use n for something else. Replace that n with m first. But wait! No! It already is consistent! We use k for the summation or product index and n for n-th partial sum or product.
...
For infinite sums, a necessary (but not sufficient) condition for convergence was that the individual summands approach zero as $n \rightarrow \infty$. This can be understood from the fact that zero is the neutral element of addition. Adding zero to some value changes nothing and adding a number close to zero changes the value only by a little bit. For infinite products, the analogous condition is that the individual factors must approach one because for multiplication, the neutral element is one. This is one reason why it is convenient to express the individual factors $a_k$ in the form $1 + h_k$ with $h_k = a_k - 1$. In products, it makes more sense to think about, how far the individual factors deviate from 1 rather than from 0 and the $h_k$ give us that information more directly than the $a_k$. They are the offset from $1$. In German, these $h_k$ values have a name: they are called "Kerne" (meaning "cores" or "kernels") but I have not yet found an English term for those (TODO: figure out if there is one!). So, we may write our products in either of the two forms:
\begin{equation}
...
\begin{equation}
 P = \prod_{k=2}^{\infty} \left(1 + \frac{1}{k} \right)
   ...
   = \infty
\end{equation}
The infinite variant of the product diverges to infinity. The individual factors in the product actually do approach 1 (from above). However, they do not not approach it quickly enough to lead to a convergent infinite product. This is somewhat analogous to the divergence of the harmonic series where we sum terms of the form $\frac{1}{n}$. These terms approach zero but not fast enough to make the infinite sum convergent.
...
% What about  P = \prod_{k=1}^{\infty} \left(1 + \frac{1}{k^2} \right)
...
\paragraph{Theorem} The infinite product $\prod_{k=k_0}^{\infty} (1 + h_k)$ converges absolutely if and only if  the infinite sum $\sum_{k=k_0}^{\infty} h_k$ converges absolutely. That is:  $\prod_{k=k_0}^{\infty} (1 + |h_k|)$ converges iff  $\sum_{k=k_0}^{\infty} |h_k|$ converges.
...
\subsection{Factorization of Functions}  % Alternative title
...
\end{equation}
which may be even more convenient in some situations. Of course, if the product is infinite, we may have to worry about convergence and even if it is finite, it is not clear whether or not the product will match the function anywhere else. If the function $f(x)$ happens to be a polynomial, though, we can actually confidently say that when we include all of the roots of $f(x)$ into our product function $p(x)$, then $p(x)$ must actually indeed be equal to $f(x)$ up to a constant scale factor. The same is true for $q(x)$ but with a different scale factor [VERIFY!]. If we now remember that a power series expansion of a function can be regarded as a sort of "infinite polynomial", we may plausibly hope that we can express our function $f(x)$ with such an infinite product. In practice, the form of $q(x)$, which I will call the $q$-form, tends to be easier to deal with. First, it has the desirable form of products where the factors are of the form $(1 + h_k)$. I think, it also tends to be less prone to divergence away from the roots (VERIFY!). To turn the $p$-form into the $q$-form, one just needs to multiply a linear factor of the form $(x - r_k)$ with an appropriate constant, namely $-1 / r_k$. Multiplying a linear factor by a constant (nonzero) factor only scales it but doesn't change the location where it becomes zero. That could be problematic if one of the $r_k$ is zero, though. But that is a problem that can easily be solved by pulling these factors out of the product sign and using the $p$-form for them. The $p$-form of a linear factor $(x - r_k)$ where $r_k = 0$ is just $x$. If we have $m$ of such factors, i.e. zero is a root with multiplicity $m$, that would contribute a factor of $x^m$ in front of the product sign. All the other linear factors with $r_k \neq 0$ pose no problem to convert into $q$-form. ...TBC...
...
%\subsubsection{Factoring the Trigonometric Function}
\subsubsection{Example Factorizations}

\paragraph{Factoring Sine}
...
\sin(x) = x \cdot \prod_{k=1}^{\infty} \left( 1 - \frac{x^2}{k^2 \pi^2}  \right)
        = \ldots \left(1 + \frac{x}{3 \pi}  \right)
          ...
          \ldots
\end{equation}
He "factored out" all the infinitely many roots of the sine function that occur at all the integer multiples of $\pi$. Each positive integer $k$ gives one factor of $(1 + \frac{x}{k \pi})$ and one of $(1 - \frac{x}{k \pi})$. These pairs of factors can be combined into a single $(1 - \frac{x^2}{k^2 \pi^2})$ factor. The $k=0$ case is treated separately to avoid division by zero. That's the single $x$ in the middle of the right hand side expression and the single $x$ in front of the product sign of the mid hand side\footnote{Who would not want to have an additional mid hand?} expression. Having found this product representation for the sine function had a couple of interesting spin off results.

% Maybe mention that the more naive approach of factoring sine as:
% sin(x) = x * (x-pi) * (x+pi) * (x-2pi) * (x+2pi) * ...
% doesn't work. The product doesn't converge. Maybe try to fix it by defining the partial products
% (up to N) by introducing a pre-factor of 1/N or maybe 1/N^2 and then consider letting N -> inf. I 
% think it may work but writing it in the form above obviates the pre-factor. Try it!
% Mention that the form above is the q(x) form. Using the naive approach would be the p(x) form.
% Note that the q(x) sastifies the necesarry condition for convergence of the individual factors 
% approaching 1 whereas in the p(x) form, the factors diverge (linearly) to infinity.

% Maybe mention that the single x factor could also be brought over to the left hand side such that
% sin(x) / x = \prod ... Doing so would directly define a product representation of the sinc 
% function which might be useful for evaluating a polynomial approximation of sinc. I think, I did
% some numerical experiments with this idea somewhere.

% Mention the product formula for cosine.
% see: https://www.youtube.com/watch?v=y12QM6d8pO0 at 7:00

% Mention that looking at the graph over real axis alone might not be enough to see _all_ the 
% zeros. Some functions may also have complex zeros. We were lucky with the sine though because
% the zeros at k*pi, k in Z are indeed all of the zeros of sine, even when considered as a complex
% function. Remind the reader that the complex versions of real functions are usually defined by
% extending their power series expressions to complex arguments (at least in case of analytic 
% functions - which the sine is).

% % https://de.wikipedia.org/wiki/Basler_Problem#Eulers_erste_L%C3%B6sung


% This Problem Took a CENTURY — Euler Needed ONE Night
% https://www.youtube.com/watch?v=LWNNlDhlGyc

%\paragraph{The Basel Problem}
\medskip
Euler discovered this product formula in the course of solving the so called \emph{Basel problem}. This is the problem of finding the value of the infinite sum $\sum_{k=1}^{\infty} \frac{1}{k^2}$. Euler found the value of this sum to be $\frac{\pi^2}{6}$ and his investigations led also to the definition of the famous Riemann zeta function. 
%...TBC.. ToDo: explain the connections ...or maybe not here - but maybe provide references
...
\end{equation}
which is known as the \emph{Wallis product} representation of $\pi$. Due to its slow convergence, the formula is useless for practical computation of $\pi$, though. But it's an interesting fact nonetheless. The Wallis product formula was already known before Euler found the factorization of the sine function.
...
% https://www.youtube.com/watch?v=nw6W_fMf008
...
% -At around 14:15, he explains how the product formula for sin(x)/x was used by Euler to solve the
%  Basel problem


\paragraph{Factoring Cosine}
The cosine function also has an expression as an infinite product. It looks like:
\begin{equation}
\cos(x) = \prod_{k=1}^{\infty} \left( 1 - \frac{4 x^2}{(2k - 1)^2 \pi^2}  \right)
\end{equation}

\paragraph{Factoring the Reciprocal Gamma}
The reciprocal of the Gamma function can be written in product form as follows:
\begin{equation}
\frac{1}{\Gamma(x)} = x e^{\gamma x} \prod_{k=1}^{\infty} 
\left( 1 + \frac{x}{k}  \right) \exp \left(- \frac{x}{k} \right)
\end{equation}
...
%   P-function
%
% - Construct the sinc function as infinite product. It's just the same formula as for sin but 
%   without the prefactor x in front of the product. Maybe construct also a windowed sinc.
...
% ToDo:
% - Explain connection of the product expression for the sine with its series expression and how the
%   result sum_n 1/n^2 = pi^2 / 6 can be extracted from that by multiplying out the product form and
...
% The Identity That Made Prime Theory Possible
% https://www.youtube.com/watch?v=gRJeZm8J7bI
% - Has very nice derivation why the product form and the infinite series form of the zeta function
%   are the same.
% - It also relates the value zeta(2) to the probability that two random natural numbers have 
%   no prime factor in common. It's 1/zeta(2) = 6/pi^2.

% The Simple Math Problem Nobody Could Solve
% https://www.youtube.com/watch?v=CC2DG391HmA&list=PLb8Y66BCLlbyvSfZsMXrGkqzw-qblybe0&index=1


% ToDo:
% - Explain the general technique of equating a function's Taylor expansion with its "infinite 
%   product" form to obtain results like Euler did with the Basel problem. In this case, he looked
%   at the sine function because the 
...
\subsubsection{Factorization Theorems}
In the factorization of the sine function, we just took all of its roots $r_k$ whose locations we already knew (they occur\footnote{And there are indeed no other roots of sine in the complex plane. This might not be obvious by looking at the graph over the real axis alone but it is true.} at the integer multiples of $\pi$) and formed an infinite product with linear factors of the form $(1 + \frac{x}{r_k})$ with some special treatment for the root at zero (which we took care of by incorporating a factor of $x$ into the product). We\footnote{Well, actually it was not us but Euler.} arrived at an infinite product that indeed did converge to the sine function. We were rather lucky that it worked out so easily, though. At the outset, it was not at all obvious that the roots alone are enough to determine the sine function completely. After all, we could multiply the whole product expression by any arbitrary function that itself has no roots whatsoever to obtain a different function that still has the exact same roots. That observation hints at that the roots of a function alone might not be enough to determine it completely. Indeed, in the case of the reciprocal gamma function, other factors appeared. These other factors had the general form of an exponential function of something. The exponential function has the interesting feature of having no roots at all in the whole complex plane. It never produces zero as output such that these additional exponential factors do not introduce any additional roots into the product. ...TBC...

%Not only is it conceivable that the actual sine function needs a constant factor in front of it - we could even multiply the product by an arbitrary function (perhaps one that doesn't itself have real zeros such as exp(x)) and the resulting would still have exactly the same zeros as the sine function. At least along the real axis. It may be a different story once we widen our perspective to the complex plane (exp does have complex roots - or does it? ...nope! It doesn't!)

% This may go into the section of the factorization theorem. In this theorem we do indeed modify the function defined via a product over the zeros by an additional factor e^{g(z)} which itself has no zeros. Moreover each factor in the product has a multiplicative multiplier e^(...) which also can nowhere become zero (because the exponential function itself has no zeros - not even in the complex plane)


\paragraph{The Weierstrass Factorization Theorem}
states that an entire function $f(z)$ in the complex plane can be constructed from the set of its roots $\{ r_k \}$ via an expression of the following general form:
\begin{equation}
f(z) = z^m \cdot e^{g(z)} \cdot \prod_{n=1}^{\infty} 
       \left(1 - \frac{z}{r_n} \right)
       \exp \left( \sum_{k=1}^{p_n} \frac{1}{k} \left( \frac{z}{r_n} \right)^k \right)
\end{equation}
...TBC...

% m: multiplicity of the root at zero. If none, m = 0 and z^m = 1
% g(z): an entire function
% exp(...): adjustment of the factors to ensure convergence of the product
% p_n: ...  when 0: sum is empty, so its zero and the exponential evalautes to 1
%           when 1: only the linear term (1/1)*(z/r_n)^1 appears. Example seein in 1/Gamma(x)
% e^{g(z)}: Has no zeros, compensates for the exp(...) factors? ..not sure. Verify!

%...TBC...ToDo: Explain hat this theorem ensures that every \emph{entire function} can be factored in a way that generalizes the procedure that Euler used to factor the sine function. Explain what "entire functions" are. The factors in the generalization are potentially more complicated than just $1 - x/r_k$, though. There may be exponential factors allowed and there may also a pre-factor involving an exponential function.  ...TBC...

%Maybe that should be a topic for complex analysis

% https://en.wikipedia.org/wiki/Weierstrass_factorization_theorem
% https://de.wikipedia.org/wiki/Weierstra%C3%9Fscher_Produktsatz
% https://mathworld.wolfram.com/WeierstrassProductTheorem.html    ...not much there
% https://encyclopediaofmath.org/index.php?title=Weierstrass_theorem

% Let's Factor Functions! | Weierstrass Factorization Theorem
% https://www.youtube.com/watch?v=y12QM6d8pO0
% -Explains factorizations of sin, cos, exp, 1/gamma and the Weierstrass factorization theorem
%  (the latter starts around 22:00, the formula appears at 24:00)

\paragraph{The Hadamard Factorization Theorem}
With certain further restrictions on $f(z)$, we can give an even more specific form that the factorization has to obey. ...TBC...

% ToDo: State the conditions and the form of the factorization under these conditions.

\paragraph{Examples Revisited}
In light of the general pattern of the factorization as shown in the Weierstrass theorem, the factorizations of sine and cosine can be recovered as special cases as follows:
\begin{equation}
\sin(z) = z \prod_{k \in \mathbb{Z}^*} 
          \left( 1 - \frac{z}{k \pi} \right) \exp \left(\frac{z}{k \pi} \right)
        = z \prod_{k=1}^{\infty} \left( 1 - \frac{z^2}{k^2 \pi^2}  \right)
\end{equation}
where $\mathbb{Z}^* = \mathbb{Z} \setminus 0$. The right hand side version is the formula that we have already seen just with $z$ instead of $x$. The formula in the middle is the new one. We can again combine the factors in pairs for positive and negative $k$ which lets the exponential parts cancel one another. [VERIFY!]. Here $m = 1$, $g(z) = 0, p_k = 1 \; \forall k$. [VERIFY]. Similarly, the cosine can be written as: 
\begin{equation}
\cos(z) = \prod_{k \in \mathbb{Z}} 
          \left(1 - \frac{2 z}{(2k-1) \pi} \right) \exp \left(\frac{2z}{(2k-1) \pi} \right)
        = \prod_{k=1}^{\infty} \left( 1 - \frac{4 z^2}{(2k - 1)^2 \pi^2}  \right)
\end{equation}


% sin(z) = z \prod_{k \in \mathbb{Z}^*} (1 - z/(k \pi) ) e^{ \frac{z}{k \pi} }
