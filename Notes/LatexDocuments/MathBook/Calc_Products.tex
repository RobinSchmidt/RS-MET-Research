...
%Not all products that have zero as result "diverge" to zero, though. Some are also just zero because one or more of the factors is itself zero. o also \emph{converge to zero}. To see what the difference between the two notions is, consider the finite product:
%\begin{equation}
% p_n = \prod_{k=2}^n  \left(1 - \frac{1}{k^2} \right)
%     = \prod_{k=2}^n  \left(\frac{k^2-1}{k^2} \right)
%     = \frac{n+1}{2n}
