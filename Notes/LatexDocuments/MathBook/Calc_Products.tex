...
In the factorization of the sine function, we just took all of its roots $r_k$ whose locations we already knew (they occur\footnote{And there are indeed no other roots of sine in the complex plane. This might not be obvious by looking at the graph over the real axis alone but it is true.} at the integer multiples of $\pi$) and formed an infinite product with linear factors of the form $(1 + \frac{x}{r_k})$ with some special treatment for the root at zero (which we took care of by incorporating a factor of $x$ into the product). We\footnote{Well, actually it was not us but Euler.} arrived at an infinite product that indeed did converge to the sine function. At the outset, it was not at all obvious that the roots alone are enough to determine the sine function uniquely. After all, we could multiply the whole product expression by any arbitrary function that itself has no roots whatsoever to obtain a different function that still has the exact same roots. That observation hints at that the roots of a function alone might not be enough to determine it uniquely. ...TBC...
