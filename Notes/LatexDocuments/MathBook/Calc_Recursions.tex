...
% ToDo: 
% - Explain that we have already seen the algorithm above to compute the square root
% - Explain the general idea behind it (fitting a tangent to the function and solving for the zero 
%   crossing of the tangent)
% - Apply the same idea to other functions: n-th root, maybe logarithm, general polynomial roots, ..
% - Explain a simple generalization that lets us find where the functions crosses any value with
%   zero just being a special case
% - Generally: x_{n+1} = x_n - f(x_n) / f'(x_n)

% \paragraph{Generalizing to $n$th Roots}

% - We apply the genral rule x_{n+1} = x_n - f(x_n) / f'(x_n) to f(x) = x^p so we have:
%   f(x) = x^p, f'(x) = p x^{p-1}  ->  x_{n+1} = x_n - x^p / (p x^{p-1}). Let p = 1/q ...tbc...
...
% The Infinite Square Root
% https://www.youtube.com/watch?v=vx46LBR1dx4
...
% The Hidden Pattern Hiding In Infinite Powers
% https://www.youtube.com/watch?v=-zpNoT_6OLA
...
TODO: Explain under which circumstances the fixed point iteration scheme $x_{n+1} = f(x_n)$ converges to a fixed point $x_0$. For this to happen $f(x_0)$ must be a contraction mapping around the fixed point $x_0$ which means that $|f'(x_0)| < 1$ (verify). Explain cobweb diagrams. What about non-differentiable functions?

% The Number That Equals Its Own Cosine
...
Gauss’ Secret Way to Calculate pi Faster
https://www.youtube.com/watch?v=7qiDDhIYx48
-About the Gauss-Legendre algorithm to compute pi
