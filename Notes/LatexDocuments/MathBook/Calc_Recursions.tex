...
It's intersting to note here that no calculus is needed. We don't evaluate any limit here. What we did instead was to observe that in an infinite nesting of functions, we are allowed to peel off the outermost layer of function evaluation. In fact, we can peel off any finite number of layers but doing it only for the outermost one was the simplest choice. This act of peeling off was legal only due to the infinite nesting. For a finite nesting, we would not have been able to do that.
