...
\begin{equation}
 x = a + b \cdot \sqrt{x}
\end{equation}
That looks a lot simpler! In fact, with a little bit of simple algebra, we can solve that equation for $x$ to obtain:
%\begin{equation}
%x_{1,2} = k \pm \sqrt{k^2 - 1} 
%\qquad \text{where} \qquad
%%k = \frac{2a + b}{2}
%k = a + \frac{b}{2}
%\end{equation}
%VERIFY! Maybe one of the solutions is wrong 
...TBC...


% Maybe start with the expression as example:
%
%   x = a + b \sqrt{a + b \sqrt{a + b \sqrt{\ldots}}}
%
% By inspection we see that this means:
%
%   x = a + b \sqrt{x}
%
% which we can solve for x. This results in a quadratic equation for x with solutions:
%
%   x_{1,2} = \frac{2a + b^2}{2} \pm \sqrt{ \frac{(2a + b)^2}{4} - 1}
%
% Verify these solutions and maybe filter out the wrong one that may have been created due to 
% squaring both sides of the equation. We could simplify the solution as follows:
%
%   k = (2 a + b^2) / 2,  x_{1,2} = k +- sqrt(k^2 - 1)

% I think, it may have to be  k +- sqrt(k^2 - a^2)
...
(3/2*sqrt(13) + 11/2,
 5*sqrt(3/2*sqrt(13) + 11/2) + 3,
 -3/2*sqrt(13) + 11/2,
 5*sqrt(-3/2*sqrt(13) + 11/2) + 3)

But this is wrong! Neither is x1 == y1 nor x2 == y2. Check the formula! If it is right, then maybe
there is some error in the reasoning. Maybe we cannot just square both sides of the equation. Maybe
we then get also things like  (-2)^2 = 2^2  from which we would falsely conclude that -2 = 2? We
expect to create an extra solution like -2 when we transform  2 = 2  to  2^2 = 2^2  because now also
(-2)^2 = (-2)^2 is a solution. But maybe we also get false "cross-solutions" like (-2)^2 = 2^2?
...
...I think, my formula is wrong. I may have made a mistake somewhere in the algebra
