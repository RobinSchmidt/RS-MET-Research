...
% ToDo: Maybe mention that convolution is only commutative under the assumption that the multiplication of the sequence elements is commutative i.e. b_i a_j = a_j b_i. This may not be true when the a_i, b_j are matrices, quaternions, etc.
...
\paragraph{$\star$ Dirichlet Convolution}
...
% To verify that the recursion above converges to sqrt(2) we can use the SageMath snippet:
%
% a = 2.0
% for i in range(1,8):
%     a = (a^2 + 2) / (2 * a)
%     print(a)
%
% It does indeed converge to sqrt(2) but it's _not_ the Babylonian algo. What is it?

% ToDo: Explain that the definition of convergence of a sequence a_n to a limit a requires us to 
% know the number a beforehand in order to apply the definition. What if we do not have any idea
% what the limit a could possibly be? The answer is to look at tail segments of the sequence. If the
% terms of the sequence get and then remain(!) arbitrarily close to each other, the further we go 
% out, then the sequence is considered to be convergent. This idea is formally captured in the 
% definition of a Cauchy sequence: ...

% Point out that it is not enough for the neighboring terms to get arbitrarily close - nor is it 
% enough that the terms within finite segments remain close. All terms in the tail end must remain
% close to one another. Take as example the sequence a_n = \sin(\pi \sqrt{n}). It's a sinusoid with
% ever decreasing frequency. Immediate neighbors as well as finite stretches tend to approach a
% constant because the frequency approaches zero. Nevertheless, the sequence does not converge to
% anything. The tail end will always eventually break out from any epsilon-hose (is that a term? In
% German, Epsilon-Schlauch indeed is one)

% https://www.desmos.com/calculator/2zwrtsuo1u
...
% The Function That Terrified Mathematicians
% https://www.youtube.com/watch?v=fozO9jyNsY8
% -The Weierstrass function is continuous because the infinite series that defines it converges
%  _uniformly_
