% TODO: Use consistently k for the summation index. At the moment, we sometimes use k and at other times n.
...
% Maybe express it more generally such that the index k does not need to start at 0 but at some arbitrary k_0
...
% Series are just a special kind of sequences - namely those which are produced by a very specific
% production rule. This production rule is to take a partial sum over a given other sequence. One
% could argue that this makes the construction a bit more complicated but one could also argue that
% sequences are just more general in that they allow arbitrary construction rules - which may be 
% even more complicated than taking partial sums over a given sequence. I think, for series, we may
% also have more rules to apply which apply only because of the special construction rule. Maybe the
% integral test for convergence is an example for such a rule. It can't be applied to arbitrary
% sequences. So, in some ways, series are a more "advanced" topic than sequences in that they 
% require sequences to be understood to even approach it but in other ways, they are just a special
% case of the more general concept of sequences. This may be confusing to the beginner. But I think,
% it can be said that sequences are more fundamental than series. Actually, our "Infinite Anythings"
% section could also be named "Misc Sequences". Infinite products, the convergents of continued
% fraction expansions, successive approximations in a Newton-Raphson iteration scheme, etc. - they
% are all just specific families of sequences.
...
% Q: Does sum_{1}^{\infty} \frac{1}{k \ln k}   converge? If so, what's the value?
...
We met the notion of convergence already in the context of sequences. Since a series is just a special kind of sequence, we can apply this notion of convergence to sequences, too. 

\subsubsection{Absolute Convergence}
Specifically for series, there is a definition for an even stronger form of convergence that is relevant in certain contexts. This stronger type of convergence is called \emph{absolute convergence}. If a series converges absolutely, it means that we can form a new series by taking the absolute values of terms of the original series and that new series still converges. We can intuitively see that this is a stronger requirement by noticing that when a series has positive and negative terms, they may partially cancel out thereby making the partial sums smaller compared to the case when all terms would have the same sign. Series that converge but do not converge absolutely, have a rather peculiar property which is encapsulated in...

% ...TBC...
% ToDo: Give the formula for the requirement for absolute convergence:
% $\sum_{k=k_0}^{\infty} |a_k| < \infty $
...
\subsubsection{Riemann's Reordering Theorem} There is a pretty interesting theorem that applies to conditionally convergent series, i.e. series that do converge but fail to converge absolutely. Such series can be made to converge to any value just by reordering the terms. It can even be made to diverge in any way you want, i.e. to plus or minus infinity or oscillate forever. This theorem is called the \emph{Riemann reordering theorem}, \emph{Riemann rearrangement theorem} or \emph{Riemann series theorem}. That result is kinda weird because in finite sums, we can reorder the terms in any way we like without changing the final sum. That is a consequence of the associativity and commutativity of addition. Infinite sums may break these laws of addition in certain cases. [VERIFY!] ...TBC....Give example - maybe use the alternating harmonic series and show how to make it approach any value

% Explain use cases. This video has some at the end (at 20:40):
...
% This section should come below the one where we explain absolute convergence!

% The Sum That Equals Any Number You Want
% https://www.youtube.com/watch?v=98TQzC4s9gI
...
% What If You Add All The Numbers?
% https://www.youtube.com/watch?v=dDWprjY5k7o

% This Sum Should Be Infinite. But Is It?
% https://www.youtube.com/watch?v=8qf6DlZ4x6o
...
% A series goes to infinity BUT "slower than" the series of 1/n?
% https://www.youtube.com/watch?v=sJ1eSTWl8EU
% -Doesn't really call it like that - but it's about a rate of divergence. Usually one sees
%  discussion of rates of convergence - but asking for a rate of divergence could also be
%  interesting
% -Maybe it makes sense to compare rates of convergence to zero with rates of divergence
%  to infinity of sequences by turning the divergent sequence into a convergent one by taking
%  the reciprocals of the elements. To make the comparison also applicable to sequences (a_n)
%  that converge to some arbitrary limit A, we can consider the sequence (a_n-A) which converges
%  to zero with the same rate as (a_n) converges to A. I'm intnetionally reverting back to talk
%  about sequences here rather than series because it makes more sense this way.
% -The example in the video of a series that diverges more slowly than 1/n is 1/(n*log(n)). Can we
%  produce even slower diverging series - maybe by using 1/(n*log(n^k)) or 1/(n*(log(n))^k) for
%  some k > 1? I think the first one just boils down to 1/(n*k*log(n)) so it's just a linear
%  scaling so it's not very interesting.
...
% ToDo: Maybe write a footnote that explains the relation between 2nd derivative and curvature as it is defined in differential geometry (as the reciprocal of the radius of the osculating circle). Exlpain how expanding the formula for that actual curvature in a Taylor series and cutting that expansion off after quadratic (I think) term, yields the 2nd derivative, i.e. the 2nd derivative is itself a Taylor approximation of the true curvature.
...
% -At around 20:00, it gives an intuitive explanation why the Taylor expansion breaks at the radius given by the distance to teh closest pole. It uses 1/x^2 approximated around a=1. It has a pole at x=0. The expansion tries to mimic that exploding behavior at x=0 but due to symmetry of the monomials, it must therefore also explode at x=2.
...
% Explain the effect of replacing one of the coeff a_k by some other coeff b_k. I think, it has the effect of adding a monomial term of the form (a_k - b_k) x^k. This can be seen by reframing the replacement of a_k by b_k as an addition of (b_k - a_k) to the original a_k. We can also see this by noting that a_k x^k = b_k x^k  + (a_k - b_k) x^k. What this shows is that just replacing one coeff by some other value does not create any interesting brand new function but just the same old function plus a scaled monomial.
...
% The Hidden Formula That Inverts Almost Any Function
% https://www.youtube.com/watch?v=3yYEV2JwIrg
...
% ToDo: Give the formula for how the Fourier transform can be obtained from the Fourier series by
% taking a limit such that the period approaches infinity. Give also the formula for how the so
% defined formula applies to / specializes to periodic functions by expressing the Fourier transform
% as a superposition of scaled and shifted Dirac delta functions. But maybe these are topices that 
% should be reserved for the section about integral transforms in the functional analysis chapter.
...
%\subsection{Other Series Expansions}

% ToDo:
% Introduce Sturm-Liouville theory as a generalization of Fourier series expansions. See the
% book "Harmonic Analysis for Engineers and Applied Scientists" (Chirikjian, Kyatkin), pg 45 ff.
% The idea is that you set up a specific kind of ordinary differential euqation that includes a
% couple of functions that you may choose freely (a weighting function w(x) and two other 
% functions q(x), s(x) Q: what role play these? and a set of (eigen)values \lambda_n) and you 
% can also choose boundary conditions. If, after having made these choices, you find the set of
% solutions to the resulting ODE, this set can be used as an orthogonal set of basis functions
% into which you can then expand an arbitrary function f(x). The Fourier basis fall out for the
% a specific choice, namely: s(x) = w(x) = 1, q(x) = 0, \lambda_n = n^2. The interval [a,b] on 
% which these functions are defined is [0,2pi] and the boundary conditions are perdiodic. For 
% other choices, other sets of basis functions fall out. Maybe mention it here and go into more
% detail in the chapter aboput ODEs. Maybe devote a section "Sturm Liouville Theory" to that 
% topic. The way I understand it so far is that Sturm-Liouville theory gives a systematic way
% to generate sets of orthogonal basis functions that can be used to expand functions in. Figure
% out how one can taylor the set of basis functions to be useful ina specific given context. Also
% give a list other other common sets of basis functions that are produced in this way.
%
% What other systematic way to generate useful sets of basis functions are there?
...
You Shall Not Pass: Mathematical Infinity Limits You Need to Know
https://www.youtube.com/watch?v=wrTTKtEq3fc

My Personal Roadmap to Sequences & Series
https://www.youtube.com/watch?v=SHKN-5R-CCc
