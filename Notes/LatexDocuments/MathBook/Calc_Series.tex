...
As a side note, an analoguos operation of convolution can also be defined between two function $f,g: \mathbb{R} \rightarrow \mathbb{R}$. In this case, the convolution is defined via an integral instead of a sum:
\begin{equation}
 h(x) = f(x) \ast g(x) = g(x) \ast f(x) \quad \text{with} \quad
 h(x) = \int_{-\infty}^{\infty} f(u) g(x-u) \, du = \int_{-\infty}^{\infty} g(u) f(x-u) \, du
\end{equation}
This is not relevant in this context here but it's sometimes good to point out the connections to other topics.
