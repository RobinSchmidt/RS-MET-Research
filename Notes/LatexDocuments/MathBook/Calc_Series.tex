...
\end{equation}
where the $a_n=\ldots, b_n=\ldots$ formulas apply to the $n \neq 0$ case. We can intepret equation \ref{Eq:FourierSeries} as a synthesis formula and equation \ref{Eq:FourierCoeffs} as an analysis formula in the sense that the former is used to synthesize (i.e. generate, create) a periodic function $f(x)$ whereas the latter takes a periodic function $f(x)$ as input, analyzes it, and spits out the synthesis coefficients. In signal processing terms,  the analysis formula actually computes a cross-correlation between the input function $f(x)$ and the respective sine or cosine function.

\medskip
...TBC...explain the convergence properties, requirements on $f$ ((square?)-integrable?), why this special case for $a_0$

\paragraph{Complex Formulation}
The theory of Fourier series if often being presented in its complex form. That hides some of its complexity in the complex numbers. Instead of using real valued sine and cosine functions for analysis and synthesis, one uses a single complex exponential function. By doing so, one reduces the apparent complexity of having to deal with two sets of coefficients, namely the $a_n$ and $b_n$ coefficients, to a single set of $c_n$ coefficients - but these coeffs are then complex valued.
