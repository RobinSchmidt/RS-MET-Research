...
\end{equation}
where I have also given the conversion formula for the other direction. With these new coeffs, we can express the series as:
\begin{equation}
f(x) = \sum_{k=0}^\infty A_k \sin(k x +\varphi_k)
\end{equation}
[VERIFY!] check, if it also works for the $a_0$ coeff - I don't think so. Give it also in terms of cosines (just add or subtract $\pi/4$ or something)
\newline
