...
% Summation and differencing are inverses - taking the difference of a summation of a function gives back the original function:
%\begin{equation}
% \Delta \sum_{k=a}^{n-1} f(n)
%=  \sum_{k=a}^{n} f(n) - \sum_{k=a}^{n-1} f(n) = f(n)
%\end{equation}
%is analog to:
%\begin{equation}
% \frac{d}{dx} \int_{a}^{x} f(t) \, dt = f(x)
%\end{equation}
% Doing it the other way around, i.e. differencing first and then applying summation, we get the original back up to an additive constant.

% VERIFY these! See here at 10:30:
...
\paragraph{Order of Difference Equations}
The \emph{order} of a difference equation is defined to be the maximum difference in the argument $n$ to the unknown function $y(n)$. One might guess that this corresponds to the highest difference that would occur when writing the equation in terms of the difference operator. However, this is not always so. Consider the following example, written down in both ways:
\begin{equation}
...
% This example is taken from hamming, page 212. However. In DSP, the order of a filter is given by the maximum delay - and we count delays for both feedback and feedforward (input) path. Here in this example, the filter would be y[n] = x[n-2], i.e. a 2 sample delay - and we would actually call this a 2nd order filter - namely a 2-zero filter

...TBC...

% ToDo:
% - Explain that the order of the DE is given by the maximum difference of the argument to the
%   *unknown* function y(n) (not to tze known rhs function f(n, if any))
% - Explain that writing the DE in terms of the difference operator can be deceiving in terms of 
%   what ordre the equation has. For example: D^2 y(n) + 2 D y(n) + y(n) = f(n) looks like it is
%   2nd order but expanding the deltas yields y(n+2) = f(n) which is trivial (i.e. 0th order).
%   See Hamming, pg 212
% - Maybe also mention functional equations as genenalization. Maybe use the example for the
%   factorial / gamma function as example for a simple nonlinear functional equation.

% Q: How would go the other way around - rewrite an equation given in terms of shifts in terms of differences? It probably just a bit of simple (but possibly messy) algebra

% Difference Equation vs Differential Equation: How Are They Similar? How Are They Different?
% https://www.youtube.com/watch?v=oimJ8jNmDRU
...
% The Most Useful Fractions in Math | Bernoulli's Numbers
% https://www.youtube.com/watch?v=h9LPb86H2tY
...
%\subsubsection{Approximating Derivatives with Differences}

% stability analysis: z-trafo for digital filters, von Neumann analysis for PDE solvers
...
% Maybe call it "Fractional Sums" ..but that suggests that we want to evaluate only at rational
% numbers when in fact, the techniques apply to real (and I think even complex) numbers
\subsection{Interpolation of Sums}
Suppose we are given a function $f(x)$ and suppose that for some order $m$, the difference $\Delta^m$ of $f$ vanishes for large values of $x$ such that $\lim_{x \rightarrow \infty} \Delta^m f(x) = 0$. We want to evaluate the sum $S(x)$ defined via:
\begin{equation}
S(x) = \sum_{k=1}^{x} f(k)
\end{equation}
...
% https://www.youtube.com/watch?v=N0Ho0XGuXZs

% Adding a Non-Integer Number of Terms
% https://www.youtube.com/watch?v=9naIxxeNdMk
% - Has another formula:
%   \sum_{k=1}^{x} f(k) = \sum_{k=1}^{\infty} ( f(k) - f(x+k) )  +   x \cdot f(\infty)
% - Maybe this formula reduces to the one given above when f(inf) = 0?
% -> Implement both formulas and try them out on some example functions f(k)
...
Deriving The Fibonacci Function From Scratch
https://www.youtube.com/watch?v=y8Wi8OZIzL4
