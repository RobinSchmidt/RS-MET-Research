\section{Functionals and Distributions} 
...
f(x) = 
 ...
 \end{cases}
\end{equation}
It is indeed infinitely often differentiable everywhere, even at the potentially problematic junctions at $-1$ and $+1$. It is, however, not analytic there: a power series expansion centered at these points will not converge to the function $f$. It think, it will converge to the zero function instead (VERIFY!).
...
% Ableitungen à la carte (Borels Lemma)
...
% Point out that we should not confuse the term distribtuion used here with the usage in probability theory. It's a different condept. But maybe there is some connection between the two? Figure out! See this video:

% The most misunderstood function in physics
% https://www.youtube.com/watch?v=eAkOrWJEWxk

% It says that it has indeed something to do with the idea of a probability density distribtuion

\subsubsection{Regular Distributions}
One way to construct such a linear functional is to multiply the input (test) function $f$ with some fixed other function $g$ and integrate the product over the interval $(a,b)$ over which our test function $f$ is nonzero\footnote{Which is obviously equivalent to integrating from $-\infty$ to $\infty$ so we could have just as well have used this infinite integration interval in the definition.}. Let's call the resulting functional $G$, so we have:
\begin{equation}
...
% Explain what functions g are suitable for defining regular distributions. They must be such that the integral above exists for any given f. I think, that means the ymust be locally integrable

% https://en.wikipedia.org/wiki/Locally_integrable_function
% https://de.wikipedia.org/wiki/Lokal_integrierbare_Funktion
% https://mathworld.wolfram.com/LocallyIntegrable.html
...
% They lose the main feature that we expect from functions: mapping values to values. But they retain some important other features. Explain which (see Weitz's video about the delta function).
...
%   are finite in these points. This is because integrals don't care about the function values at 
%   isolated points (at least, as long as these values are finite)
%  -This notion of weak equality establishes an equivalence relation of the set of (suitable)
%   functions. Maybe bring as example two variations of the sign function. One that defines 
%   sign(0) = 0 and another that defines sign(0) = 1
...
%
% Google's AI says:
% "Functional Analysis and Measure Theory: Two functions, f and g, are considered weakly equal (or equal almost everywhere, denoted f = g a.e.) if the set of points where they differ has measure zero."
% when searching for weakly equal function
...
%  (Lecture notes by Steven G. Johnson)
% https://math.mit.edu/~stevenj/18.303/delta-notes.pdf  
% -Very well written lecture notes. It's like a tutorial.
...
% -I have a copy with highlighting here:
%  https://drive.google.com/drive/folders/1THzpUt9L0vX10yvBjMoOMEd6xmF4YMk0

% What physics students need to know about the Dirac delta function
% https://www.youtube.com/watch?v=eAkOrWJEWxk
...
\end{equation}
Now, observe how there's no $g'$ anymore in the right hand side. That means, we don't have to be able to differentiate $g$ anymore! Instead, we now have to differentiate $f$. Is that any better? Yes - it absolutely is! Differentiating $f$ will never pose a problem to us because $f$ was assumed to be a test function, so it's smooth (by definition) which means that it is differentiable, even infinitely often(!), everywhere. For test functions, we also we have $f(a) = f(b) = 0$ which makes the boundary terms vanish. So, what remains is:
\begin{equation}
 G'(f) = - \int_a^b g(x) f'(x) \; dx
\end{equation}
...
% ToDo:
% -Explain that for the derivatives of jump discontinuities, we see delta functions scaled by the
%  height of the jump. If the function f(x) jumps down by 3 at x = 5, for example, the scaling 
%  factor would  be -3. We would see a $ -3 \delta(x - 5)$ term appear in the derivative f'(x). The
%  function value f(5) would be irrelevant (I think)
% -Explain what is mean by the formulation "test it with a test function". I think, this means the 
%  evaluation of the integral of the product of f with the test function in the case of a regular
%  distribution. But what about singular distributions?
...
% I finally understood the Weak Formulation for Finite Element Analysis  by Dr. Simulate
% https://www.youtube.com/watch?v=xZpESocdvn4
