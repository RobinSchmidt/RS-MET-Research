...
\end{equation}
Now we have to differentiate $f$ instead of $g$ and that will never pose a problem to us because $f$ was assumed to be a test function, so it's differentiable infinitely often. Moreover, the boundary term vanishes because we have $f(a) = f(b) = 0$. So, what remains is:
\begin{equation}
 G'(f) = - \int_a^b g(x) f'(x) \; dx
\end{equation}
