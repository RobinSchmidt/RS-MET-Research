...
% I think, the statement "The dimensionality of the variety will typically be given by $n-m$" can be made precise in the sense taht "atypical" situations where the dimension is less than n-m can be considered aedge cases that occur for a set of measure zero within some parameter space of polynomial coeffs? For example, when we consider 1 equation in 3D of degree at most 2 such that
% F(x,y) = a + b x + c y + d x^2 + e xy + f y^2  will typically be a 2D manifold but thare are "edge cases" for cases that lie on a manifold with less than 7 dimensions in the 7D parameter space. Is that correct? It's my personal mental image. So, when we pick the 7 parameters a,b,c,d,e,f at random, the chacce to pick a combination such that the variety is not a 2D manifold but some lower dimensional object is zero?

% But: what about when the solutions become complex?
...
% Can a variety geometrically be characterized as a union of manifolds?
...
https://www.youtube.com/watch?v=-iF9h9fePYk  Calculations in Another World

https://en.wikipedia.org/wiki/Tropical_geometry
https://de.wikipedia.org/wiki/Tropische_Geometrie
- "tropische Geometrie ... kann als stückweise-linearisierte Version der algebraischen
   Geometrie aufgefasst werden. Algebraische Varietäten werden dadurch zu kombinatorischen Objekten"
...
-About schemes
-1:30: "A topology is a notion of nearness". Q: How is that captured in the set-theoretical 
 definition of "a topology"? -> something for the Topology chapter.

Maybe the relation between algebraic geometry and differential geometry can be framed as: In 
algebraic geometry, the problem is to find the zero set of a system of multivariate polynomials. 
These zero sets are manifolds (of maybe sets of manifolds - the zero sets need not be connected).
In differential geometry, we deal with parametric descriptions of manifolds. Therefore it may make
sense to say that in differential geometry, we deal with objects that can be seen as *solutions* to
the *problems* that are raised in differential geometry. For example, if the problem is: find the
zero set of the bivariate polynomial x^2 + y^2 - 1, i.e. the set of all pairs (x,y) such that
x^2 + y^2 - 1 = 0, the solution is (x(t),y(t)) = (cos(t), sin(t)) with t in 0..2pi. The solution is
not unique though because it can be re-parametrized, for example by using  (x(t),y(t)) = 
(cos(t^2), sin(t^2)) with t in in 0..sqrt(2pi). Maybe it can be made unique if we demand a 
parametrization by arc length. In the general case where the manifold is nD, we should perhaps 
demand that all curves that result when we hold n-1 parameters constant and only vary one of them,
the resulting curves should all be parametrized by arc-length.
