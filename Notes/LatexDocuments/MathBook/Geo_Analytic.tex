...
\begin{tikzpicture}[
  ...
  \draw[->] (0,-0.3) -- (0,5) coordinate[label = {left:$y$}] (ymax);
  \path[name path=x] (0.3,0.5) -- (6.7,4.7);
  \path[name path=y] plot[smooth] coordinates {(-0.3,2) (2,1.5) (4,2.8) (6,5)};
  \scope[name intersections = {of = x and y, name = i}]
    \draw      (0.3,0.5) -- (6.7,4.7) node[pos=0.8, below right] {Line};
    \draw (i-1) node[dot, label = {above:$\mathbf{r}_1$}] (i-1) {} -- node[left]
      {$f(x_0)$} (i-1 |- O) node[dot, label = {below:$x_1$}] {};
    \path (i-2) node[dot, label = {above:$\mathbf{r}_2$}] (i-2) {} -- (i-2 |- i-1)
      node[dot] (i-12) {};
      
    \draw(i-12) -- (i-12 |- O) node[dot, label = {below:$x_2$}] {};                     
    \draw[blue, <->] (i-2) -- node[right] {$y_2 - y_1 $} (i-12);
            
    \draw[blue, <->] (i-1) -- node[below] {$x_2 - x_1$} (i-12);
