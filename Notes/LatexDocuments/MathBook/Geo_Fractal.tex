...
% Riemann's fractal function
% https://www.youtube.com/shorts/FgdFOehfL_E
% -The function is: f(x) = \sum_{n=1}^\infty \frac{\sin(\pi n^2 x)}{n^2}
% -One can also replace the sine with the cosine to get another function and an interesting 2D 
%  fractal resuls when on uses the original f for the x-coordinate and the cosinne version for the
%  y-coordinate in a parametric curve.
% -Maybe write some C++ code to generate various fractal functions and use them in an oscillator.
%  Maybe generalize from sine/cosine to a pair of sines with arbitrary phase or maybe even make the
%  phase a function of n to get more variations. Experiment with other exponent for n, not just n^2.
%  I guess using n^p with use adjustable p will make the sound brighter for lower p.
