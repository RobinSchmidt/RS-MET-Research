...
% -motivate this definition of matrix multiplication by considering a matrix a s a function to
%  be applied to a vector and the ask what function composition would do. The matrix mutliplication
%  rule falls out naturally and necessarily.  
...
% -To see that matrix multilication is not commutative, observe that the entries of the result are
%  formed by taking scalar products of the _rows_ of the left operand with the _columns_ of the
%  right operand. If you swap the operands, you would be taking very different scalar products and
%  there's really no reason to assume that they would give the same results.

% How Matrix Multiplication Was Discovered
% https://www.youtube.com/watch?v=5K6_UNOMjzg
...
% TODO: Write a little bit about the importance of these featuers. 

% When matrices occurr in various applications, we sometimes can ensure that some of these properties listed below will hold for our matrices. Sometimes we can ensure this by the way we construct these matrices, sometimes by other means (which?). (Examples: Jacobian matrices (symmetric), Covariance matrices (symmetric), ...Hermitian and unitary matrices in Quantum mechanics, etc.). When faced with a problem involving a mtarix, it is always advisable to first check, if our matrix in question must satisfy some of the special properties listed below because if it does, we may have some more theorems atv our disposal that we may use to solve our porblems. ...tbc...
...
%\paragraph{Frobenius Normal Form}

% The Single Most Undervalued Fact of Linear Algebra
% https://www.youtube.com/watch?v=8gNKE5jVqYY
% -Explains the connection between matrices and graphs.
% -A matrix in Frobenius normal form correponds to a graph that is not strongly connected (verify!)
...
% The SECRET Property of ALL 2 x 2 matrices!  Linear Algebra Proof!
% https://www.youtube.com/watch?v=pwkNnHVHVUc
% -For any pair A,B of 2x2 matrices: (A B - B A)^2 = c I where c is a constant and I is the 2x2 
%  identity matrix
...
https://www.youtube.com/watch?v=TNYTcW8kkqk  How Matrices Were Discovered


How about defining a "commutative product" or "symmetric product" as follows:

  C = (AB + BA) / 2

If A and B commute, it would be identical to the normal matrix product, so the result would be just
AB which would also equal BA in this special case. Can we say something interesting about the
relation between AB-C and BA-C in the general case? We could call these matrices the left and
right asymmetric parts of the product of A and B.
