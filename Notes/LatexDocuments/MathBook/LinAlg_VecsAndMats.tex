...
%all of of its columns are orthogonal to each other, i.e. the scalar product of one column with any other column gives zero. 


%A square matrix is called \emph{orthogonal} when all of of its columns are orthogonal to each other, i.e. the scalar product of one column with any other column gives zero. 

%Non-obviously, this is equivalent to requiring that the scalar product of each row with any other row gives zero [VERIFY]. 

%Often it is also required that the scalar product of a column (or row) with itself must be one. This terminology is inconsistent with the definition of orthogonality of a set of vectors. When a set of vectors has this feature of pairwise orthogonality \emph{and} every vector is of unit length (that's what it means to have a scalar product of one with oneself), then that set of vectors is called \emph{orthonormal} and ortho\emph{gon}ality does not require the norms to be unity. Some (very few) authors also define matrices as ortho\emph{norm}al when both, pairwise orthogonality and unit norm of the columns (or rows), hold true and indeed use the term ortho\emph{gon}al for the less restrictive condition that is consistent with orthogonality with a set of vectors. I think using the consistent terminology is actually better but unfortunately, the inconsistent terminology seems to have prevailed. So, an ortho\emph{gon}al matrix is one whose columns (or rows) are an ortho\emph{norm}al set of vectors.
...
% Toeplitz, Circulant, Unitary, Jordan
