...
A regular eigenvector $\mathbf{v}$ to an eigenvalue $\lambda$ is a vector with the property $\mathbf{A v} = \lambda \mathbf{v}$, i.e. a solution to the equation $(\mathbf{A} - \lambda \mathbf{I}) \mathbf{v} = \mathbf{0}$. Let's give the matrix in this equation a name: $\mathbf{N} = \mathbf{A} - \lambda \mathbf{I}$ so we can write: $\mathbf{N v} = \mathbf{0}$ instead for the eigenvector equation. Now a generalized eigenvector is a vector $\mathbf{v}$ with the property  $\mathbf{N}^k \mathbf{v} = \mathbf{0}$ for some natural number $k$. [VERIFY, Figure out if $k$ has a special name. Order? Weight? Degree?]
