...
Two matrices $\mathbf{A}$ and $\mathbf{B}$ are said to be simultaneously diagonalizable if there exists a single matrix $\mathbf{P}$ such that $\mathbf{P}^{-1} \mathbf{A} \mathbf{P}$ and  $\mathbf{P}^{-1} \mathbf{B} \mathbf{P}$ are both diagonal matrices. For that to happen, the matrices $\mathbf{A}$ and $\mathbf{B}$ must have the same eigenvectors because in a diagonalization, these eigenvectors appear as the columns of the diagonalizing $\mathbf{P}$ matrix [VERIFY]. Note how the features of being \emph{similar} and being \emph{simultaneously diagonalizable} are complementary: Two matrices are similar, iff [VERIFY!] they have the same eigen\emph{values}. Two matrices are simultaneously diagonalizable iff [VERIFY] they have the same eigen\emph{vectors}. That implies that if two matrices have both features, they must have the same eigenvalues \emph{and} the same eigenvectors and hence in fact be the same matrix [VERIFY]. Two matrices commute if and only if they are simultaneously diagonalizable. A set $A$ of matrices is simultaneously diagonalizable if all pairs of matrices in the the set are simultaneously diagonalizable.  [TODO: there are some conclusions in here that I have drawn myself and need to be verified]
