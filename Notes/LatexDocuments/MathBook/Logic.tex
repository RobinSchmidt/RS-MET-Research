...
% The similarity of and/or and multiplication/addition is as follows: we can interpret 0 as F and
% any positive value as T. Multiplying by 0 is the same as "and"ing with F. Adding a positive value
% to anything results in a positive value. If we don't want to deal with values > 1 and want a 
% unique representation of T, we can do our binary calculuations mod 2. Note that the mod operation
% must then be done after each addition/or-operation. We canot just chain any number of additions 
% and the do a single mod because then all the even sums would map to F but they they should map
% to T as well.
...
% Rsolution: (p or r) and (q or not r)  ->  p or q
...
% Proof theory studies proofs based on syntax, i.e. it's about proof that are formally verifiable

% The soundness and completeness of logic
% https://www.youtube.com/watch?v=IjqaEdoOuBc

% Resolution Proofs in Logic
% https://www.youtube.com/watch?v=8qV56o7uW0w
% - It's an automated way to produce a proof by contradiction
...
% https://www.youtube.com/watch?v=FwVaTzEbDNM  Proofs with and without induction



% It's Not Just You. Proof By Contradiction Feels Wrong
% https://www.youtube.com/watch?v=iTRxf70u7u8
...
% ToDo:
% -Give truth tables for not, and, or for the 3 possible values True, False, Unknown
...
% 3-valued logic uses 3 values: T,F,U for true,false,unknown. It uses 3x3 truth tables to define
% the logical operators "and" and "or"

% Maybe it could also be interesting to consider a 4-valued logical system that treats truth and
% falsehood as one aspect and knowledge or ignorance as another "orthogonal" aspect. We could work
% with values T,F for true,false and with values K,U for known,unknown. Then we would have 4 cases:
% KT: known and true, KF: known and false, UT: unknown and true, UF: unknown and false. Then we 
% would make combined truth-knowledge tables like so:
%
%        AND                  OR
%
%     KT KF UT UF          KT KF UT UF        ...verify these! I really had to think a bit to
%  KT KT KF UT UF       KT KT KT KT KT        fill these out. That may be a strong hint that
%  KF KF KF KF KF       KF KT KF UT UF        mechansing reasoning with such a system may be a
%  UT UT KF UT UF       UT KT UT UT UT        worthwhile thing to do.
%  UF UF KF UF UF       UF KT UF UT UF
%
% Maybe we could have two types of negation operators. The regular one operates on the T,F values
% as before and the the new one operates on the K,U values. Although, to realistically model
% knowledge, this operator would not really model any real process. With knowledge, it would make
% more sense to have unary operators "learn" and "forget" where "learn" sets K and U both to K and
% forget set K and U both to false. Flipping the knowledge state isn't really thing. There is no 
% process that makes us forget what we know and simultaneously lets us learn what we don't know.
% This difference could perhaps be the thing that makes such a system interesting.
%
% 4-valued logic could be seen as a refinement of 3-valued logic where the U value from 3-valued 
% logic is further subdivided into UT and UF. To map a 4-valued value to a 3-valued value we would
% do: KT -> T, KF -> F, UT -> U, UF -> U. Going from 3 to 4 values would basically restore the 
% property that we always have knowledge about the truth or falsehood of the facts/propositions 
% themselves and it would use the K,U distinction not to model lack of knowledge about the facts
% themselves (as we do with the U value in 3-valued logic) but rather to model a higher level
% knowledge (meta knowledge) about the presence or absence of knowledge about the facts.
%
% Maybe we could even go to higher levels with this scheme to model our knowledge about knowledge.
% Like KKT would mean: we know that we know that it's true, etc.
...
How to Speak Math — Discrete Math Ep. 1
https://www.youtube.com/watch?v=_w_ESLeOFUU

Introduction to Type Theory
https://www.youtube.com/watch?v=ZuU29pV1e14

Introduction to Math Speak
https://www.youtube.com/watch?v=JDFAlRv2Oio

The Most Disappointing Proof in Maths
https://www.youtube.com/watch?v=xVM5hdXRgLs
-About Goedel's incompleteness theorems

Automated Theorem Proving | How Computers Do Logic
https://www.youtube.com/watch?v=hUxhqXm9cgA

Discrete Mathematics Isn’t Hard. It’s Taught Wrong.
https://www.youtube.com/watch?v=dmoLNj6-Rkg
-Starts with logic but also builds up to higher level topics. There are egregious mistakes, though
