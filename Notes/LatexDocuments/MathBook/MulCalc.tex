...
- https://www.youtube.com/watch?v=3ael6LnLvOA  Quintessence of Matrix Calculus （#SoME4)
  -has also some material about autodiff (can be applied to matrix-calc, I guess?)
...
-Calculus on manifolds:
 -Differential goemtery is used to describe the manifold itself.
 -We deal with scalar- and vector fields (maybe also tensor fields) _on_ the manifold
  -I think, we have different mathematical formalisms at our disposal to express these things:
   tensor calc, exterior calc, vector calc (for 2D manifolds in 3D spcae - verify!)
 -Important ideas: tangent space, covariant derivative, vector bundles?, pullback/pushforward
 -Currently, the whole Geometry chapter is after the mutivariable calc chapter. The only thing that
  comes after is "Functional Analysis". Maybe we could make "Calc on Manifolds" and "Functional 
  Analysis" sections in some sort of "Higher Calculus" or "Beyond Calculus" section.
 -Maybe some of the  multivariable calc topic should be moved into this "Higher Calc" chapter. Like
  Exterior, Geometric and Tensor calc.
 -Start by introducing (scalar, periodic) functions on a circle. Visualize them in 2 ways:
  1: by wrapping the function values f(t) around a cylinder in xyz-space, i.e. as the 3D curve:
     (x(t),y(t),z(t)) = (cos(2pi*t),sin(2pi*t),f(t))
  2: by creating a "height-relief" on a 2D curve like so: 
     (x(t),y(t)) = (r(t)*cos(2pi*t),r(t)*sin(2pi*t)) with r(t) = 1 + a*f(t)  with some suitable 
     scaling factor a such that the ripples on the circle look nice.
  -Point out the equivalence (isomorpy) between a circle and a 2pi long section of te number line
   when we identify the endpoints of the interval with one another.
  -Discuss expansions as Fourier series.
 -Proceed to introducing double periodic functions in the xy-plane and how they can be viewed as
  functions on a torus. We may also viusalize them as height relief or by color coding the values
  (or both)
  -Discuss expansions as 2D Fourier series
 -The introduce functions on the sphere.
  -Discuass expansions in shperical harmonics.
