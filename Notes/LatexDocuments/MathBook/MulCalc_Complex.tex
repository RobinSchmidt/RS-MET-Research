...
In complex analysis, the integrals we are mostly interested in are so called contour integrals. To set these up, we need as first ingredient a given \emph{contour} $\gamma$. This is some curve in the complex plane. That means $\gamma$ is a function from some real interval $[a,b]$ into the complex plane $\gamma: [a,b] \rightarrow \mathbb{C}$. As second ingredient, we need out actual function $f(z)$ which we want to integrate. When we then define the contour integral of $f(z)$ along the contour $\gamma$ like this:
\begin{equation}
