...
% I think, among the square roots of unity, -1 is primitive but +1 is not because +1 does not 
% generate the whole set of all (two) square roots of 1 via its powers. Contrast that to the 
% square roots of -1. These are +i and -i and both of them generate the full set (plus some more
% elements, namely -1 and +1). If we consider the powers of i (or -i), we will generate: i,-1,-i,1 
% and then cycle back. I think, these power of i (or -i) form a group under complex multiplications.
% What about the n-th roots of an arbitrary complex number z on the unit circle? In which cases will
% we get groups?




\paragraph{Primitive and Principal Roots}
As we have seen, in the complex domain, the $n$th root function is multi valued. Among the possible values that solve the equation $z^n = w$ for some given right hand side $w$, some are designated as more fundamental than others and have special names.

% ToDo: point out the danger of confusing the two notions. The words sound similar and the meaning 
% is similar as well - but the definitions are subtly different nonetheless.

% https://en.wikipedia.org/wiki/Root_of_unity#General_definition
% -> contains definitions of "primitive root"

% https://en.wiktionary.org/wiki/principal_root
% -> pricipal root greates real part and positive imaginary part (implies: smallest angle?)
% https://en.wikipedia.org/wiki/Principal_value#Square_root

% https://mathworld.wolfram.com/PrimitiveRoot.html
% https://mathworld.wolfram.com/PrincipalRootofUnity.html

% I think, the notions of primitive and/or principal roots apply also to modular integers.

% I think, a primitive n-th root is one whose powers are able to generate all the other n-th roots.
% The priciple root is just one of them. Consider the 6-th roots of unity and denote the principal 
% value by r. Among the other 6th roots, I think, r^5 would also qualify as "primitive" because its
% powers also generate all 6th roots of unity (but in a different order than the powers of r 
% itself). I think r^m is a primitive root whenever m and n are mutually prime? ...verify all of 
% this - I'm not sure.

\subsubsection{Exponential and Trigonometric functions}
Extending the domains of rational and algebraic functions from the real to the complex numbers was a straightforward process requiring only some algebra. The $n$th roots are a bit messy to deal with because we needed to introduce the concept of multivalued functions but they nevertheless did not require much higher level math tools to define. To extend the domains of the exponential and trigonometric functions, we'll need a bigger gun from calculus: power series. The nice thing about power series is that in order to evaluate them, we only need to know how to perform additions and multiplications - and we actually indeed do know, how to add and multiply complex numbers! Using the power series representation of a function is a very common technique in math whenever we desire to define functions of arguments that are not simple numbers but for which we nevertheless know how to add them, multiply them and scale them by real numbers\footnote{We need addition to perform the summation, multiplication to form the powers and scaling to apply the coefficients. It may appear that scaling by a coefficient and multiplication are the same thing. But this is only the case here, because the (real) scaling factors can also be regarded as complex numbers. But if the objects are matrices, for example, scaling by a scalar factor and (matrix) multiplication are indeed different operations.}. None of these operations are a problem here. We indeed can do all these operations with complex numbers. The same idea can also be applied to matrices, multivectors and even operators. But here, we are interested in complex numbers. Let's recall the power series of some functions:
\begin{equation}
...
% What about functiosn with singularities? If they can be expressed as a rational function of power series without singularities (such as tan = sin/cos), we can just evaluate the series of numberator and denominator separately and the use complex division (It requires division to be defined - no problem here, but may be problematic in other mathematicla structures where no division is available). In other cases we may have to fool around suing different expansion points for different regions of the complex plane (as is regularly done in analytic continuation)
...
% I think, "analytic" is defined as: "can be expandedn in a cenvergent Taylor series" whreas "holomorphic" is defined as: "has a complex derivative" -> verify this. I that's true, it would mean that analyticity is, at first glance, the stronger requirement. Does the idea of "holomorphic" come up somewhere outside complex analysis?

% https://en.wikipedia.org/wiki/Holomorphic_function

\subsection{Complex Integration}

% In real analysis, we integrate over intervals along the real axis. The result is a number that we can geometrically interpret as an area balance. Integrals in complex analysis, on the other hand, can be interpreted physically in terms of work done along a path (real part of the integral) and flux across that path (imaginary part of the integral). For this interpretation, we need to interpret the complex function $f(z)$ as a vector field in the $2D$ plane.
...
% So in complex analysis, the setting is:
% -We interpret f(z) as a vector field that assigns a vector to each point of the z-plane
% -We specify a curve that traces out some path in the z-plane along which we intend to integrate
% -We want to compute the flow along and the flux across our curve
% ...
...
% Maybe just use something like \bar{v} ot \hat{v}. The bar might be beset because we already use that notation for complex conjugation elsewhere. It's somewhat ugly though, to always carry around that bar. Maybe use the notation $\mathbf{p (z)}$ where p is a 2D vector and z is also a 2D vector. That is consistent with our use of vector valued functions of vectors used previously. If we write the Polya vector field like that, it may not matter so much anymore how we call its 2nd component because we now have a notation to write the whole vector, so we may not need to reference its components so often anymore. But: the usual notation p_x, p_y to refecrence its components can't be used because we want to use that for partial derivatives. Maybe do indeed use u, \bar{v}. The second is ugly but we may try to avoid it in formulas as much as possible

% It turns out that when $f$ is an analytic function

% Ahh - damn! This notation of u,v clashes with our previous usage of v and Im(w) rather tahen -Im(w). What can we do? maybe use p,q instead of u,v. We would have p=u, q=-v. P is nice because it's the first letter of Polya. Or maybe just use p = -v and keep using u as is.
...
% I think it's the divergence-free property that rules out minima and maxima in the potential. All stationary points are saddles


\subsubsection{Contour Integrals} 
In complex analysis, the integrals we are mostly interested in are so called contour integrals. To set these up, we need as first ingredient a given \emph{contour} $\gamma$. This is some curve in the complex plane. That means $\gamma = \gamma(t)$ is a function from some real interval $[a,b]$ into the complex plane $\gamma: [a,b] \rightarrow \mathbb{C}$. As second ingredient, we need out actual function $f(z)$ which we want to integrate. When we then define the contour integral of $f(z)$ along the contour $\gamma$ like this:
\begin{equation}
%\int_{\gamma} f(z) \; dz = \int_a^b f(z(t)) z'(t) \; dt
\int_{\gamma} f(z) \; dz = \int_a^b f(\gamma(t)) \gamma'(t) \; dt
\end{equation}
then what we mean by this is the following procedure: We split the interval $[a,b]$ into (infinitely) many small parts and at each value $t \in [a,b]$, we first evaluate $\gamma(t)$. Then .... ...TBC...

% dz = z'(t) dt

\paragraph{Example}
Let our contour be the unit circle parameterized as $\gamma(t) = e^{\i t}$ with $t \in [0, 2\pi]$.

% Maybe take f(z) = sin(z) / z

% https://en.wikipedia.org/wiki/Contour_integration#Contour_integrals
% Uses the notation f(z(t)) z'(t) dt
% but maybe it's better to use f(\gamma(t)) \gamma'(t) dt
% 
...
% https://www.youtube.com/watch?v=_opXzi09t9I  at 14:30
...
In real analysis, integrals for integer power functions are very important as building blocks for evaluating more complicated integrals. For example, they could be used for integrating a power series term wise. There, integrating may have referred to either: finding an antiderivative (indefinite integration) or finding the area balance under a function (definite integration). In complex analysis, we are mostly concerned with definite integrals along contours (aka curves or paths) and most of the time, these contours will form closed loops. The result of such an integration is not another function of $z$ but a (complex) number. Again, contour integrals of integer power functions are a valuable building block for the evaluation of such integrals for more complicated functions. This is because for these, we have a nice and simple formula available and for more complicated functions, we can (due to linearity of integration) just add up the contributions of the individual monomial terms, if we assume the function to be given as a series expansion. For a monomial\footnote{I'm using the term monomial a bit loosely here in the sense that it can refer to an expression like $z^n$ where $n$ is allowed to be any integer. The usual definition assumes $n$ being a natural number so we are actually dealing with a generalized notion of monomials here.} function $f(z) = z^n$, these contour integrals are given by:
\begin{equation}
\oint_{\gamma} z^n \; dz = 
\begin{cases}
0        \qquad & n \neq -1 \\
...
\end{equation}
where the contour $\gamma$ can be any closed loop that winds counterclockwise around the origin exactly once\footnote{Such a loop is said to have a \emph{winding number} of $1$} (verify!). Now this result is rather peculiar. It says that the contour integral around a closed loop around the origin (with winding number 1) for a monomial function $z^n$ is always zero except for the case $n=-1$ in which case it is equal to $2 \pi i$. The case $n=-1$ means that we are dealing with reciprocation function $f(z) = z^{-1} = \frac{1}{z}$. This function is, among all powers of $z$, the odd one out and it is so important that I have even written down the formula for this special case again alongside with the general formula. ...TBC...

% ToDo: mention winding numbers and what changes when the contour loops around multiple times. The value just gets multiplied by the winding number. This number can also be neagtive which means that the loop encircles the origin clockwise rather than counterclockwise
...
\end{equation}
Where I now have also given you the formula to compute the coefficients $a_n$ which I left out in the previous mention, because at that point, you couldn't possibly have made sense of it\footnote{Unless, of course, you already had prior knowledge about complex analysis.} and it may have scared the hell out of you. It may still may look a bit scary, but now we have the tools under our belt to handle it ...TBC...
...
\end{equation}
This is because for $n=-1$, the denominator $(z-z_0)^{n+1}$ in the calculation formula for $a_n$ in equation (\ref{Eq:LaurentSeries}) simplifies to 1. The name residue stems from the fact that it is the only thing that remains after we do a contour integration around a closed loop\footnote{Again: with winding number one. You know the drill by now, so I'll henceforth assume that to be implied.} around $z_0$. ...TBC...
...
% -Has very nice example at the end for using the theorem to compute a real integral. Namely, that of
%  cos(x) / (1 + x^2)  along the whole real line. The result is \pi / \e  ...of all things :-O
...
% Maybe use the closed form of the geometric series as the first motivating example and introduce the
% series for the zeta function only later. Because the geometric series has a nice closed form 
% expression (for |z| < 1) but the expression can easily be evaluated even when |z| >= 1. For zeta, we do not have such a nice epxression, so the situation is more complicated so we should potpone that for later.
...
ToDo:
- Wite down the (Polya) vector field components u(x,y) = x^2 - y^2, v(x,y) = -2 x y of the function 
  f(z) = z^2. Consider a curve given by the unit circle x(t) = cos(t), y(t) = sin(t). Evaluate u,v
  along that circle to produce two functions u(t), v(t). We should ge a sine and cosine as output
  again but with twice the frequency. Wiht u(x,y) = x, v(x,y) = y, we would have gooten the original 
  sine/cosine pair back. We may construct higher degree bivariate polynomimals from z^3, z^4, ... to
  produce sines/cosines with frequencies of all integer frequencies.
...
The Integral That Broke Mathematics | Fresnel Integrals & The Euler Spiral
https://www.youtube.com/watch?v=17nSCNAZ0qQ

Imaginary Numbers to Fractals — 9 Visual Chapters
https://www.youtube.com/watch?v=qp8OqLGp-mA
-6:06 Conformal maps preserve angles, local area is scaled by |f'(z)|^2



Demystifying Complex Numbers: a 300-Year Journey | Applied Complex Variables, ep.1
https://www.youtube.com/watch?v=zgbP5C5mpcU
