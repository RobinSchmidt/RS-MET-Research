...
As in real analysis, integrals for integer power functions are very important. Just note that here, we are not computing indefinite integrals (aka antiderivatives) but rather contour integrals around a closed loop. the result of such an integration is not another function but a (complex) number. For a function $f(z) = z^n$, these integrals are given by:
\begin{equation}
\oint_{\gamma} z^n \; dz = 
\begin{cases}
0        \qquad & n \neq -1 \\
...
\end{cases}
\end{equation}
where the contour $\gamma$ can be any closed loop that winds around the origin exactly once (verify!).
...
% The formula for the coefficient $a_n$ looks scary, but
