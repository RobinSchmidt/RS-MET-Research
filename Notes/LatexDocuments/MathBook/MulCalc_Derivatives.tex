...
The approximation given in equation (\ref{Eq:TaylorMultivariate2ndOrder}) can be interpreted as a quadratic (i.e. 2nd order) approximation to our function $f$. To see where we are going, let's write it as:
\begin{equation}
 f(\mathbf{x + h}) \approx 
   \frac{1}{0!} f(\mathbf{x}) 
 + \frac{1}{1!} \mathbf{g^T h} 
 + \frac{1}{2!} \mathbf{h^T H h}
\end{equation}
This doesn't change anything because $0! = 1, 1! = 1, 2! = 2$ but it may help us to recognize a pattern that we are familiar with from the one dimensional Taylor series formula. The second order Taylor polynomial in 1D was:
%\begin{equation}
% f(x + h) \approx \frac{1}{0!}  h^0 + \frac{1}{1!} 
%\end{equation}



%Consider a scalar valued function with vector valued inputs, i.e. a function $f: \mathbb{R}^n \rightarrow \mathbb{R}$. 

...TBC...ToDo: give motivation and formula - maybe also in multi-index notation


%Multivariable Taylor Series Derivation
%https://www.youtube.com/watch?v=zxafRk45vtE

% ToDo: multivariable Taylor Expansion
