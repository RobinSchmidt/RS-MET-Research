...
%\medskip
\paragraph{Directional Derivative via Gradient}
...
\end{equation}	
for some small deviation vector $\mathbf{h}$. If the input to $f$ is 2D, then this quadratic approximation function will be the locally (at $\mathbf{x}$) best fitting paraboloid. Such second order approximations of a multivariate scalar function play an important role in numerical optimization algorithms, i.e. algorithms that try to locate minima or maxima of such functions. The simpler ones of these algorithms\footnote{For example: gradient descent and its refinements (like using adaptive step size control and/or momentum terms).} use only the gradient but the more advanced ones\footnote{For example: quasi-Newton methods or algorithms based on so called "conjugate gradients".} may also use (an approximation of) the Hessian. So, these ideas are very important in practical applications that involve multidimensional optimization such as machine learning.
...
\end{equation}
Writing it like this may help us to recognize a pattern that we are familiar with from the one dimensional Taylor series formula and it may enable us to find the proper generalization for approximations of arbitrary order. Without further ado, I'll give it to you: A general multivariate Taylor polynomial looks like this:
\begin{equation}
...
%https://en.wikipedia.org/wiki/Taylor_series#Taylor_series_in_multiple_variables
...
% Why I DON'T Like Calculus III
% https://www.youtube.com/watch?v=_37sn8_s8js
% -About how differential give a simpler approach to multivariable calculus than partial derivatives
