...
\begin{equation}
 ...
 \end{pmatrix}
\end{equation}	
is called the Jacobi matrix	of $\mathbf{f}$ at $\mathbf{x}$. The $k$th row of the matrix is the transpose of the gradient of the $k$th component function $f_k$. It defines a linear approximation to $\mathbf{f}$ at a given $\mathbf{x}$ in the sense that for a small vector  $\mathbf{h}$, we have:
\begin{equation}
...
...TBC...
%-explain how this matrix represents a linear approximation: f(x + dx) ~= f(x) + J dx
