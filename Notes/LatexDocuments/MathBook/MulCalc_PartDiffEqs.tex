...
% Explain similarities to the heat equation
...
\paragraph{The Schroedinger Wavefunctions} ...tbc...
%\paragraph{Quantum Wavefunctions} ...tbc...

% aka Schroedinger Equation
% is structurally like the diffusion equation but with a twist: the coefficient is imaginary which turns the whole thing into some sort of wave equation. The complex coefficient implies that the solution function u(x,t) will, in general, itself be complex valued (Q: is it possible that in some very special case, we may get away with a purely real function? I don't think so. If the current state is purely real, the infinitesimal update will be in a purely imaginary direction, I think)
...
% aka Maxwell's Equations -> this is a *system* of PDEs not just a single PDE -> even more difficult
% to handle
...
% -a homogeneous PDE has a zero right hand side
% -an autonomous PDE does not depend on time (or space), i.e. it's time invariant (or more generally)
%  shift-invariant. It has constant coefficients, i.e. no coefficient-functions (or the functions are
%  all just constant functions)


\subsubsection{Order and Degree}
% -order is highest derivative
% -degree is highest total exponent in case of polynomial PDEs, i.e. those that contain products of
%  derivatives - I think, polynomial terms involving only the variables themselves do not count - I %  think, that can be seen as a right-hand side
% -If the degree is 1, the PDE is linear
...
% Subtypes of 2nd order PDEs (which are very common in physcis)
...
% Explains the difference in terms of the sign of B^2 - A C for the 2D case. Maybe in nD, it's a determinant of some matrix? Or maybe about the sign distribution of its eigenvalues?
...
% what else? product ansatz? Laplace transform
...
% Distinctions between methods for static systems (where we don't have a time-stepping) such as the Laplace equation and dynamic systems (where we must do time stepping) such as the wave equation.
...
Laplace equation in 3D:  u_xx + u_yy + u_zz = 0
Wave equation in 2D:     u_xx + u_yy - u_tt = 0

Both are PDEs in 3 independent variables and both are 2nd order in all variables. The only difference (besides the different name for the 3rd coordinate) is that in the 1st, we have all plusses and in the 2nd, we have both plusses and minusses (well just one minus, but one is enough). I think, that's the difference between "elliptic" and "hyperbolic" PDEs? Compare that to euqations of conic sections from analytical geometry. If that is right, what would parabolic in between case look like? Maybe the derivative wrt to the 3rd coordinate must be zero? But then the solution would just be constant wrt to that variable. Figure out! Wait. i think it makes sense to interpret it in terms of coefficients:

 c_x u_xx + c_y u_yy + c_z u_zz = 0

Then, when all coeffs have the smae sign, we have an elliptic PDE, if both signs occur, we have a hyperbolic PDE. And maybe we have a parabolic PDE if at least one coeff is zero? But then the repective variable would be totally unconstrained which seems also wrong.


Chebyshev Collocation Method for the Helmholtz Problem
https://www.youtube.com/shorts/Y5m6hzsm0H4

The Language of Physics Partial Differential Equations (Heat Equation)
https://www.youtube.com/watch?v=de46ITZo6Ag


Gray–Scott Model
https://www.youtube.com/shorts/B6XX_4W9e0Q

This PDE is Beautiful - Phase Field Modeling
https://www.youtube.com/watch?v=areJqeWc49E


ToDo:

Maybe try to write (or vibe code) a little interactive program (maybe in JS or Python as Jupyter
notebook) for exploring linear PDEs with constant coefficients of the general form:

  a_0 + a_1 u_t + a_2 u_tt + ... = b_0 + b_1 u_x + b_2 u_xx + ...

where the user has sliders to dial in the coefficients a_i, b_i and a plot shows the solutions and
updates interactively when the user moves the sliders. It would also need some way to set up intial
and boundary conditions. Maybe it could also make sense to allow for complex coefficients (like in
the Schroedinger equation). If we allow complex coeffs, we could give the user sliders for real and
imaginary parts or mayby use some sort of xy-pad - although I think, that would be harder to use 
because it would be too easy to dial in a nonzero imaginary part unintentionally - so maybe separate
1D sliders are the better choice. The constant coeffs a_0 and b_0 could actually be merged into one
single coeff but for symmetry reasons, I have wrote it down like given. Maybe we could instead 
replace a_0, b_0 by some sort of driving function. Maybe we could use:

  a_1 u_t + a_2 u_tt + ... + b_1 u_x + b_2 u_xx + ... + f(x,t) = 0

or

  a_1 u_t + a_2 u_tt + ... + b_1 u_x + b_2 u_xx + ... = f(x,t)

instead. We could set f(x,t) to a constant, too. The plot should be some sort of 2D plot. Maybe with
x going from left to right and t going from front to back such that the front represents the state 
"now" and the back represents the initial condition. Maybe there could also be a time slider that 
sets up, what "now" means such that the plot always shows the entire history up to now in a sort of
waterfall plot. Maybe the app could be called "PDE Explorer"

  
Question:

How is the Fourier trafo used to solve PDEs vs ODEs? In the book "Harmonic Analysis" pg 28 ff, the
authors apply a spatial FT to the PDE (heat, wave, Schroedinger, etc.) to obtain an ODE in time 
only. I think, when applied to an ODE, we get an algebraic equation? What's the general pattern?
Each FT reduces the dimensionality of the DE by 1? And a 1D DE, i.e. and ODE, therefore becomes
a algebraic equation, i.e. no "differential" stuff going on anymore.


Top 23 Differential Equations in Mathematical Physics
https://www.youtube.com/watch?v=tOC8vfVAnus
