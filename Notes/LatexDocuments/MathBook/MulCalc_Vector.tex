...
\end{pmatrix} 
\end{equation}
We introduced two possible notations on the left hand side at once: one using $\grad$ and one using the nabla-operator $\nabla$. We also introduced two possible notations on the right hand side: one using the "partial" symbol $\partial$ and another one using subscripts. You will find both and possibly more in the literature. 
