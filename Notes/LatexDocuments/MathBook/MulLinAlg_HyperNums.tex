...
% Maybe that section should go into the "Algebras" section in the "Abstract Algebra" chapter. Or maybe keep both sections but look at different aspects in both. But nah!
...
% https://www.youtube.com/watch?v=hZrBGiWkUyk

% A Delicate Fit: Verifying that the Triakis Tetrahedron is Rupert
% https://www.youtube.com/watch?v=jDTPBdxmxKw
% -Has at ~4:30 a formula to convert from quaternion to rotation matrix
...
what is the complex numbers for 3 dimensions?
https://www.youtube.com/watch?v=ZKZLS0uoW_o
-Explains why a 3D number system is impossible. At 3:12: 
 Assume a,b,c are all 3D numbers of the general form x + y*i + z*j for two different imaginary 
 units i and j where x,y,z are all real numbers. 
 Now require associativity, i.e. the products (a*b)*c = a*(b*c) must be the same for
 all possible a,b,c. Take a=b=i, c=j. We thus require (i*i)*j = i*(i*j). The requirement simplifies
 to -j = i*(i*j). Let's write ij for i*j. The problem is now to define what ij need to be. Let's 
 assume ij can be expressed in the general for x + y*i+ i*j and lets write that as x + yi + zj. So,
 we have -j = i(x+yi+zj). Now comes a step where the video simplifies this to:
 -j = xi + yi^2 + zij where the creator apparently uses commutativity of multiplication without 
 explicitly mentioning it. But let's go along with it. After a bit of algebra, the equation 
 simplifies to -j = (xz-y) + (x+yz)i + z^2 * j. This requires that (xz-y) = 0, (x+yz) = 0, z^2 = -1.
 But the last z^2 = -1 is impossible because z is assumed to be a real number.
