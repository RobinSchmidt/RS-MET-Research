\section{Analytic Number Theory}


\subsection{Euler's Product Formula}
Let $s$ be a complex number with a real part strictly greater than one. Then the following sum and product will converge to the same value:
\begin{equation}
\sum_{n=1}^{\infty} \frac{1}{n^s} = \prod_{p \in \mathbb{P}} \frac{1}{1 - \frac{1}{p^s}}
\end{equation}
This is a pretty stunning result and it is known as the Euler product formula. We need the $\Re(s) > 1$ condition for the sum to converge. For the product to converge, we could be a bit more liberal and just require $\Re(s) > 0$ [VERIFY]. What makes this formula so interesting is that on the left hand side, we have a sum over \emph{all} integers whereas on the right hand side, we have a product involving \emph{only} the primes. Thus, this formula establishes a relationship between a very simple and boring sequence of numbers which we fully understand (namely $1,2,3,4,5,6\ldots$) and a very mysterious sequence of numbers (namely the sequence of primes $2,3,5,7,11,13,\ldots$) that we are yet trying to understand. Thereby the formula establishes a nice starting point for investigations of the sequence of primes.

% A crazy result using Euler's Product Formula
% https://www.youtube.com/watch?v=tSfNNXCBGrs

% The Riemann Hypothesis (Christmas Lecture 2016) [English subtitles]
% https://www.youtube.com/watch?v=sZhl6PyTflw
% bei 1:04

\subsection{Riemann's Zeta Function}


% How The Riemann Zeta Function Encodes The Prime Numbers
% https://www.youtube.com/watch?v=9vs0OpmcjXY

% The Most Mysterious Pattern Nobody Can Prove
% https://www.youtube.com/watch?v=pyEGbhwYOeo

% What does Riemann Zeta have to do with Brownian Motion?
% https://www.youtube.com/watch?v=YTQKbgxbtiw

% Functional Equation for The Riemann Zeta Function
% https://www.youtube.com/watch?v=xR-kcOyV0m8

% https://pub.math.leidenuniv.nl/~evertsejh/ant13-9.pdf
% -Derivation of the functional equation of the zeta function

% https://hapax.github.io/mathematics/zeta/

% Why Does Pi Show Up in a Counting Problem?
% https://www.youtube.com/watch?v=CzfsdMNsBNw

% Math is BROKEN: 1x2x3x...= sqrt(2 pi)
% https://www.youtube.com/shorts/5wowkaS-vcM
% -Assigns a finite value to this diverging product via the Zeta function

% Fourier Transform Masterclass: FT, DFT, FFT step by step Derivation
% https://www.youtube.com/watch?v=BHjkSjpB82g
% -Derives the functional equation for the zeta function (after first deriving FT, DFT, FT)
%  from the Poisson Summation formula

% The Million Dollar Equation No One Can Solve
% https://www.youtube.com/watch?v=Co3nEKnIuwA

\subsection{$L$-Functions}

\subsection{Langlands Correspondence}

% Revolutionary Math Proof No One Could Explain...Until Now
% https://www.youtube.com/watch?v=RX1tZv_Nv4Y

\subsubsection{Modular Forms}




% Should go into its own file:

\section{Additive Number Theory}

% https://en.wikipedia.org/wiki/Additive_number_theory

\subsection{Integer Partitions}

%https://en.wikipedia.org/wiki/Rogers%E2%80%93Ramanujan_identities


% My favorite Euler proof
% https://www.youtube.com/watch?v=vgiDQIXpXis
% - Proof that the number of odd partitions (in which only odd numbers appear) equals the number of
%   distinct partitions (in which no number appears more than once). 

\begin{comment}

A crazy result using Euler's Product Formula
https://www.youtube.com/watch?v=tSfNNXCBGrs

\end{comment}