...
% TODO: What a about the number 0? It cannot be expressed as such a product. Maybe that's why number theorists tend to not view zero as a natural number (whereas set theorists do)? Figure that out and maybe add a note about that.
...
% results! If in doubt, I recommend to implement and use a custom mod(a,b) or modulo(a,b) function
% and unit test that with all possible cases, i.e. a and b positive or negative or zero.
...
%
% I think, we also have:  d|(a+b)  iff  d | ((a%d) + (b%d))  where % is modulo (aka remainder)
% and d | (a*b)  iff  d|a  or  d|b  where the "or" is, of course, an inclusive or
...
% whenever gcd(m,n) = 1, i.e. whenever m and n are coprime. If the relation holds even when m,n
% are coprime, the function is called completely or totally multiplicative. See:
% https://en.wikipedia.org/wiki/Multiplicative_function

% What about a property like f(m*n) = f(m) + f(n)? That would behave like logarithms. The
% "number-of-factors" function would behave like that when factors are counted with their 
% multiplicity.
...
-The term "square-free" appears quite a lot all over math - in number theory but also in the theory
 ...
 etc. terms. But they are meant as well. ...I think...verify! This applies also to polynomials.
