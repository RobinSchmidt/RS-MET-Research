...
% Does it make sense to consider something like "modular rationals"? Maybe we could take modular
% integers with a non-prime modulus, such that we don't get a field and then consider pairs of them
% in a similar way as we do with rational numbers?
...
% The Day Prime Numbers Broke
% https://www.youtube.com/watch?v=EV-v-9KbJ7Y
...
% ToDo: Maybe dwell a bit more about the idea of (finitely) zero-extending a number leftward,
% Motivate writing down the leading zeros by the following observations from real life:
% We allow ourselves to imagine trailing zeros. But leading zeros? Well, in some contexts - for example - to achieve proper alignment of rightmost digits in a column of numbers, we allow ourselves to write down a finite number of leading zeros. For example, when we have a bunch numbers with up to 3 digits like 253, 745, 34, 896, 24, 7, ... we could write them as 253, 745, 034, 896, 024, 007, ... This helps also to have them be sorted lexicographically correctly in the sense that lexicographic order should match the order of the numbers - for example in naming conventions for files. Or in dates - we may write 2026/07/05 for the 5th of June in 2026. Note the leading zero in 07 and 05. So, indeed, a finite number of leading zeros is a thing in certain contexts. ...maybe give as other example: MyFile_version001.txt, ..., MyFile_version042.txt, ..., MyFile_version137.txt,  ...so, writing down (at least a finite number of) leading zeros is actually not totally unheard of. Just like with the trailing zeros, we now just imagine this to go on forever. After all, we do not actually write down an infinite amount of trailing zeros either - in practice, it's also a finite number - but we nevertheless think of it as being continued infinitely.
...
% ToDo:
%
% - Maybe start form the definition of real numbers as equivalence classes of sequences of rational
%   numbers
%
% - From this setting, give a couple of examples for how a real number (maybe take pi as example) can
%   represented using various different sequences that converge to pi. Of special interest among 
%   those are: the sequence of convergents of a continuous fraction representation (see
%   https://oeis.org/A002485  https://oeis.org/A002486), 
%   (3/1,31/10,314/100,3141/1000,31415/10000,...), 
%
% - Note that the second one results from a general decimal expansion of a real number x of the form:
%   x = sum_k c_k 10^k  where  k can, in general, range over all integers (the values c_k for k > 1
%   are zero here, though). 
%
% - Mention that we can use any base b instead of 10. In general, we can represent real numbers by a
%   weighted geometric series:
%   x = sum_k c_k b^k  where the weights c_k are in 0,1,2,...,b-1
%   If we assume that after some cutoff point k_max all coeffs c_k are zero, we get a convergent
%   series. Note that we allow k to be positive or negative here. Using such a weighted geometric 
%   series is just a special choice for a representative for the equivalence class of convergent 
%   rational sequences.
%
% - Representing real numbers like this actually aready seems to be the p-adic representation? Is
%   the only difference between a p-adic number and a base-b representation of the number the
%   definition of distance between two numbers?
%
% - The book "Zahlen" (Ebbinghaus) also mentions that such a base b representation is analoguous to
%   a Laurent series representation of a function as:
%   f(x) = sum_k c_k (x-a)^k  
%   where the difference (x-a) between x and the expansion point a takes the role of the base b. The
%   book doesn't really say it like this (it explains it much more abstractly) but I think, this is
%   what the analogy is about.
...
% -About q-derivatives. Maybe it should be put in the section about derivatives.


% Breaking the Laws of Math
% https://www.youtube.com/watch?v=Is7y7cvTh2s
% -About a modified commutative property that says:  x y = q y x.
% -Maybe we should put it into the Ring Theory chapter. Or maybe Group Theory. The q seems to be
%  something like a "multiplicative commutator" or something? I made that term up.
% -It seems to have a relation to q-integers, q-derivatives, etc.

% See also:
% https://github.com/JuliaSymbolics/SymbolicUtils.jl/issues/96
% https://arxiv.org/pdf/1202.2264  NON-COMMUTATIVE q-BINOMIAL FORMULA
...
When the modulus m is prime, we get a finite field. When the modulus is not prime, could it make sense to consider pairs of these modular integers and somehow interpret them like "modular rationals"? Try it!

How about defining algebras over vectors of modular integers, e.g. algebras over finite fields. Take the complex numbers (or quaternions, octonions, etc.) as example. Instead of letting the components be real numbers, use modular integers. Does that give something interesting? Maybe the norm (if needed) should be defined without invoking a square root - like in the Gaussian integers.
