...
% graph theory: studies connectivity in networks. It's related to topology. It can be thought
% as a discrete version of it. Topology is basically about connectedness of points that live in a
% continuous space. In graph theory, we care about connecteness of a set of discrete nodes.

% Maybe geometry should also be consideredpart of the "trunk"
...
% https://www.youtube.com/watch?v=4Z7HXiNywtU
...
% It's a bird's eyes overview and also some sort of personal knowledge management (PKM) tool for myself

% It's a living document that probably will never be finished
...
% Make a paragraph about crafting definitions - and explain why they often look so complicated and
% abstract (and sometimes unmotivated/arbitrary). They always try to capture the essence of some 
% very much non-arbitrary idea in the most general way possible - making it so general often leads 
% to these cumbersome formulations.


% Zeichenhandeln im Raum: Dimensionen mathematischer Notation
% https://www.youtube.com/watch?v=Mg0HlRb0Mrk
% -1:13:26: A matrix is formally a map from a tuple (i,j) of natural numbers (within certain ranges)
%  to a (typically real or complex number). In terms of sets, it's a subset of (N x N) x R where x
%  is the set product 

% John von Neumann: In mathematics, you don't understand things. You just get used to them.

% Richard Feynman: If you think, you understand quantum mechanics, then you don't understand it.
% (or something along these lines)

% Maybe mention that this book will probably never be finished. It's an ongoin work-in-progress.
% Maybe mention that some more notes and comments can be found in the LaTeX source and link to it.

Maybe explain the various activities in math research:
-proving conjectures to turn them into theorems
-finding new proofs for existing theorems to potentially get new insights, simplify proofs
-coming up with new conjectures - involves spotting patterns in data, 
 ex: Conservative matrix field - could even be seen as new paradigm?
-generalizing existing theorems (example: generalized Stokes theorem)
-tightening existing theorems (when they involve inequalities like upper bounds)
-finding bridges between disparate math fields (Langlands program?)
-finding computational shortcuts and better (e.g. more accurate) algorithms for computations
 examples: FFT, Karatsuba multiplication
-Finding entirely new paradigms. Founding entirely new subfields of math.
 examples: Calculus (Newton, Leibniz), Graph Theory (Euler), Set Theory (Cantor), Logic (Boole?),
 Complex Numbers (Bombelli, Cardano, Descartes, Tartaglia, Euler, ...), Conservative matrix field?

Maybe mention different usages of pi:
-circle constant, prime counting function, permutation, fundamental group
