...
% I think, the probability function is also known as probability measure or probability 
% distribution (or are there subtle differences? Why are there different words?):
...
%As a motivating example, let's assume that I roll a fair die but don't tell you the result. As far as you know, the result could be any number between 1 and 6 and each outcome has equal probability. If I now tell you that the result was even, I have given you a bit\footnote{I intend to mean the word \emph{bit} here in an informal, colloquial sense but coincidentally, is indeed one bit in the information-theoretical sense of the word which we will define later.} of information such that you can narrow down the possible outcomes to $\{2,4,6\}$. If I now also tell you that the result was greater than $3$, I have given you another bit of information that let's you narrow down the result further to $\{4,6\}$. If I finally tell you that the result is less than $6$, I have given you enough information to infer that the result must have been $4$. We want to construct a measure of information that has the property that the sum of the 3 individual pieces of information given (even, $> 3$, $< 6$) comes out the same as the information content of directly telling you that the result was $4$. [VERIFY! Calculate the example through!] ...TBC...


% maybe use "chunk" or "piece" instead of "bit"
...
% Example: 
% - Example: Roll a die and assume the result the result is 4. If I tell you that the result is even,
%   I have gievn you some bit of information. If I also tell you, the result is greater than 2, I've
%   given you another bit of information. If I also tell you that the result is less than 6, I have
%   given you enough information such that you can uniquely identify the result 4.
% 

% - We want to construct a measure that tells us, how informative a given event A is. Let's call 
%   that measure "information" I(A). We want it to satisfy the following requirements:
% - I(A) = 0  if  P(A) = 1  -> a certain event has no information
% - if P(A) < P(B) then I(A) > I(B)  -> an unlikely event has more information than a likely one
% - if A and B are independent and C = "A and B" then I(C) = I(A) + I(B)
% 
% Note also:
% - An event that has zero probability has infinite information. Note that zero-probability does not
%   necesarrily mean "impossible" when we deal with continuous random variables. If we throw a dart
%   at a board to hit any particular point is actually zero (for all point) - nevertheless, the dart
%   must end up somewhere.

% Why Entropy isn't Mysterious
% https://www.youtube.com/watch?v=_ZfPDjNTcLc



% Maybe it's more natural to define the information of an event with probability p as 
% I = log2(1/p) rather than I = -log2(p). The result is the same, but the minus is somehow
% weird from an aesthetic perspective. 3b1b says that viewing it as log_{1/2}(p) is also a nice way
% to frame it.
%
% https://www.youtube.com/watch?v=l6DKRf-fAAM  at around 15:40
% https://www.youtube.com/watch?v=GlYgs6v2YfU

\paragraph{Entropy}

% -Entropy is the expectation value (or mean) of the information
% -It's a property of the random variable itself (not of individual realizations of it). More 
%  precisely, it's a property of the probability distribution that governs the random variable.
% -Maybe use a fair and an unfair die as example to eplain the concept. Maybe the unfair die could have
%  P(x = 1) = 0.5, P(x=2) = ... = P(x=6) = 1/10. Maybe use also the exreme case where P(1) = 1.
...
% The 12 Coin Puzzle, Deconstructed #SoME4
% https://www.youtube.com/watch?v=tDfgBGFKZIQ
...
% A geometric approach to Benford's Law
% https://www.youtube.com/watch?v=vr8z-Wk45m0
% -p(d) = log10((d+1)/d)  where d ist the leading digit (i.e. in 0..9) and p(d) is its probability
...
% https://www.youtube.com/watch?v=MNrn_Knx8qI
% -Derives normal distribution from the binomial distribution

% 2D Normal Distributions
% https://www.youtube.com/watch?v=JOhoWfGWIk8
...
% https://www.youtube.com/watch?v=x8qzeyaaWwA
...
% https://www.youtube.com/watch?v=IICU2KloP-8


% What about a 1/cosh(x) distribution (appropriately scaled). It should have fatter tails than
% the Gaussian because they fall off only exponentially (i.e. like exp(x) rather than like exp(x^2))
% and is still nicely smooth
...
% https://www.youtube.com/watch?v=PBVfkJccqqs

% https://en.wikipedia.org/wiki/Median#Probability_distributions

% https://math.stackexchange.com/questions/924776/finding-the-mean-and-median-of-a-probability-density-function
% The median of a probability density function f(x) with cumulative probability density F(x) is the value of x such that F(x) = 1/2
% -> this definition works only for normalized probability denisties f(x). for a general function f(x), I think maybe the definition given in Micheal Penn's video above is the appropriate generalization (verify!)
...
% ToDo: introduce alo different means: geometric, harmonic, generalized, arithmetic-geometric, circular, etc.

% the Stolarsky mean
% https://www.youtube.com/watch?v=830nq6j6a-E
% https://en.wikipedia.org/wiki/Stolarsky_mean
% https://de.wikipedia.org/wiki/Stolarsky-Mittel

% https://en.wikipedia.org/wiki/Logarithmic_mean
% https://de.wikipedia.org/wiki/Logarithmisches_Mittel
...
% Circular Statistics - Nour Malek and Frederic Simard
% https://www.youtube.com/watch?v=rwFEQklcJvw
...
% https://www.youtube.com/watch?v=dy6W7tm2-UI
% - Mean: Center, Variance: spread, skew: asymmetry, kurtosis: tail thickness
...
%---------------------------------------------------------------------------------------------------
\subsubsection{Parameters of Pairs of Distributions}
% Rename this. Maybe Mesurements on Distribution Pairs
...
% https://www.youtube.com/watch?v=GlYgs6v2YfU  around 31:35
...
% List operations here that take distributions as inputs and produce new distributions
...
% ToDo: KL-Divergence, Cross Entropy
...
- The probability that at least one key is the correct one if you have a bucket of infinitely many
  keys for infinitely many doors and try a random key for each door is 1-1/e ~= 63%
...
https://www.youtube.com/watch?v=zIyMz5YUdcY

What are Sigma-Algebras? And Why Do We Need Them?
https://www.youtube.com/watch?v=PegyQkhbh0c


Don't Solve Stochastic Differential Equations 
https://www.youtube.com/shorts/OUGDlKnRLOI


Probability Theory
https://www.youtube.com/playlist?list=PLZODkMAJsE2jIRT4R6moAv6Xofid1wQpj

Das Paradoxon, das deine Intuition zerstört
https://www.youtube.com/watch?v=2mlIQEtSXd0
-About the birthday paradox

Probability is not Likelihood
https://www.youtube.com/watch?v=zgir07qRdGw

Probability Theory is an Extension of Logic
https://www.youtube.com/watch?v=0yF9TvMeAzM

Math Says "Always Take This Bet"
https://www.youtube.com/watch?v=oXtBcvOvXDI

What even IS probability? #SoME5
https://www.youtube.com/watch?v=eBpp6yXX6gU
