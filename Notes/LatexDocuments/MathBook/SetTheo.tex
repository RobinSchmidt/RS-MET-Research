...
% -explain notation - hwy lowercase x,y,z etc. for sets. Looks better in logic formulas
%  ...but A,B,C looks better elsewhere and is more consistent with other math fields. Do we
%  want consistency with prdicate logic or with general math?

% https://en.wikipedia.org/wiki/Universal_set


\subsection{Unrestricted Comprehension Axiom [VERIFY!]}
In naive set theory, there is one simple and very general axiom that allows us to construct sets. Let $x$ be any kind of object and $\varphi(x)$ a predicate, i.e. a function into which we can plug in $x$ and then $\varphi$ will return a boolean value that tells us whether or not $x$ has some property. With $\varphi$ in hand, we can build the set $S = \{x: \varphi(x)\}$ of all objects that satisfy our predicate. That we can always do this, for any $\varphi$ whatsoever, is called the \emph{unrestricted comprehension} axiom, sometimes also \emph{naive comprehension} axiom. 
...
The most common system of axioms that mathematicians use today is the one proposed by Zermelo and Frankel, usually augmented with the so called axiom of choice. This system of axioms is usually called the ZFC system (for Zermelo, Fraenkel, Choice). There are multiple equivalent ways to choose the set of axioms for ZFC and some of them may be redundant in the sense that they contain axioms that are not strictly necessary, i.e. could be proved as theorems from some of the other axioms. If these redundant axioms are present, then it is because they make the whole thing more intuitive and convenient [VERIFY!]
...
This axiom states that whenever we have two sets $x$ and $y$, there is a set $z$ whose elements are precisely $x$ and $y$. This set is given by $z = \{x, y\}$ and called the \emph{pair} of $x$ and $y$. We can write it formally as:
\begin{equation}
...
\exists x \forall y: \neg (y \in x)
\end{equation}
In any set theory which has the axiom schema of separation and axiomatically postulates existence of any set, the existence of the empty set is a theorem and therefore doesn't need to be an axiom. In ZFC, we do indeed have the axiom schema of specification and we also have the axiom of infinity, so the existence of the empty set is actually a theorem in ZFC (VERIFY!). The empty is unique due to the axiom of extensionality. It is usually denoted by $\emptyset$ or (less often) by $\{ \}$.
