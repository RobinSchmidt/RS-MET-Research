...
% It's also messed up wrt to the headline levels. Look at the document structure in TeXStudio - the section headlines are at the level where normally the chapters belong - and the "Set Theory" headline is missing altogether
...
This axiom states that whenever we have two sets $A$ and $B$, there is a set $C$ whose elements are precisely $A$ and $B$. This set is given by $C = \{A, B\}$ and called the pair of $A$ and $B$. It is unique by virtue of the axiom of extensionality. The axiom of pairing also allows us to build singletons by simply letting $A = B$ such that  $C = \{A, B\} = \{A, A\} = \{A\}$. It also lets us build the ordered pair $(A,B) = \{ \{A\}, \{A, B\} \}$ and from ordered pairs we can recursively build ordered $n$-tuples as $(A_1, A_2, \ldots, A_n) = ((A_1, A_2, \ldots, A_{n-1}), A_n)$.
