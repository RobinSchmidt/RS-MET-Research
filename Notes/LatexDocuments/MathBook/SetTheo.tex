...
% Maybe sort the paragraphs: succ, order, max, add, mul, exp

\paragraph{Successor}
The simplest operation that we want to define is the successor function $s(x)$. Here $s$ denotes a function or unary operation and $x$ denotes a set constructed like above. We then define: $s(x) = x \cup \{x\}$. The successor of a set $x$ is just that same set $x$ with $x$ itself add to it. For example, we have $3 = s(2) = s(\{0,1\}) = \{0,1\} \cup \{2\} = \{0,1,2\}$. With that construction, each natural number $x$ is just the set of all natural numbers that are less than $x$. [TODO: Explain how we could define a predecessor function - we could do it by extracting the maximum as explained below.]

\paragraph{Addition}
We define addition of two natural numbers $x,y$ recursively in terms of the successor function $s(x)$ as follows: $x + 0 = x$, $x + s(y) = s(x + y)$. The first formula says that when the right operand is $0$, the result is just the left operand. The second formula says that when the right operand is not zero, we should consider it as the successor of some other number $y$\footnote{For an actual implementation, we would have to operationalize the "consider it as the successor of" somehow to figure out, what that $y$ is. This would have to be done in terms of a predecessor function.}. The right hand side of the formula then says, we should apply addition to $x$ and $y$ and then take the successor of that. If we apply this rule recursively, we will eventually end up in the base case $x + 0 = x$ which we can then use directly. 

\paragraph{Multiplication}
To define multiplication, we use the rules: $x \cdot 0 = 0$, $x \cdot s(y) = (x \cdot y) + x$. This is again a recursive definition that at some point will hit the base case. 

...TBC...(look it up in the implementation)
...
\end{equation}
Later, we will generalize this to infinite sets of natural numbers. In the (possibly) infinite case, we'll have to replace the maximum with the supremum [VERIFY!]. Note also that with the so defined maximum operation, we actually do have a way to implement the predecessor function on our Neumann numbers.
