...
As we have already mentioned we define: $s(x) = x \cup \{x\}$. The successor of a set $x$ is just that same set $x$ with $x$ itself add to it s new element. For example, we have $3 = s(2) = s(\{0,1\}) = \{0,1\} \cup \{2\} = \{0,1,2\}$. Instead of $s(x)$, we may also write $x+1$ when this is more convenient. So, we define:
\begin{equation}
x + 1 = x \cup \{x\}
\end{equation}
...
\begin{equation}
x < y \Leftrightarrow x \in y \Leftrightarrow x \subset y
\end{equation}
