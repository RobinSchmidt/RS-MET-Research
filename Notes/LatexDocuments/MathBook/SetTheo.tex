...
\medskip
We now formalize these ideas as follows: We want to define a new kind of "number" that measures the size of sets. We want the class\footnote{It turns out that the cardinal numbers do not fit into a set. Just like the ordinals, they form a proper class. In a sense, there are "too many" of them. } of these new numbers to be comparable in the sense of a strict total order. That means, we want establish an equivalence relation and a strict total order relation on the class of these numbers.  ...TBC...

\paragraph{The Aleph Function}
[!!!VERIFY!!! I made some stuff up according to my own interpretation]
What we eventually want is a function into which we can plug in an arbitrary set $A$. This function should produce as output an object that is size-comparable according to a suitable order relation. We will denote this function as $|A|$ like an absolute value. To construct the function, we first apply a suitable well-ordering function $W(A)$ to it to obtain an ordinal number $\alpha = W(A)$. According to the well ordering theorem, such a function exists for any set whatsoever. To this so found ordinal $\alpha$, we now apply a function that we denote by the Hebrew letter "aleph" to compute $\aleph_{\alpha}$. That aleph function produces another ordinal number. That is: aleph is a function from the ordinals to the ordinals. It's input can be any ordinal. Its output will be specific ordinals - namely, for any ordinal $\alpha$, our $\aleph$ function will spit out the next smaller limit ordinal....TBC...and VERIFY!!!
