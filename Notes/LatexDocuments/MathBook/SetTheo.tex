...
% Cardinals: one, two, three, four, ......, infinity?
% Ordinals:  first, second, third, fourth, ...., last?
%
% Counting (i.e. measuring "size") vs ranking (i.e. designating "position")
%
% In the finite case, we can use the same kind of numbers, namely natural numbers, to do both.
% The distinction between these two fundamentally different functions of numbers becomes very 
% important in the infinite case. Counting and ranking behave quite differently in the infinite 
% case. Ranking is generally more complicated. There is such a thing as "the first place after 
% infinity" but there is no such thing as "one more than infinity" - this is still just infinity.
% Adding one to "counting infinity" changes nothing but adding one to "ranking infinity" does 
% indeed designate a different rank, namely one position higher. This difference is captured in the
% differences between cardinal and ordinal arithmetic (addition in this case). In the finite case
% "counting arithmetic" and "ranking arithmetic" are the same thing. In the infinite case, they are
% not.
...
% See "RS-MET-Research\Notes\LatexDocuments\MathBook\Scratch\SetTheory.txt" for some more 
% elaboration on the recursive implementation. It uses the language of formal specification, i.e.
% specifying the type set using a signature and implementing the operations in an algebra
...
% The Most Mind-Bending Paradoxes in Math and Physics
% https://www.youtube.com/watch?v=yrJA0J7jciY
% -Explains Russel's paradox in terms of a barber: the barber shaves everyone who doesen't shave
%  themselves. Does the barber shave himself?

% Der Streit der Mathematiker (Kurze Geschichte der Mathematik 4)
% https://www.youtube.com/watch?v=BSJRyo-A0Rw
% -Explains Russel's paradox by a nice example at 1:27:36. It's about a book that is a catalog of all
%  books that are non self-referential. Does the catalog itself belong into the catalog?
...
% The Paradox That Broke Mathematics
% https://www.youtube.com/watch?v=aRTQTTbdINY
...
\subsubsection{Axioms of Zermelo and Fraenkel (ZF)}
The most common system of axioms that mathematicians use today is the one proposed by Zermelo and Fraenkel and it is abbreviated by ZF. It is usually augmented with the axiom of choice in which case the whole axiomatic system is abbreviated as ZFC. There are multiple equivalent ways to choose the set of axioms for ZF and some of them may be redundant in the sense that they contain axioms that are not strictly necessary, i.e. could be proved as theorems from some of the other axioms. If redundant axioms are present, then their purpose make the system more intuitive and convenient [VERIFY!].
...
% ToDo: Maybe order them differently - in roughly descending order of obviousness:
% 1: empty set, 2: extensionality, 3: pairing, 4: union, 5: power set, 6: specification/subset, 
% 7: replacement, 8: regularity, 9: infinity
% I think, 2 (extensionality) is not really an axiom but rather a definition. We _define_ two sets 
% to be equal, if they have the same elements. i.e. sets a and B are equal if and only if every a in
% A is also an element of B and every b in B is also an element of A. That's the _definition_ of 
% what we mean when we say that two sets are equal.


\paragraph{A1: Axiom of Empty Set}
...
\paragraph{A2: Axiom of Extensionality}
...
\paragraph{A5: Axiom of Regularity}
...
\paragraph{A6: Axiom of Power Set}
...
% aka restricted comprehension - could also be called filtering axiom or axiom of subset formation
...
\paragraph{A9: Axiom Schema of Replacement} The axiom schema of replacement says that the image of a set under any definable mapping is again a set. [...TBC...explain what "mapping" means in this context. It's a formal term. The formal statement of this axiom is complicated]

% https://en.wikipedia.org/wiki/Axiom_schema_of_replacement
% https://en.wikipedia.org/wiki/Functional_predicate

% Maybe make that a paragraph A10: Axiom of Choice and rename the subsubsection: ZFC: Zermelo, Fraenkel, Choice
...
\subsubsection{Macro Construction Steps}
%\subsubsection{Higher Level Construction Steps}
The axioms give us some ways to create new sets from already existing sets by way of just postulating that these are valid ways to create new sets. Using just these construction steps as primitive operations, we can combine them in certain ways to assemble them into some important higher level construction steps to create sets with a certain structure. We assemble the micro-steps that are postulated as valid via the axioms into some sort of macro-steps, so to speak. The most important specimen of these macro steps are the following: It was already mentioned that the axiom of pairing allows us also to construct singleton sets as a special case. The ability to create singletons, i.e. to wrap a given set into another pair of curly braces, will become important in the definition of natural numbers where will need it, together with the axiom of union, to define the successor function. In an earlier chapter (see page \pageref{Sec:KuratowskiPair}), we have also seen that this pairing allows us to construct sets that we can interpret as \emph{ordered pairs} or 2-tuples. Such 2-tuples are the basis to construct the set product $A \times B$ of two sets $A$ and $B$. This product is just defined as the set of all possible ordered pairs $(a,b)$ where $a \in A$ and $b \in B$. Starting form this set product, we can invoke the axiom of specification to filter out pairs that satisfy a particular condition that is of interest to us. When we do this, we create a subset of $A \times B$ which defines a \emph{relation} between $A$ and $B$. Some particular relations are of special interest. One of those are \emph{functions}. With the ability to construct functions, we can also construct the set of all possible functions from a set $A$ to a set $B$ which we denote by $B^A$. Another important type of relations are \emph{order} relations which, together with tuple-formation, allow us to construct ordered sets\footnote{An ordered set is modeled as tuple $(A,<)$ with a set $A$ and a $<$ relation defined on it.}. Yet another important type of relations are \emph{equivalence} relations which, in turn, allow us to construct \emph{equivalence classes} and \emph{quotient sets} where the latter is the set of all equivalence classes. This construction step will be important in constructing the integers from the naturals and the rationals from the integers. The set product (i.e. tuple formation) together with forming a union can be used to construct the so called \emph{disjoint union} which will be important in the definition of ordinal sums. As we can see, from these few axiomatically postulated primitive construction steps, we can create quite an arsenal of higher level construction steps which we can use to build various sorts of sets that have a particular inner structure which makes them well suited to represent all kinds of mathematical objects that are of interest to us: numbers, tuples, functions, shapes, spaces, etc. - all the good stuff that we normally associate with math on an applied level.

% By iterating the product, we can create arbitrary $n$-tuples
% Using our mechanism to form equivalence classes, we can abstract away unimportant strutural 
% details of classes of sets by considering them equivalent whenever we can define appropriately 
% defined isomorphisms. Isomorphims are bijective functions which must sastisfy certain additional 
% constraints that are typically expressed in terms of preserving relations: For a function 
% f: A -> B  if  a1 R_A a2 and b1 = f(a1), b2 = f(a2) then the function must be such that 
% b_1 R_B b_2 where a_1,a_2 in A, b_1,b_2 in B  and R_A, R_B are relations of A and B. For example, 
% order isomorphisms must preserve the order of elements: if a_1 < a_2 the we must always have 
% f(a_1) < f(a_2) for f to qualify as an order ismorphism. In abstarct algebra, there are also other 
% kinds of isomorphisms such as group- and ring isomophisms which must preserve binary operations.

% pair/doubleton, singleton, ordered pair (Kuratowski), set-products A x B, relations, functions,
% set-exponentiaion A^B, orders, equivalences, equivalence classes / quotient sets.

% orders, functions and equivalence relations are formed by using the set product together with
% the axiom of specification

% We will also need disjoint unions later to define ordinal addition. These can be formed using a
% product together with a union

% Set intersections can also be 
% defined using the axiom of specification: A \cup B = { x \in A : x \in B }. Likewise, the set 
% difference can be defined via specification: A \setminus B = { x \in A : x \notin B }
...
% TODO: Verify if "chain of subset inclusions" is an official term. I think, the term "chain" is used in set theory and has indeed this intended meaning. Verify that. If true, mention it.
...
% TODO: Maybe include a reference to the definition of big unions in the introductory chapter

% In the section about naive set theory, I mention in a comment how to handle the edge case of an 
% empty big product. I'm not sure where to fit in that info best in the book. Here or there?
...
% Maybe we can define an alternative to "axiomatic set theory" which we could perhaps call 
% "algorithmic set theory" in which we pose that a set A exists if there is a finite algorithm 
% (as in: can be expressed with a finite amount of code) that can decide in finite time if any given
% (finite?) set x is or isn't an element of A? The algorithm would supposedly do some sort of 
% pattern matching. It could, for example, analyze the structure of x to determine whether or not it
% is a von Neumann natural (or integer or rational). Maybe it would boil down to analyze the 
% structure of graphs. Maybe in such a context, arbitrary graphs could be seen as generalizations to
% sets? Sets are, I think, directed acyclic graphs (verify!).
...
% May be needed to understand the limit step in ordinal numbers and transfinite induction?
...
% ToDo: explain the term "partial order" and give examples. One example of a partial order that is
% not total is the "divides" relation on the natural numbers.

% https://en.wikipedia.org/wiki/Completeness_(order_theory)


% Maybe make this section about "Order Types" only and tal about partial, total and well orders and about
% distinguished elements (min, max, sup, inf, least, greatest, etc.). traet the isomorphisms as sub-point 
% in "Operations" on ordered sets. Basically, an isomorphism is a map from one ordered set to another, so 
...
% ordinallity = order type (but applies only to well orders)
...
% I think, ordinal numbers do _not_ provide an exhaustive ctatlog of all order types (i.e. one member per
% order type) but only for all the well orderings. Orders like those of the intgeres, rationals, etc. are 
% _not_ isomorphic to any ordinal number. Verify!
...
\subsection{Operations on Ordered Sets}


\subsubsection{Concatenated Orders}
%\subsubsection{Concatenating Orders and the Ordinal Sum}
%\subsubsection{Ordinal Sum}
Consider two disjoint totally ordered sets $A$ and $B$ and form their union $C = A \cup B$. ...TBC...

% https://www.youtube.com/watch?v=UxhFy4deLQA  at 30:50

% See also ABoST, pg 169 - ordinal sum

% Google AI overview for "ordinal sum"
%Ordinal sum is the addition of two well-ordered sets, where one set is placed entirely before the other to form a new, larger ordered set. For example, the ordinal sum of the set of natural numbers \(\{0,1,2,...\}\) and the set of three elements \(\{a,b,c\}\) would result in the ordered set \(\{0,1,2,...,a,b,c\}\). This operation is associative but not commutative, meaning the order of addition matters: \(3+\omega =\omega \), but \(\omega +3>\omega \)

% https://mathworld.wolfram.com/OrdinalAddition.html

% see ABoST, p87 for (anti)lexicogrpahical order and p.170 for ordinal product
\subsubsection{Lexicographical Orders}
%\subsubsection{Lexicographical Order and the Ordinal Product}
%\subsubsection{Lexicographic Order}
Consider the set product $C = A \times B$ of two totally ordered sets $A$ and $B$ where we have a strict order relation $<_A$ defined on $A$ and another one $<_B$ defined on $B$. ...TBC...

%We can now define an order relation $<_C$ on $C$. For clarity, I will now drop the subscript and will assume that it clear that when we compare elements form $A$, we mean $<_A$ so I'll just write $<$. We define $<$ for $C$ as follows: Let $c_1 = (a_1, b_1)$ and $c_2 = (a_2, b_2)$ be two elements of $C$. We will say that $c_1 < c_2$ if $a_1 < a_2$ and we will say that $c_2 < c_1$ if $a_2 < a_1$. In the case of equality of the first component, i.e. $a_1 = a_2$, we take the second component into account: If $b_1 < b_2$ then $c_1 < c_2$ and if $b_2 < b_1$ then $c_2 < c_1$. If we also have $b_1 = b_2$ in addition to $a_1 = a_2$ then we conclude that $c_1 = c_2$. We call such an order defined on the product set $C$ that was derived from the orders of the components a lexicographic order. We first compare the first component. Only if they happen to be equal, we compare the second component. In these comparisons, we use the respective orders defined on $A$ and $B$. The pair $(C,<_C)$ is also called the ordinal product of $(A,<_A)$ and $(B,<_B)$ VEERIFY!...TBC...mention also antilexicographic order

% Check if we really need to require total orders or if the defition also applies to posets. If so, state it in terms of posets because that's more general. ABoST says (p.87) that (anti)lexicographic ordering
% applies also to posets but for the ordinal product (p.169, Def. 9.6), it requires well ordered sets

% I think, for the concatenation and lexcograpühical otrdering themenselves, we do not really need total
% orders or even well orders. They can be defined for partilal orders as well. It's just that the order of
% the resulting set will then by the type of order of the least specific operand. If both operands are
% well-ordered, the result will also be. If one if the operands is a total order and the other is a only 
% partial order, the result will be only partially ordered, etc. BUT: to perform the additional step of 
% ismorphically mapping the result onto an ordinal number, we need both operands to be well ordered such
% that the result will also be well ordered because only in this case, we can find an order isomorphism to
% an ordinal numbers. Maybe make susubs for: order concatenation (+), lexicographical order (*) here and
% subsubs for ordinal sums + and ordinal products * in the section about ordinal numbers

% I think, if we would define the disjoint union of A,B as  (0 x A) \cup (1 x B) rather than 
% (A x 0) \cup (B x 1) theh we could use lexicographical rather than antilexicographical ordering

% Explain that this notion of lexicographical order is actually a bit different from the order in a dictionary because in a dcitionary, the words are allowed to have different lengths where as here, all "words" (i.e. 2-tuples) have the same "length", namely 2. I think, antilexicographical order is not the reverse of lexicogrpahic (as one might think) but rather lexicographic by looking at the 2nd component first, i.e. reading the "words" backwards. ...verify!

% Google AI overview for "ordinal product"
%Ordinal product of ordered sets Definition: Given two ordered sets (posets) \(A\) and \(B\), their ordinal product is a new poset denoted \(A\otimes B\).Elements: The elements of \(A\otimes B\) are the pairs \((x,y)\), where \(x\) is from \(A\) and \(y\) is from \(B\).Ordering: The order relation \(\le _{\otimes }\) is defined as follows: \((x,y)\le _{\otimes }(x^{\prime },y^{\prime })\) if and only if (\(x<x^{\prime }\)) or (\(x=x^{\prime }\) and \(y\le y^{\prime }\)).Analogy: This ordering is similar to a lexicographical order, where the first component (\(x\)) is compared first, and if they are equal, the second component (\(y\)) is compared
...
\subsubsection{Initial Segments} ...TBC...


\subsubsection{Cuts} ...TBC...


\subsection{Well Ordered Sets} A well ordered set is a set with a total order defined on it and the additional property that every non-empty subset has a least element with respect to the given order. The natural numbers $\mathbb{N}$ with our usual $\leq$ relation are such a well ordered set. 
...
In the chapter about logic, we have learned about the proof technique of induction. It is a technique to prove statements $\varphi(n)$ that should hold for any natural number $n$. The technique can be generalized to work not only over the set $\mathbb{N}$ but over any well ordered set - for example, over the sets\footnote{But wait: The ordinal and cardinal numbers are proper classes. Is there such a thing as a "well ordered class"? Figure out!} of all ordinal or all cardinal numbers which we will encounter later. The generalized technique is called \emph{transfinite induction}. The basic technique that works over $\mathbb{N}$ had to distinguish two cases: (1) the base case and (2) the successor case. In transfinite induction, we need to consider a third case: (3) the limit case. It is called by that name because... ...TBC...
...
In set theory, sets are the fundamental objects from which everything else is built up. Every mathematical object must somehow be encoded as some sort of set. To do all the cool math things that we love (or hate) so much, we obviously need numbers. So, we somehow need to construct sets that behave like numbers. Set theorists will sometimes casually talk about things like the subsets of a number. This will at first sound totally nonsensical - what the heck is a subset of $10$ or $3/7$ or $\pi$ supposed to mean? We are used to think of numbers as atomic entities. But in set theory, numbers indeed \emph{are} sets (and therefore can have subsets) and we need to get used to this point of view.
...
% Numbers from Naught
% https://www.youtube.com/watch?v=gAUc7nhqhrU
...
The most basic of all number systems is the system of natural numbers. We can construct the set of natural numbers from ZF as follows: Given any set $x$, we define the successor function as $s(x) = x \cup \{ x \}$. Note that to be able to do this, we had to use the axiom of union and the axiom of pairing (which implies the constructibility of singletons). Then we give the names $0,1,2,3,\ldots$ to the following sets:
...
% The set von Neumann natural numbers is well-ordered by the element relation. I think, it is also well-orderd by the subset relation. Verify!
...
% Mention that the so constructed Neumann numbers can serve as a sort of canonical/prototypical set
% that we can use to make size comparisons against using the criterion of being able to construct
% a bijection. The set 4 = { 0,1,2,3 } has cardinality 4 and can therefore be used to compare all
% sets against. By the way: We thereby establish an equivalence relation between all sets. We can
% define the idea of four as the set of all sets that have the same cardinalilty as the set 4. I'm
% not sure, if that's the way mathematiciancs think about it, though.
...
% - Maybe it makes sense to define the number 3 as the equivalence class of all sets that have 3 
%   elements and call the specific set with 3 elements that results from the von Neumann 
%   construction the "von Neumann representative" of this equivalence class?
...
% https://www.youtube.com/watch?v=NHZt8eBKcRA
% - We use sets to "encode" numbers. I usually say, we "model" numbers by sets - but "encoding"
%   may capture it better what is going on here.
...
% what is a real number exactly?
% https://www.youtube.com/watch?v=ASXe5boIC08
% -Mentions both ways to construct the reals (Dedekind cuts and Cauchy sequences) and their
%  respective emphasis on different aspects of the reals.
% -Defines the real valued exponential function a^x in terms of rational exponents as:
%  a^x = \sup \{ a^q : q \in Q, q <= x  \}
% -A real number is an element of a complete ordered field

% Maybe yet another way to define real numbers is as infinite decimal expansions (possibly with some
% extra care to make the representation of a number unique). But maybe that is actually a special
% case of the "Cauchy sequences" approach? A decimal expansion can be seen as a particular series
% expansion x = ... + a_2 10^2 + a_1 10^1 + a_0 10^0 + a_{-1} 10^{-1} + a_{-2} 10^{-2} + ...
% for a given sequence of a_k coefficients that extends from -inf to +inf.

% Norman Wildberger: The Problem with Infinity in Math
% https://www.youtube.com/watch?v=l7LvgvunVCM
% -Mentions that reals can be defined in various ways: Dedkind cuts, equivalence classes of Cauchy
%  sequences, contiued fractions and more. Historically, they were conceived as infinite decimals
%  which, I think, is a particular representative form the aforementioned equivalence class of
%  Cauchy sequences

\subsubsection{Complex Numbers} Having the real numbers in hand, constructing the complex numbers is actually very simple. We just define a complex number as a pair of real numbers. That's it. This time, we don't even need equivalence classes or infinite sets. Of course, we still need to define the operations of addition and multiplication. But that is also easy. Addition, once again, just works element wise.  ...TBC...ToDo: explain multiplication, explain that C is algebraically complete, cannot be ordered into an ordered field, maybe explain also subtraction and division
...
\end{equation}
The leftmost definition is the base case in various notations. The middle equation is the successor step. The $(\alpha+1)$th beth number is computed from the $\alpha$th beth number by taking its power set. Remember that numbers \emph{are} sets in the present context, so yes - we can indeed take the power set of a number, even if that feels nonsensical when one is not familiar with the set theoretical representation of numbers. VERIFY!...TBC...Explain limit step
...
% -Talks about arithmetic with Beth numbers
...
For finite sets, size comparisons are easy: We just count the number of elements, which results in a natural number, and whichever set has the greater count, has the greater size. Simple enough. But what if we can't quantify the size of a set with a natural number? The idea is to observe that for finite sets, two sets of equal size can always be put into a bijection with one another and vice versa. That means that for sets of equal size, we can always find a function that maps one set to the other bijectively. And, vice versa, if we can find such a bijection, then we can conclude that the two finite sets have the same size. Usually many such bijections exist, but proving the existence of just one\footnote{Such an existence proof should ideally explicitly construct the bijection. Q: Are there other ways to show that a bijection exists, without explicitly constructing it?} is enough to demonstrate that the two sets have equal size. The idea is now to generalize this to infinite sets: Two sets - finite or infinite - are \emph{defined} to be of equal cardinality whenever there exists a bijective function between them. It's important to realize that this is a definition, not a theorem so it cannot be proven right or wrong. But if decide to go along with this definition, a lot of pretty interesting and partially counterintuitive consequences will necessarily follow from it. Perhaps the most striking one of them is that a strict subset of an infinite set can have the same size as the whole set. For example, the set of even numbers has the same size as the set of all natural numbers - if "size" is understood in the sense of "cardinality", that is. There do exist other notions of size of a set in math - for example based on measures (in the sense of measure theory) or based on asymptotic density (think of the prime counting function to quantify "how many primes there are", for example). However, in classic set theory, the relevant notion of the size of a set is usually taken to be the cardinality.

% Maybe make a paragraph "Intuitions and Definitions" that explains some of the counterintuitive consequences that follow from the "bijection" definition and maybe mention alternative definitions (for example, based on measure theory or asymptotic density). One counterintuitive consequence is that a strict subset of an infinite set can have the same size as the whole set.

% Note that I'm careful here saying "there exists" rather than "we can find" because the latter
% formulation seems to require an explicit construction - and I'm not sure if that is strictly
% necessary or if perhaps more indirect ways of such existence proofs also exist. 
% -> Figure this out! Perhaps a proof-by-contradiction also works? Assume that there exists no 
% bijection and then derive a contradiction from that and therfore conclude that a bijection must
% exist? This would show the existence of a bijection *without* constructing it explicitly.

% The Neumann naturals can serve as prototypes for sets of a given size. Any set that can be put
% into a bijection with 4 = {0,1,2,3} has the same size as 4

\medskip
We do not only want to answer the question whether or not two sets are of equal size. We also want to know, which of the two sets is bigger in the case of non equal sizes. To this end, we observe that for finite sets, whenever we can find an injective but not bijective function $f$ from one set $A$ to another set $B$, then the set $A$ is smaller. Injective means that every element from $A$ is mapped to a different element in $B$. If an injective map is not bijective, i.e. it fails to be surjective, then that means that there are leftover elements in $B$ which are not reached by $f$. We interpret that circumstance as $B$ having a greater size than $A$ such that we cannot establish a one-to-one correspondence. Of course, by flipping the roles of $A$ and $B$, we can also define what it means for set $A$ to be the bigger one of the two. We could perhaps also formulate that in terms of a surjective but non-injective map (verify), but it's more economic to just observe the symmetry of the situation and flip the roles of $A$ and $B$.
...
To get a first impression for how one can prove the existence of bijections, we will now show why $|\mathbb{Q}| = |\mathbb{N}|$ by a proof technique invented by Cantor. I'll take a variation on the usual presentation by actually showing that $|\mathbb{N} \times \mathbb{N}| = |\mathbb{N}|$ from which $|\mathbb{Q}| = |\mathbb{N}|$ follows immediately as quick corollary. By using this technique, one can demonstrate the general possibility to create a bijection between a countably infinite set and its cartesian product with itself. We'll take the set $\mathbb{N}$ as the prototypical example of such a set, but the technique readily generalizes to any countably infinite set $A$ by just pointing out that, by definition of the term "countably infinite", the set $A$ can be put into a bijection with $\mathbb{N}$ and that the composition of two bijections is also a bijection. To demonstrate the existence of a bijection between $\mathbb{N}$ and $\mathbb{N} \times \mathbb{N}$, we will explicitly construct one in the following way: We create a 2D table of the pairs $(n, m)$ where $n,m \in \mathbb{N}$ and then traverse the 2D table in diagonals. It looks like this:
\[
...
\end{cases}
\end{equation}
This may be a bit harder to swallow. But we have already seen how $\mathbb{N}$ can be bijectively mapped to $\mathbb{N}^2$, so it might be somewhat plausible to believe that the set product of infinite sets does not yield larger sets in terms of cardinality. 

\medskip
$\star$ As a historical side note: In the case of mapping $\mathbb{R}$ bijectively to $\mathbb{R}^2$, Cantor himself couldn't believe it at first and famously wrote in a letter to Dedekind "I see it, but I don't believe it". But he found a rather simple bijection: To bijectively map a real number $z$ in the interval $(0,1)$ to a pair of real numbers $(x,y)$ in the same interval, just take the decimal expansion\footnote{Actually, the decimal expansion of many numbers is not unique - consider $1 = 0.999\ldots$. And one perhaps also has to take some care about how to treat the position of the decimal dot. But these are minor complications that can be solved.} of $z$ and alternatingly put digits into $x$ and $y$, i.e. "unzip" the decimal expansion into two. The reverse mapping is just a "zip" operation. Now we also need a bijective map between $(-\infty, \infty)$ and $(0,1)$. A class of functions that work are sigmoids that asymptotically approach $0$ as $x \rightarrow -\infty$ and approach $1$ as $x \rightarrow \infty$. There are many such sigmoids - among them the logistic function and a suitably scaled arc-tangent. So - yeah - it works out to find bijections between $\mathbb{N}$ and $\mathbb{N}^2$ and it works also for  $\mathbb{R}$ and $\mathbb{R}^2$. And it works in general. Of course, these examples are no proof - but they are a plausibility check.
...
% https://www.youtube.com/watch?v=5slSCEsDDKI

% Infinite numbers have only finitely many (nonzero) digits
% https://www.youtube.com/watch?v=dFsa4VeZ0cU
...
\end{equation}
where $k$ is a natural number, the $c_i$ are also natural numbers and the $\beta_i$ are ordinal numbers and are sorted from largest to smallest, i.e. $\beta_1 > \beta_2 > \ldots > \beta_k$. The greatest exponent $\beta_1$ is called the degree of $\alpha$. Compare that to the way in which usually interpret decimal numbers as a weighted sum of powers of $10$. It's very similar here - just that the basis is not $10$ but $\omega$ and the weights are multiplied with the powers from the right. This is really necessary because ordinal multiplication is not commutative and multiplying an infinite ordinal from the left by any natural number changes nothing. The exponents $\beta_i$ are themselves ordinals and therefore can, in turn, also be expressed in Cantor normal form and so on until one eventually arrives at an expression that contains only natural numbers and $\omega$s. This is similar to the hereditary base-$n$ notation of natural numbers. ...TODO: give alternative form without the $c_i$ coeffs

% Explain why this definition via recursion must eventually hit the base case where the exponents become natural numbers. I guess, it has to do with the ordinals being well ordered?
...
% Just like the von Neumann natural numbers can be used as prototypes for finite sets,
% each ordinal number can serve as a prototype for a specific well ordered set. This works because 
% for any well ordered set, there exists a unique ordinal number that is order isomorphic to it. 
% Give examples for that. What is the ordinal that is isomorphic to the integers, rationals, reals,
% etc.?
...
% when ···+1+1+1 != 1+1+1···
% https://www.youtube.com/watch?v=sJs5gLIRR7A
...
% Surreal Numbers (reading group)
% https://www.youtube.com/playlist?list=PLFDAtBX3CjcTUglDoslG708REt92EfnjU


% The Big Bang of Math: The origin of all numbers | Joel David Hamkins and Lex Fridman
% https://www.youtube.com/watch?v=heO9-93q55I

% Surreal Numbers
% https://www.youtube.com/watch?v=AD0l8hh_iN0

% a positive number smaller than every fraction
% https://www.youtube.com/watch?v=lj6uROFHqi0
...
To show that two sets $A$ and $B$ have the same cardinality, we must, by definition, show that a bijection between $A$ and $B$ exists. The Cantor-Bernstein theorem asserts that it suffices to show that an injection from $A$ to $B$ and an injection from $B$ to $A$ exists. If these two injections exist, then the theorem assures that also a bijection between $A$ and $B$ exists. 

\medskip
Constructing these two injections (or indirectly showing that they exist) can, in some situations, be easier than showing directly that a bijection exists. This is because it may be easier to construct two individual maps that both satisfy one criterion (injectivity) than it is to construct a single map simultaneously satisfies two criteria (injectivity and surjectivity). So, the theorem may help us to show that two sets have the same cardinality by (sometimes) simplifying the task of proving the existence of a bijection.
...
% Cantor's Dream | Understanding the Continuum Hypothesis
% https://www.youtube.com/watch?v=2eD7THel0fE
...
% https://www.youtube.com/watch?v=w4VgvJWr9Vg
% -Let S be a partially ordered set. If every totally ordered subset of S has an upper bound in S,
...
% https://www.youtube.com/watch?v=uLFl98iBcxM

% Math's Most Deceptive Sequence
% https://www.youtube.com/watch?v=XUDkQA7cVWI
...
Endless Sizes of Infinity, Explained in 5 Levels
https://www.youtube.com/watch?v=V0VVYsbP0WU


Every Type of Infinity Explained in Under 6 Minutes
https://www.youtube.com/watch?v=vKJfXN2nsSc

Trees: The Most Versatile Structure | From Computer Science to Set Theory #SoME4
https://www.youtube.com/watch?v=Jals63WUsLs

The other side of numbers...
https://www.youtube.com/watch?v=9HZDiLsJ4-Y

Dense vs. Uncountable | Are They Actually Different?
https://www.youtube.com/watch?v=qMg5qlFDpyk
-Has exmples for sets that are uncountable but not dense. Cantor set, Smith-Volterra-Cantor set, ...
 https://en.wikipedia.org/wiki/Smith%E2%80%93Volterra%E2%80%93Cantor_set


Not into Math? Watch This: A Beginner's Guide to Set Theory
https://www.youtube.com/watch?v=lKdYW-2TmqU


The Axiom that Shapes Infinity
https://www.youtube.com/watch?v=Nn_pqCEtfwg


This Infinite Math Object Fits in a Surprising Place | SoME4
https://www.youtube.com/watch?v=ghw_p8CIHfI
-About creating chains of subsets of the natural numbers and how such chains are uncountable. This
 stuff really starts only at around 16:00. Before that, it's mostly well known preliminaries

You can't find such a set | Continuum Hypothesis
https://www.youtube.com/watch?v=h9-eA1qKWqY
...
https://www.youtube.com/watch?v=Fa5MQTDIhrY

What Is Infinity, Actually? (beyond the pop-sci headlines)
https://www.youtube.com/watch?v=rHtqGrtcB1w

This Simple Proof Changed Infinity Forever
https://www.youtube.com/watch?v=ZfSCTP1A2S0
