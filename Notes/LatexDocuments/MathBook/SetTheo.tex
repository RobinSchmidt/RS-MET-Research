...
A set $A$ is called transitive when its element relation is transitive, i.e. $b \in a \in A \Rightarrow b \in A$. This is equivalent to the requirement that each element must also occur as subset: $a \in A \Rightarrow a \subseteq A$. A set $A$ is transitive if and only if the union over all its elements gives a subset of the original set: $A \subseteq \bigcup_{a \in A} a$ or in more compact notation $A \subseteq \bigcup A$...TBC...
