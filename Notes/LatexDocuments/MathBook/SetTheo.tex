...
% -There's a school of thought in the philosophy of mathematics that rejects the axiom of infinity. It's aptly called finitism...
...
%---------------------------------------------------------------------------------------------------
\subsection{Terminology, Notation, Definitions}
In the following we will define a couple of important set theoretical terms that we will need later.
...
A set $A$ is called transitive when its element relation is transitive, i.e. $b \in a \in A \Rightarrow b \in A$. This is equivalent to the requirement that each element must also occur as subset: $a \in A \Rightarrow a \subseteq A$. A set $A$ is transitive if and only if the union over all its elements gives a subset of the original set: $A \subseteq \bigcup A$...TBC...
...
% I think, the simples set that we can construct which is not a natural number is the set
% {{0}}
...
\end{cases}
\end{equation}
Such a definition allows us to plug any set into the function. If we specifically plug ordinal numbers into it, we could plot it as a function mapping ordinals to ordinals. For the finite ordinals, it would be the identity function. For the infinite ordinals, it would be piecewise constant and make an upward jump at every limit ordinal that represents a higher cardinality. For $\omega, \omega+1, \omega^2, \omega^\omega, \varepsilon_0, \ldots$ its would stay constant and just output $\omega$. The first jump would be at $\omega_1$ where it also jumps up to $\omega_1$, etc. The jumps are indeed rather coarsely spaced out. Not even every limit ordinal has a jump. [VERIFY! Much of it is my personal interpretation] ...TBC...
...
The outputs of the $\card$ function are specific ordinal numbers. We are now interested in its ascending sequence of outputs for infinite sets...TBC...
...
% card is used to meausre set sizes, aleph is used to order infinities

% Much the stuff below is obsolete - clean up:

% I think, this is where it gets wrong!
%To this so found ordinal $\alpha$, we now apply a function that we denote by the Hebrew letter "aleph" to compute $\aleph(\alpha)$ which we more typically denote with index notation as $\aleph_{\alpha}$ rather than function application notation with parentheses [WHY?]. That aleph function produces another ordinal number. That is: aleph is a function from the ordinals to the ordinals. Its input can be any ordinal but its output can only be specific ordinals. On infinite ordinals, it behaves qualitatively a bit like a floor function - it steps up at certain ordinals and remains constant in between those. ....TBC...and VERIFY!!!! I made some stuff up according to my own interpretation and I'm totally not sure if it is correct]

%\paragraph{The Aleph Function [VERIFY!]}

% I think, what we explain here is the cardinality function, also denoted as card(A). We should not confuse the card-function with the aleph-function. I think, the purpose of the aleph function is to order infinities whereas the purpose of the card function is to measure the size of the set. these are different things

% Wait - no  - the function-application interpretation is also wrong aleph_0 is NOT aleph(0)
% in fact aleph_0 is aleph(omega). So, the notation alpeh_x is not the same as aleph(x). That may also explain why x <= aleph_x is plausible as said here:
% https://de.wikipedia.org/wiki/Aleph-Funktion
% https://en.wikipedia.org/wiki/Cardinality

% Namely, for any ordinal $\alpha$, our $\aleph$ function will spit out the next smaller limit ordinal ...Nope! w*w is a limit ordinal as well but for w*w, we still get alpeh_0. The steps are even corser than that. Aleph steps up from aleph_0 to aleph_1 at w_1 which is faaar above w*w, w^w, .. . epsilon_0, ...
...
% I think, that interpretation is wrong. The aleph function does not seem to take a set as input but an ordinal number....or well...that actually fits to the interpretation
...
% \alpha = W(A)  where W is the function that well-orders A
% \aleph(\alpha) = |\alpha|
...
% -cardinal numbers can be view as establishing an equivalence relation on sets...just 
%  like ordinal numbers, I think.
...
% -The equivalence is given by existence of a bijection
% -we can also define an order based on existence of injections that are not bijections
...
% Alep_0 is the smallest infinite size because every infinite part of it is still of the same size
...
-Deiser's book says, we obtain the derivative (on R) by first adding the boundary to the set and then
 removing all isolated points - not sure, if that's the same notion.
