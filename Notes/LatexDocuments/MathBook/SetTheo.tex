...
\end{equation}
That is: the real number $\sqrt{2}$ is represented as a pair of (infinite) sets of rational numbers $A$ and $B$. All elements of $A$ are strictly less than $\sqrt{2}$ and all elements of $B$ are greater or equal to $\sqrt{2}$. The general idea is that a real number $x$ cuts the number line of the rational numbers into two parts. The left part contains all rational numbers less than $x$ and the right part contains all rational numbers greater or equal to $x$. The number $x$ itself does not need to be rational. If the real number $x$ happens to be rational, then the rational number representing $x$ becomes a member of the right set $B$. In this case, $x$ is the minimum of $B$. If $x$ is irrational, $B$ has no minimum. In this $x$ becomes the infimum (i.e. greatest lower bound) of $B$. ...TBC...explain addition and multiplication (interval arithmetic)
