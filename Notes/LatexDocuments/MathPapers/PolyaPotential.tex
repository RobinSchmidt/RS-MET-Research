...
\subsection{Products}
Let's turn to products. We assume that:
\begin{equation}
f(z) = g(z) h(z) \quad \text{where}  \quad
g(z) = \sum_k a_k z^k, \quad
h(z) = \sum_m b_m z^m
\end{equation}
...
Looks like the numerators of real and imaginary parts are exactly as in the corresponding positive powers of z? And the denominator is always Re^2 + Im^2? And in the potential, the denominator is (x^2 + y^2)^(n-1). And the numerator of the n-th line is the same as the real part of the (n-1)-th line and the whole thing is divided by n-1? 
