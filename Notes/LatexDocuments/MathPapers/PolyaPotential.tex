...
\end{verbatim}
Looks like the numerators of real and imaginary parts are exactly as in the corresponding positive powers of $z$? And the denominator is always $u^2 + v^2$? And in the potential, the denominator is $(x^2 + y^2)^{n-1}$. And the numerator of the $n$-th line is the same as the real part of the $(n-1)$-th line and the whole thing is divided by $n-1$?  So, I think, the general formulas for the Polya vector field and potential for $f(z) = 1/z^n$ are:

\begin{equation}
\label{Eq:PolyaFieldNegativePowers}
\boxed
{		
\tilde{u}_n(x,y) = \frac{u_n}{u_n^2 + v_n^2} ,\;
\tilde{v}_n(x,y) = \frac{v_n}{u_n^2 + v_n^2} ,\quad
\tilde{P}_n(x,y) = \frac{-u_{n-1}}{(n-1)(x^2+y^2)^{n-1}}
}
\end{equation}
...
[...but these formulas need to be verified. Do it numerically! Maybe move the Sage derivations into an appendix and keep only the results in the main text. Maybe instead of the tilde, use $u_{-n}$ etc.]
