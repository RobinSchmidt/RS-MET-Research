...
% Maybe rather than saying "holomorphy and analyticity imply each other" (which is not wrong but perhaps a somewhat weird framing), it would be better to say that holomorphy implies analyticity where the latter, at first glance, seems to be an even stronger requirement. Surely, for a function to be Taylor expandable, its first derivative must exist. ...and then _on_top_ of that existence of the 1st derivative, all higher derivatives must also exist _and_ the Taylor series must converge.
...
-Maybe mention that there is another way to boil down the 4D information of analytic functions into
 a 3D object: one could just remove either the real or imaginary part. The other part can be
 reconstructed by using the Hilbert transform. See the theory of harmonic functions.
