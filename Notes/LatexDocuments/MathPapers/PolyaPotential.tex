...
\end{itemize}
When we want to read such a contour plot, we need two steps of interpretation: (1) We must be able see the arrows from the contours, i.e. mentally translate the contour plot into an arrow plot. (2) interpret the arrows as real and (negated) imaginary part - or better yet: as magnitude and angle - of a complex number.
...
\end{subfigure}
\end{figure}
For $z^2$, we already know that we get the monkey saddle, i.e. a saddle with space for two legs plus a tail. Let's remind ourselves that for $z^1$, we got a regular saddle which, in this context, is sometimes also called a horse saddle. It has space for tow legs only so it would be suitable for humans. For $z^3$, we get a kind of saddle in which a four legged animal could sit comfortably. Note the different scaling on the color bar for $z^3$. The range is asymmetric here and the zero level is shifted more into the reds because that is the range where the values of $P(x,y)$ happen to fall in when the $x-$ and $y-$ranges are both $[-1,+1]$. In this range, the surface goes more down then up because the corners of the plot rectangle happen to coincide with all minima of $P$ and we generally expect more extreme values further away from the origin and the corners are furthest from the origin. That's merely a technical aspect of the plotting. Anyway - we saw that for $z^1$, we got a regular two-leg saddle, for $z^2$ we got a 3-leg saddle and for $z^3$ we got a 4-leg saddle. A pattern seems to be emerging - and it is indeed true that for $z^n$, our potential surface $P(x,y)$ looks like a saddle surface of order $n+1$. For a discussion of such higher order saddles, see \cite{HighOrderSaddles}.
...
\end{subfigure}
\end{figure}
There is another noteworthy feature of this sequence of saddle surfaces: The higher the order of the saddle, the flatter the interior region near the origin and the steeper the cliffs further away from the origin. ...TBC...

\subsection{Plots for Negative Powers: $f(z) = z^{-n}$}
Having seen Polya potentials of positive power of $z$, the logical next step is to ask how Polya potentials for negative powers of $z$ look like. In figure \ref{Fig:ContoursInvPow_1_2}, we see the contour maps for $1/z$ and $1/z^2$. 
\begin{figure}[p]
...
ToDo:
-The placement of the plots in the document makes no sense. How can we convince LaTeX to 
 place them where they belong? Maybe we need to make a special appendix for the figures.
 ...
 plot that occurs sporadically with a lot of text, h seems to give better results.
