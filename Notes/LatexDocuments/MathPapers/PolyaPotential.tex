...
\subsection{Plots for $f(z) = z^n$}
We now want to look at plots for functions of the form $f(z) = z^n$ for some nonnegative integer $n$. We have already seen that the Polya potential for $f(z) = z^2$ gives us, up to scaling, the monkey saddle $P(x,y) = x^3/ 3 - x y^2$. In general, we will get a saddle surface of order $n+1$ for the Polya potentials $P(x,y)$ for complex functions of the form $f(z) = z^n$ [VERIFY!]. For a discussion of such higher order saddles, see \cite{HighOrderSaddles}.
% Show plots for z^0, z^1, z^2, z^3, z^4, z^5

% We get a saddle point of order n+1
