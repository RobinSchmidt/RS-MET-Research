...
\centering
\includegraphics[width=1.0\textwidth]{../MathFigures/PolyaSurfaceSquare.png}
\end{figure}
...
\centering
\includegraphics[width=\textwidth]{../MathFigures/PolyaVectorsSquare.png}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{../MathFigures/PolyaContoursSquare.png}
\end{subfigure}
\end{figure}
We interpret this as follows: when the input to $P(x,y)$ is of the form $(x,0)$, then the gradient will point into some direction $(a,0)$ where $a$ is always positive except at $(0,0)$ where $a=0$. More quantitatively, $a=x^2$ which is the partial derivative wrt $x$ of $P(x,y) = x^3/3 - x y^2$ along the line where $y=0$. The surface for $f(z) = z^2$ given by $P(x,y) = x^3/3 - x y^2$ is, up to a scale factor, actually well known: it is called the monkey saddle, see \cite{MonkeySaddleWiki}. In contrast to a regular saddle, which provides channels for the two legs of a horseman, the monkey saddle also provides an additional third channel for the tail. In the plot of the contour map on the right of figure \ref{Fig:SquareVectorsAndContours}, we notice that wherever a contour line has vertical segment, the corresponding arrow of the vector field would be horizontal. This can also be seen as a consequence of the fact that on any surface described by a bivariate function $z = P(x,y)$, the gradient is always orthogonal to the lines of constant height, i.e. the contour lines. This fact gives us a general rule for how to interpret contour map plots such as the one in the right of figure \ref{Fig:SquareVectorsAndContours}: wherever we see contour lines densely bunched together, there is a steep gradient - that is: a gradient with high absolute value. The direction of that gradient (steep or not) is always perpendicular to the contour line at the given point. From the two possible perpendicular directions, it's the one that points upward, i.e. toward increasing $z$-values. In our plots here, that means into the direction of greater darkness and more blue. Up to now, we have identified the part of the domain, where the output of $f(z) = z^2$t is purely real, i.e. the gradient of $P(x,y)$ is horizontal. These are the precisely $x$- and $y$-axis. Along the $x$-axis, the gradient points to the right indicating a positive real part of $f(z)$ and along the $y$-axis, the gradient points to the left indicating a negative real part of $f(z)$. Along these axes, the imaginary part of $f(z)$ is zero.
...
-Don't use z for the height as in z = P(x,y) when we already use z as argument to f = f(z). The letter z means different things in these two situations which is a clash of notation.
