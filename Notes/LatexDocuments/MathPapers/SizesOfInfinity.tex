...
The additivity condition is sometimes also formulated as $|A \cup B| = |A| + |B|$ when $A,B$ are assumed to be disjoint or as $|A \sqcup B| = |A| + |B|$ where $|A \sqcup B|$ is the disjoint union. We don't want to restrict ourselves to disjoint sets nor do we want to deal with the more complicated operation of taking a disjoint union so we just subtract out the number of the doubly counted items again. This formula is actually a special case (for $n=2$) of the more general inclusion-exclusion principle that holds for the size of the union of $n$ (finite) sets. Some of the names of the listed facts are not standardized - I just made them up to be able to refer to the facts by name later.

% Maybe mention that the formula for the union is a special case of the inclusion-exclusion principle, namely the n=2 case (where n is the number of sets that we unionize).

% Q: Is there also a quotient law? Maybe call them "sum law", "product law", etc. ..or maybe "... rule"
...
\paragraph{}
It is noteworthy here to compare the so found exponential growth function $f(u) = 2^u$ for the uncountable set $[0,1)$ to the previously found a sum-of-factorials growth function for the countable set of all permutations. Factorials grow faster than exponentials and their sum even faster than that. That implies that the framework to measure set sizes in terms of growth functions is not a refinement of Cantor's cardinalities but may actually disagree with the cardinality way of comparing set sizes. One set may be considered bigger than another by cardinality yet smaller by growth. That actually surprised me a bit. I expected agreement and refinement.
...
\begin{eqnarray}
 ...
 f(U) =& (b-a) U + 1                 & \qquad [a,b]
\end{eqnarray}
Again, a leap of faith is required. I boldly claimed that this is the "most natural" thing to do. But is it really? And even if you agree that it is - is it therefore the right thing? I don't know, but remember that we are trying to model intuitions here, so I'll just run with it for now and see where it leads. [NOTE: Maybe it could be justified by appealing to measure theory. I think, what we are really trying to do here is to develop a function-valued measure for sets.]
...
described in RS-MET-Research\Notes\LatexDocuments\MathBook\Scratch\SetTheory.txt.
Discuss what this means with respect to how fast sequences of sets can grow. It appears that the growth of all sets should set an upper bound on how fast sets can grow - but I think, this is only due to allowing only elementary operations in the set birthing process. If at each step/day, we would allow to form the set of all functions of all previously known sets, we'd get a faster growing sequence of sets. As derived

Consider the size of the set of all injective functions from a set A to another (bigger) set B. Let
a = |A|, b = |B|. Take as example A = {1,2,3}, B = {4,5,6,7,8}. We need to map 1 somewhere. For this we have b possibilities. Then we need to map 2 for which we have b-1 possibilities because the functions needs to be injective so we can't choose the same image as for 1. For mapping 3, we have b-2 possibilities and so on. I think, we arrive in general at 
(b-0)*(b-1)*(b-2)*(b-3)*...*(b-(a-1))  ...verify and compare with size of the set of all bijections from a set A of size a to itself - this is the set of all permutations and therefore, the number is given by a!.
...
https://www.youtube.com/watch?v=NunMJxrRJJM
-about asymptotic density
...
Maybe include a paragraph that compares how different ways of quantifying the size of infinite sets
compare the sizes of 3 infinite sets: P: primes, N: naturals, Q: rationals, R: reals, R^2 RxR: 2D vectors of reals. The 3 ways to quantify the size are called C(..): cardinality, M(): measure. D(): asymptotic density or growth:

Cardinality:     P = N, N = Q, Q < R, R = R^2
Density/Growth:  P < N, N < Q, Q < R, R < R^2
1D Measure:      P = N = Q = 0, Q < R, undef?
2D Measure:      P = N = Q = R = 0, R < R^2   

Is there also a 0D measure, i.e. a measure on discrete sets like P,N,Q or does the idea only apply to continuous sets like R and R^2? Maybe also include the unit interval (I or U...look up if there's a common notation for it) and the unit square I^2 or U^2. Maybe as yet another way to quantify infinite sets consider the idea of dimension. I think discrete sets like the a single point, a finite number of points or a countable number of points are zero dimensional, the unit interval is 1D, the unit square is 2D, etc. I think, via dimension, the unit square is indistinguishable from the whole R^2 whereas by a 2D measure, they are distingushable: the unit square gets the value 1 whereas R^2 gets the value inf. What about objects with fractal dimensions such as the Cantor set?



Conclusion: Infinity is complicated and there is more than one way to quantify it and the different ways will sometimes give different results for particular size comparisons. When asking a question like: "Are there more natural numbers than rational numbers?" or "Are there more real 2D vectors in the unit square than real numbers in the unit interval?" we need to be more specific what we need by "more", i.e. which method of size comparison we want to use. Questions like: "How many..." cannot be assumed to be automatically and intuitively understood correctly. When we talk about finite sets, every "How many..." question has a natural number as answer. With infinite sets, this is not so and we must more precisely specify what we actually mean by that phrase. It's not self evident anymore.


Intuitively if one set A is a strict subset of another set B, we would say that A < B because when we take away all elements a in A from B, there will be something left over. When Cantor says that A < B when there exists and injective map from A to B that fails to be surjective, i.e. doesn't reach everey b in B, this seems to appeal to the sam intuition that there are element in b left over that are not reached by the map.

The big disadvantage of using the strict subset criterion for comparing infinite sets is that is not always applicable. We can apply it only, if one set is the subset of the other. Cantor's beijection based definition circumvents that by allowing a map between the sets.

In particular for subsets of the naturals, we could quantify their infiniteness by considering how slow or fast the sum of their reciprocal converges or diverges. Not sure, if that's a useful measure, though. 

How about considering the amount of information needed to store a finite number of elements and then looking at how that grows when storing all elements. We would somehow need to define a sampling algorithm that exhaustively samples the whole set in the limit as some variable n goes to infinity.


One justification for using cardinalities could also be the fact that vector spaces are isomorphic
iff they have the same dimension. In this case, the "dimension" is understood to be a cardinality.
See:
https://www.youtube.com/watch?v=VgyIKqNmA74
at around 40 min. At 45:30, he says trhat R^R has the dimension 2^c where c is the cardinality of 
the continuum. But careful: dimension does not mean cardinatlity. What cardinality does R^R have?
