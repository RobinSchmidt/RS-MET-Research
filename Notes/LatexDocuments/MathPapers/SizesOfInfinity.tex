...
Is there also a 0D measure, i.e. a measure on discrete sets like P,N,Q or does the idea only apply to continuous sets like R and R^2? Maybe also include the unit interval (I or U...look up if there's a common notation for it) and the unit square I^2 or U^2. Maybe as yet another way to quantify infinte sets consider the idea of dimension. I think discrete sets like the a single point, a finite number of points or a countable number of points are zero dimensional, the unit interval is 1D, the unit square is 2D, etc. I think, via dimension, the unit square is indistinguishable form the whole R^2 whereas by an 2D measure, they are distingushable: the unit suqare gets the value 1 whereas R^2 gets the value inf.
