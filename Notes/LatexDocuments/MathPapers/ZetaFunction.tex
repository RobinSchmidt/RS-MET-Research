...
...TBC... Some questions and wild guesses: Could it be that the (nontrivial and/or trivial) stationary points are connected by geodesics? If so, could these geodesics for the nontrivial ones have $x=1/2$ everywhere, i.e. go straight up along $y$? Could that mean something with regard to the Riemann hypothesis? What about the directions of principal curvature at these points (or elsewhere)? What about Gaussian curvature, Riemann tensor, Ricci tensor, metric tensor, Christoffel symbols, etc.? What about the Laplacian, divergence, etc. of the gradient field (curl is zero in a potential field)?
