...
\medskip
We can in general just always use $V_n(x,y)$ and add to it a $x^{n+1}/(n+1)$ term as integration constant because all the $u_n$ have exactly one term that is independent of $y$ and that term is always $x^n$ which, when integrated, gives $x^{n+1}/(n+1)$. That gives:
\begin{equation}
...
\end{equation}
VERIFY that! 
The full Polya potential is then be given by re-assembling the sum:
\begin{equation}
