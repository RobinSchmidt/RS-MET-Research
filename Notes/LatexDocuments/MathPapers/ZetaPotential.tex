...
\end{equation}
see \cite{Boost}, \cite{Wiki}. [ToDo: VERIFY them and write them in a more consistent form (they have been grabbed from different sources)! Maybe explain the gamma function briefly. Mention that they are also often referred to as the functional equation(s) and how they are super important for defining the zeta function outside the region of convergence of the sum and product expressions]
...
% Actually, it's only one of the Cauchy-Riemann equations that ensures the symmetry of the Jacobian. The other one (u_x = -v_y) ensures that the gradient field has the additional property of being divergence-free. A vector field being a gradient field does by itself only ensure that it is curl-free but it may still have divergence.
...
% the part below is now also in the PolyaPotential.tex paper and can be removed here:
\medskip 
...
% up to here, the text has been copied into PolyaPotential.tex, so it can be removed up to here and we
% can refer to the PolyaPotential paper
...
% text from here has also been moved into PolyaPotential.tex:
\medskip
...
\end{equation}
% up to here, the text has been copied into PolyaPotential.tex
...
%...Questions: Are there any saddles along the critical line or will we just see alternating minima and maxima as we move upward in the $y$ direction at $x=1/2$? Are there any saddles anywhere at all? I think, there have to be some, if minima and maxima exist (right?). What about the stationary points associated with the trivial zeros?...TBC...I really need to create some plots and add them here but my implementation does not yet converge fast enough for points further away from $s=1$ to show the interesting parts of the landscape. More work needs to go into the code before I can use it to create plots. In particular, I need more of the $\gamma_n$ or $c_n$ coefficients. The code currently uses a precomputed table of $\gamma_n$ for $n$ up to $10$. That's enough for proof of concept experiments and some unit tests but not for plotting.

\subsection{Analyzing the Geometry of $P(x,y)$}
It could be interesting to view the surface defined by $P(x,y)$ through the lens of differential geometry. Differential geometry of 2D surfaces deals mostly with questions about curvature of these surfaces but also with things like geodesics, torsion of curves (in the surface), etc.. In differential geometry, in particular in surface theory, the setting is usually framed in the following way: we assume that we have three bivariate functions $x(u,v), y(u,v), z(u,v)$ where in this notation, our $u,v$ are actually assumed to be inputs (called parameters or coordinates in this context) and $x,y,z$ are the outputs (the points in 3D space that belong to the surface), i.e. the notation in surface theory has it the other way around compared to what we used previously in the context of vector fields where $x,y$ were the inputs and $u,v$ the outputs. Switching to that notation, a bivariate function such as $P(x,y)$ is a special case of that in which we have $x(u,v) = u, y(u,v) = v, z(u,v) = P(x(u,v),y(u,v))$. ...TBC... 

% Some questions and wild guesses: Could it be that the geodesics between the nontrivial saddles have $x=1/2$ everywhere, i.e. go straight up along $y$? Could that mean something with regard to the Riemann hypothesis? What about geodesics between trivial and nontrivial zeros of same index? What about the directions of principal curvature at the saddles (or elsewhere)? What about Gaussian curvature, Riemann tensor, Ricci tensor, Ricci scalar, metric tensor, Christoffel symbols, etc.? Lots of stuff to explore in this zeta-related landscape!

% What about the Laplacian, divergence, etc. of the gradient field (curl is zero in a potential field)? ..I think, the others are also zero. Divergence by virtue of the first CR equation, Laplacian because u, v are harmonic conjugate functions.
...
%Maybe we could also try to numerically compute $P$ from zeta itself. I don't really know algorithms to numerically compute potentials from vector fields but I have some ideas for how it could be done...TBC...


%Other ways could be based on other expressions for the Polya potential which I did not yet derive. 


%which, in the special case of a bivariate function, is related ...TBC...
...
-What, if we draw a geodesic from the 1st to the 2nd nontriival zero and then another one
 from the 2nd to the 3rd. Will the curve made from these two segments be continuous (or even
 smooth) at the junction? We can do this numerically.
...
ToDo:
-Give expressions for the entries of the Jacobian. They can be derived from the 
 expressions of the Polya vector fields. Such expressions may be needed to compute
 the curvature of the surface defined by the potential and other differential 
 geometry features. Of fundamental importance in diff. geo. is the metric tensor, so 
 maybe try to compute expressions for that first. I think, for this, we don't even need
 the potential and can start directly with the partial derivatives of it which are 
 actually our given functions. Using the notation from here
 https://en.wikipedia.org/wiki/Metric_tensor we would get:
   r(u,v) = (x(u,v),y(u,v),z(u,v)) = (u,v,P(u,v))
 and with
   E = r_u . r_u ,  F = r_u . r_v ,  G = r_v . r_v
   r_u = (1,0,P_u), r_v = (0,1,P_v)
 so: E = P_u^2, F = P_u P_v, G = P_v^2   
 Maybe to not get confused, call the coordinates in the diff-geo part a,b instead of 
 u,v. Then we can keep using u,v for the real and negated imag part of our complex 
 function, i.e. the partial derivatives of P which are then also functions of a,b
   r(a,b) = (x(a,b),y(a,b),z(a,b)) = (a,b,P(a,b))
 Then
   E = r_a . r_a, F = r_a . r_b, G = r_b . r_b
   r_a = (1,0,P_a), r_b = (0,1,P_b)
   E = P_a^2, F = P_a P_b, G = P_b^2    
 and then we can use our original Polya vector field notation for u, v and we have:  
   P_a = u(a,b), P_b = v(a,b)
 and so: 
   E = u^2, F = u v, G = v^2
 ...that seems to mean that in order to to do diffgeo on complex potentials, we don't 
 even need an expression for the potential itself. Expressions for the real and imag part 
 of the function are enough. Does that mean, we can forget about the embedding of the 
 potential surface in 3D and just think about its intrinsic geometry? Maybe work it out
 for a simple but not trivial example function like z^2 or 1/z or e^z or z^2 / (1 + z)
 or ((z+1)*(z-1)) / ((z+i)*(z-i)). What happens if we apply the formulas for E,F,G for the
 metric tensor to a general vector field that is not necessarily derived from a potential
 and therefore has no geometric interpretation in terms of a surface? Can this still say
 something of interest about the vector field? Maybe compute its determinant. Then we have
 scalar field derived from the vector field. Maybe plot it for some well understood example
 fields. 
   
-Maybe split out a paper about computing Polya potentials for some simple functions
 like 1/z, z^n, (1/z)^n. That paper could then also tackle some more functions like exp, 
 sin, cos, tan, log, sqrt, pow, sinh, cosh, tanh, asin, acos, etc. Cite that paper here. 
-Try to find a prodcut rule and quotient rule for Polya potentials. Maybe it should be 
 based on multiplication and division of the power series coeffs, i.e. convolution and
 deconvolution, aka Cauchy-product (an Cauchy-quotient?). Actually, that would be rules 
 for multiplying and divding power series in general. It doesn't matter if we use the 
 result to compute coeffs for the pwer series of Polya potential or for soemthing else.
 Could product and quotien rule for f*g and f/g be possible that avoids the use of a 
 power series expansion and is just written in terms of f,g themselves? What about a 
 chain rule? Maybe that should be based on an algorithm to nest polynomials.
-In that paper, we could also define a new type of complex integral. For the regular 
 complex integral, we have
 $I   = \int_a^b f(z) dz = F(b) - F(a)$ where $F$ is an antiderivative of $f$. Let's define
 $I_P = \int_a^b \overline{f(z)} dz = P(b) - P(a)$ where $P$ is a Polya potential for $f$.
 I think, this identity should hold (verify!). Unlike $I$, $I_P$ would always be a real 
 number. Q: what happens if we do such an integral around a closed loop around a (simple) 
 pole? The regular integral would return the residue of the pole (times $2 \pi \i$, I think). What would this new integral return in this case? Maybe try it numerically! Or maybe try
 to analytically find a path integral of the Polya potential of $1/z$ given by 
 $u = x/(x^2+y^2), v = y/(x^2+y^2)$ around a loop enclosing $0$, maybe the unit circle.
 Ah - this video defines soemthing similar:
 https://www.youtube.com/watch?v=itEqPTJpxUo

At 7:30, this video has a nice, short derivation of the equality of the Euler product and 
Riemann's series definition of zeta:
https://www.youtube.com/watch?v=i2e_9Ms7Jg4


Maybe instead of zeta(s) itself, consider (z-1) * zeta(s). The multiplication by (z-1) cancels the
pole of zeta at s=1 so the resulting function may be easier to deal with. Maybe it's easier to
evaluate algorithmically because its Laurent series converges faster? The function becomes 
holomoprhic in the whole complex plane, i.e. is an entire function.

Find the Polya potential of that function. Maybe it has "nicer" geometric features than the original zeta function? Look at equipotentials and geodesics between the saddles.
